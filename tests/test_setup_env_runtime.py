...
def test_compose_defaults_keep_semantic_and_mechanical_models_separate() -> None:
    ...
    for source in (compose, core_compose):
        assert "AGVM_LLM_MODEL: ${AGVM_LLM_MODEL:-gpt-4.1-mini}" in source
        assert "AGVM_RETRIEVAL_MODEL: ${AGVM_RETRIEVAL_MODEL:-gpt-4.1-mini}" in source
        assert "AGVM_ANSWER_MODEL: ${AGVM_ANSWER_MODEL:-gpt-4.1-mini}" in source
        assert "AGVM_PLANNER_MODEL: ${AGVM_PLANNER_MODEL:-gpt-4.1-mini}" in source
        assert "AGVM_AI_SPATIAL_MODEL: ${AGVM_AI_SPATIAL_MODEL:-gpt-4.1-mini}" in source
        assert "AGVM_BRANCH_CONTROLLER_MODEL: ${AGVM_BRANCH_CONTROLLER_MODEL:-gpt-4.1-mini}" in source
        assert "AGVM_EVIDENCE_JUDGE_MODEL: ${AGVM_EVIDENCE_JUDGE_MODEL:-gpt-4.1-mini}" in source
        assert "AGVM_MASTER_MODEL: ${AGVM_MASTER_MODEL:-gpt-4.1-mini}" in source
        assert "AGVM_GROW_SEMANTIC_MODEL: ${AGVM_GROW_SEMANTIC_MODEL:-gpt-4.1-mini}" in source
        assert "AGVM_LLM_MAX_CONCURRENT_REQUESTS: ${AGVM_LLM_MAX_CONCURRENT_REQUESTS:-1}" in source
        assert "AGVM_LLM_QUEUE_TIMEOUT_SECONDS: ${AGVM_LLM_QUEUE_TIMEOUT_SECONDS:-30}" in source
        assert (
            "AGVM_GROW_AI_INVESTIGATOR_WALL_SECONDS: "
            "${AGVM_GROW_AI_INVESTIGATOR_WALL_SECONDS:-600}"
        ) in source
        assert (
            "AGVM_GROW_AI_INVESTIGATOR_TIMEOUT_SECONDS: "
            "${AGVM_GROW_AI_INVESTIGATOR_TIMEOUT_SECONDS:-180}"
        ) in source
        assert (
            "AGVM_GROW_AI_REVIEW_RESERVE_SECONDS: "
            "${AGVM_GROW_AI_REVIEW_RESERVE_SECONDS:-180}"
        ) in source
        assert "AGVM_COMPILER_MODEL: ${AGVM_COMPILER_MODEL:-gpt-4o-mini}" in source

    assert "AGVM_LLM_MODEL=gpt-4.1-mini" in env_example
    assert "AGVM_RETRIEVAL_MODEL=gpt-4.1-mini" in env_example
    assert "AGVM_ANSWER_MODEL=gpt-4.1-mini" in env_example
    assert "AGVM_PLANNER_MODEL=gpt-4.1-mini" in env_example
    assert "AGVM_AI_SPATIAL_MODEL=gpt-4.1-mini" in env_example
    assert "AGVM_BRANCH_CONTROLLER_MODEL=gpt-4.1-mini" in env_example
    assert "AGVM_EVIDENCE_JUDGE_MODEL=gpt-4.1-mini" in env_example
    assert "AGVM_MASTER_MODEL=gpt-4.1-mini" in env_example
    assert "AGVM_GROW_SEMANTIC_MODEL=gpt-4.1-mini" in env_example
    assert "AGVM_GROW_AI_INVESTIGATOR_WALL_SECONDS=600" in env_example
    assert "AGVM_GROW_AI_INVESTIGATOR_TIMEOUT_SECONDS=180" in env_example
    assert "AGVM_GROW_AI_REVIEW_RESERVE_SECONDS=180" in env_example
    ...
    public_env_path = ROOT / "public-core-docs" / "examples" / "env.example"
    if not public_env_path.exists():
        assert (ROOT / ".agvm-public-export-marker").exists()
        public_env_path = ROOT / ".env.example"
    public_env = public_env_path.read_text(encoding="utf-8")
    assert "AGVM_LLM_MODEL=gpt-4.1-mini" in public_env
    assert "AGVM_GROW_SEMANTIC_MODEL=gpt-4.1-mini" in public_env
    assert "AGVM_LLM_MAX_CONCURRENT_REQUESTS=1" in public_env
    assert "AGVM_LLM_QUEUE_TIMEOUT_SECONDS=30" in public_env
...
def test_model_capability_targets_are_role_based_and_deduped(
    ...
    assert {"role": "grow_semantic", "model": "gpt-4.1-mini"} in targets
...
def test_provider_key_test_is_bounded_non_persisting_and_redacted(
    ...
    assert response.json()["models"] == [
        ...
        {"role": "retrieval", "model": "gpt-4.1-mini", "status": "valid"},
        {"role": "answer", "model": "gpt-4.1-mini", "status": "valid"},
        {"role": "planner", "model": "gpt-4.1-mini", "status": "valid"},
        {"role": "ai_spatial", "model": "gpt-4.1-mini", "status": "valid"},
        {"role": "branch_controller", "model": "gpt-4.1-mini", "status": "valid"},
        {"role": "evidence_judge", "model": "gpt-4.1-mini", "status": "valid"},
        {"role": "master", "model": "gpt-4.1-mini", "status": "valid"},
        {"role": "grow_semantic", "model": "gpt-4.1-mini", "status": "valid"},
    ]
