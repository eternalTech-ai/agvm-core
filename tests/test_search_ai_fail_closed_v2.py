...
def test_search_ai_call_retries_once_after_provider_queue_timeout(monkeypatch: Any) -> None:
    monkeypatch.setattr(retrieval, "llm_enabled", lambda: True)
    calls: list[dict[str, Any]] = []
    execution_record: dict[str, Any] = {}

    def fake_structured_json(**kwargs: Any) -> tuple[dict[str, Any] | None, str | None]:
        calls.append(dict(kwargs))
        if len(calls) == 1:
            return None, "llm_queue_timeout:waited_ms=30000.12"
    ...
    payload = retrieval._run_attested_search_ai_json(
        ...
        system_prompt="Return the requested Search control payload.",
        user_prompt="Search control input.",
        schema_name="agvm_public_v1_landing_plan_v1",
        schema={"type": "object", "properties": {"ok": {"type": "boolean"}}},
        timeout=60.0,
        role="ai_spatial",
    )
    ...
    assert len(calls) == 2
    assert calls[0]["schema_name"] == calls[1]["schema_name"] == "agvm_public_v1_landing_plan_v1"
    assert execution_record["call_name"] == "ai_spatial_landing"
    assert execution_record["provider_queue_retry"] == {
        "schema_version": "agvm.search_ai_provider_queue_retry.v1",
        "retry_used": True,
        "retry_status": "recovered",
        "primary_error": "llm_queue_timeout:waited_ms=30000.12",
        "retry_reason": "llm_queue_timeout",
    }


def test_search_ai_call_does_not_retry_deadline_exhausted_queue_timeout(monkeypatch: Any) -> None:
    monkeypatch.setattr(retrieval, "llm_enabled", lambda: True)
    calls = 0

    def fake_structured_json(**_kwargs: Any) -> tuple[None, str]:
        nonlocal calls
        calls += 1
        return None, "llm_queue_timeout:deadline_exhausted:waited_ms=30000.12"

    monkeypatch.setattr(retrieval, "structured_json", fake_structured_json)

    with pytest.raises(retrieval.SearchAiExecutionError) as raised:
        retrieval._run_attested_search_ai_json(
            call_name="ai_spatial_landing",
            model="gpt-4.1-mini",
            system_prompt="Return the requested Search control payload.",
            user_prompt="Search control input.",
    ...
    assert calls == 1
    assert raised.value.provider_error == "llm_queue_timeout:deadline_exhausted:waited_ms=30000.12"


def test_attested_search_ai_provider_reports_provider_error_as_contract_tuple(
    monkeypatch: Any,
) -> None:
    def failing_search_call(**_kwargs: Any) -> dict[str, Any]:
        raise retrieval.SearchAiExecutionError(
            "ai_spatial_landing:agvm_public_v1_landing_plan_v1",
            "transport_error:timed_out",
        )

    monkeypatch.setattr(retrieval, "_run_attested_search_ai_json", failing_search_call)

    provider = retrieval._attested_search_ai_provider("ai_spatial_landing")

    payload, error = provider(schema_name="agvm_public_v1_landing_plan_v1")

    assert payload is None
    assert error == "transport_error:timed_out"


def test_fast_planner_seed_anchors_opaque_identifiers_to_brain_local_content(monkeypatch: Any) -> None:
    monkeypatch.setattr(retrieval, "llm_enabled", lambda: True)
    calls: list[dict[str, Any]] = []

    def fake_structured_json(**kwargs: Any) -> tuple[dict[str, Any], None]:
        calls.append(dict(kwargs))
        kwargs["execution_metadata"].update(_attestation())
        return {
            "answer_strands": [
                {
                    "answer_field": "marker_identity",
                    "answer_hypothesis": "AMBRA-195, the complete SHA, and Europe/Rome are brain-local release markers.",
                    "goal": "recover exact brain-local release marker facts",
                    "landing_hint": "AMBRA-195 SHA Europe/Rome",
                    "priority": 0.93,
                    "destination_queue": [
                        {
                            "label": "release marker evidence",
                            "guide_area": "History",
                            "memory_type": "episodic",
                            "radial_expectation": "inner",
                            "priority": 0.91,
                            "rationale": "Exact marker facts are stored in reviewed brain content.",
                        }
                    ],
                    "required_entities": ["AMBRA-195", "Europe/Rome"],
                    "expected_evidence": ["complete SHA tied to AMBRA-195"],
                    "success_criteria": ["answer exact marker, full SHA, and timezone from brain content"],
                }
            ],
            "seed_summary": "Brain-local opaque marker retrieval.",
            "seed_query_class": "direct_fact",
        }, None

    monkeypatch.setattr(retrieval, "structured_json", fake_structured_json)

    payload, error = retrieval._run_fast_planner_seed_request(
        RetrieveRequest(
            query_text="Qual è il marcatore AMBRA-195, lo SHA completo e il fuso Europe/Rome associato?"
        ),
        identity_source={},
    ...
    assert payload and payload["answer_strands"]
    assert len(calls) == 1
    system_prompt = calls[0]["system_prompt"]
    assert "opaque identifiers" in system_prompt
    assert "brain-local retrieval anchors" in system_prompt
    assert "do not reinterpret them as external scientific, biological, molecular, or public-database domains" in system_prompt
    assert "AMBRA-195" in calls[0]["user_prompt"]
    assert "Europe/Rome" in calls[0]["user_prompt"]
...
def _materialized_spatial_contract(
    answer_strands: list[dict[str, Any]] | None,
) -> dict[str, Any]:
    strand = dict((answer_strands or [{}])[0])
    mission_id = str(
        strand.get("mission_id") or strand.get("strand_id") or "strand_ai_1"
    )
    return {
...
def test_optional_ai_spatial_landing_gap_does_not_block_clean_install_search(
    monkeypatch,
) -> None:
    def spatial_gap(**_kwargs: Any) -> dict[str, Any]:
        return {
            "schema_version": "agvm.public_v1_landing_landing_contract.v1",
            "status": "blocked",
            ...
            "materialized": False,
            "certifiable": False,
            "error": "invalid_json:inverse_answer_paths_missing,direct_sharded_ai_spatial_failed",
            "missing_reasons": [
                "inverse_answer_paths_missing",
                "direct_sharded_ai_spatial_failed",
            ],
            "inverse_answer_paths": [],
            "metrics": {"ai_landing_count": 0, "ai_path_count": 0},
        }

    monkeypatch.setattr(retrieval, "build_public_v1_landing_contract", spatial_gap)
    ...
    spatial_runtime = plan["ai_spatial_landing_contract_runtime"]
    assert spatial_runtime["materialized"] is False
    assert spatial_runtime["source"] == "fresh_llm_sharded"
    assert "direct_sharded_ai_spatial_failed" in spatial_runtime["missing_reasons"]
    assert plan["planner_runtime"]["planner_path"] == "ai_attested"


def test_runtime_optional_ai_spatial_gap_reaches_attested_probe_execution(
    monkeypatch,
) -> None:
    ...
    def spatial_gap(**kwargs: Any) -> dict[str, Any]:
    ...
    def stop_after_runtime_spatial(*_args: Any, **_kwargs: Any) -> dict[str, Any]:
        raise StopAfterRuntimeSpatial()

    monkeypatch.setattr(retrieval, "llm_enabled", lambda: True)
    monkeypatch.setattr(retrieval, "build_public_v1_landing_contract", spatial_gap)
    monkeypatch.setattr(retrieval, "_master_loop_executable_work", stop_after_runtime_spatial)

    query = RetrieveRequest(query_text="Find reviewed context.", retrieval_mode="balanced")
    plan = retrieval.prepare_runtime_plan(
        query,
        {"buckets": []},
        {"core_nodes": []},
    ...
    with pytest.raises(StopAfterRuntimeSpatial):
        retrieval.retrieve_runtime(
            ...
            {"core_nodes": []},
            prepared_plan=plan,
            search_id="search-runtime-spatial-clean-install-gap",
        )

    assert len(spatial_calls) == 2
    assert all(call["allow_ai"] is True for call in spatial_calls)


def test_admitted_flash_query_plan_materializes_ai_spatial_preflight(
    monkeypatch,
) -> None:
    ...
    def spatial_builder(**kwargs: Any) -> dict[str, Any]:
        ...
        return _materialized_spatial_contract(kwargs.get("answer_strands"))
    ...
    assert builder_calls[0]["deferred"] is False
    assert builder_calls[0]["allow_ai"] is True
    assert "cache_only" not in builder_calls[0]["mode_budget"]
    assert spatial["source"] == "fresh_llm"
    assert spatial["materialized"] is True
    assert spatial["certifiable"] is True
    assert spatial_runtime["source"] == "fresh_llm"
    assert spatial_runtime["materialized"] is True
    assert plan["planner_runtime"]["planner_path"] == "ai_attested"
    assert plan["planner_seed_runtime"]["planner_seed_source"] == "llm"
    assert all(item["planner_family"] == "ai" for item in plan["answer_strands"])


def test_query_run_reuses_admitted_ai_spatial_before_route(monkeypatch) -> None:
    class StopAfterRuntimeSpatial(Exception):
        pass
    ...
    assert spatial_calls[0]["deferred"] is False
    assert dict(plan["ai_spatial_landing_contract"])["source"] == "fresh_llm"
    ...
    assert [call["deferred"] for call in spatial_calls] == [False]
    event_types = [event_type for event_type, _ in events]
    assert "ai_spatial_materialization_started" not in event_types
    assert "ai_spatial_materialization_completed" not in event_types
...
def test_late_invalid_json_materializes_fourteen_matches_into_review_package(
    monkeypatch,
) -> None:
    monkeypatch.setattr(retrieval, "llm_enabled", lambda: True)
    provider_calls = 0
    matches = [
        {
            "node_id": f"reviewed-project-{index}",
            "title": f"Reviewed source family {index}",
            "summary": f"Distinct grounded project evidence family {index}.",
        }
    ...
    def runtime_impl(*args, event_callback=None, **kwargs):
        ...
        if event_callback:
            event_callback(
                ...
                {
                    "matches": matches,
                    "visited_node_ids": [item["node_id"] for item in matches],
                },
            )
        _runtime_ai_call("master_judge")
        raise AssertionError("invalid JSON must fail before a runtime result is returned")
    ...
    result = retrieval.retrieve_runtime(
        RetrieveRequest(
            query_text="Show recent reviewed project evidence.",
    ...
    package = dict(result["context_package"])
    contract = dict(package["contract"])
    assert result["status"] == "review_required"
    assert result["stop_reason"] == "review_required_ai_provider_error_after_useful_evidence"
    assert len(result["matches"]) == 14
    assert package["status"] == "review_required_with_preserved_evidence"
    assert package["preserved_after_late_ai_failure"] is True
    assert result["context_package_recovered_from_preserved_evidence"] is True
    assert int(dict(package.get("metrics") or {}).get("hot_item_count") or 0) == 14
    preserved_section = next(
        section
        for section in package["sections"]
        if section.get("key") == "preserved_evidence"
    )
    assert len(preserved_section["items"]) == 14
    assert preserved_section["items"][0]["node_id"] == "reviewed-project-1"
    assert contract["passed"] is False
    assert "invalid_json" in contract["missing_reasons"]
    assert result["semantic_contract_runtime"]["provider_error"] == "invalid_json"
    assert result["billing"]["charged_units"] == 0
...
def test_mcp_entrypoint_contains_spatial_provider_timeout_as_fail_closed_result(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        core_retrieve_router,
        ...
        lambda *args, **kwargs: nullcontext(
            {"brain_id": "brain_timeout", "node_count": 1}
        ),
    )
    monkeypatch.setattr(
        core_retrieve_router,
        "current_brain_id",
        lambda: "brain_timeout",
    )
    monkeypatch.setattr(
        core_retrieve_router,
        "_public_search_ai_admission",
        lambda _request: _admission(),
    )
    failure = retrieval.SearchAiExecutionError(
        "ai_spatial_landing",
        "transport_error:timed out",
    )
    failure.search_id = "search-mcp-spatial-timeout"
    request = RetrieveRequest(
        query_text="Find reviewed context.",
        response_mode="context",
    )
    terminal_result = core_retrieve_router._build_planning_failed_search_result(
        failure.search_id,
        request,
        failure,
        elapsed_ms=19_000.0,
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("failed planning must not start a Search worker")
        ),
    )

    response = core_retrieve_router._run_mcp_retrieval_tool(
        "retrieve_context",
        McpRetrievalToolRequest(query_text=request.query_text),
    )

    assert response.search_id == failure.search_id
    assert response.status in {"blocked", "failed"}
    assert response.context_package["status"] == "planning_failed"
    assert response.completion_contract["final_materialization_pending"] is False
    assert response.mcp_delivery_contract["client_payload_state"] in {
        "blocked",
        "failed",
    }
    assert response.mcp_delivery_contract["run_finished"] is True
    assert response.mcp_delivery_contract["terminal_for_inspection"] is True
    assert response.semantic_contract_runtime["provider_error"] == (
        "transport_error:timed out"
    )
    assert response.semantic_contract_runtime["billing"]["charged_units"] == 0
    ...
    http_response = TestClient(app).post(
        ...
        json={"query_text": request.query_text},
    )
    assert http_response.status_code == 200
    http_payload = http_response.json()
    assert http_payload["search_id"] == failure.search_id
    assert http_payload["status"] in {"blocked", "failed"}
    assert http_payload["semantic_contract_runtime"]["provider_error"] == (
        "transport_error:timed out"
    )
    assert http_payload["semantic_contract_runtime"]["billing"]["charged_units"] == 0
