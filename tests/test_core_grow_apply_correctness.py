...
def test_semantic_context_window_overflow_recovers_source_bound_preview(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    brain = _brain(tmp_path, "brain-source-bound-context-window")
    _patch_brain_resolution(
        monkeypatch,
        {brain["brain_id"]: brain},
        active_brain_id=brain["brain_id"],
    ...
    source_text = (
        "I currently prefer sencha tea when validating the final release commit 8297d636."
    )

    class ContextWindowGrowEngine:
        def preview(self, **_: Any) -> dict[str, Any]:
            raise ValueError(
                'grow_ai_unavailable:http_error:400:{"error":{"message":'
                '"Your input exceeds the context window of this model. Please adjust '
                'your input and try again.","type":"invalid_request_error",'
                '"param":"input","code":"context_length_exceeded"}}'
            )

    monkeypatch.setattr(grow_router, "_GROW_ENGINE", ContextWindowGrowEngine())
    monkeypatch.setattr(grow_router, "llm_runtime_status", lambda: {})
    app = FastAPI()
    app.include_router(grow_router.create_core_mcp_ops_router())
    client = TestClient(app)
    ...
    response = client.post(
        ...
        json={
            ...
            "raw_input": source_text,
            "input_kind": "manual_text",
            "source_label": "Direct personal preference",
            "run_preview": True,
            "user_instruction": (
                "Create a self memory with source provenance, semantic placement, "
    ...
    assert payload["can_apply_now"] is True
    assert payload["investigation_id"].startswith("mcp-grow-source-bound-")
    assert payload["investigation_session"]["provider_recovery_reason"] == (
        "semantic_provider_context_window_exceeded"
    )
    assert "context_length_exceeded" in json.dumps(
        payload["investigation_session"]["provider_failure"]
    )
    assert payload["source_investigation"]["source_bound_recovery"]["reason"] == (
        "semantic_provider_context_window_exceeded"
    )
    assert payload["selected_preview_ids"]
    ...
    assert stored is not None
    assert stored["preview_bundle"] == payload["preview_bundle"]
    assert stored["deterministic_source_attestation"]["provider_executed"] is False
    assert stored["deterministic_source_attestation"]["semantic_claims_emitted"] is False
...
def test_browser_client_tokenless_apply_consumes_v3_binding_with_ui_selection_order(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    brain = _brain(tmp_path, "brain-browser-client-order")
    _patch_brain_resolution(monkeypatch, {brain["brain_id"]: brain}, active_brain_id=brain["brain_id"])
    calls = _patch_persist_selection(monkeypatch)
    ...
    preview = grow_router._grow_source_preview(
        ...
        grow_router.McpGrowSourceRequest(
            brain_id=brain["brain_id"],
            raw_input="A provider-attested source with two selected memories.",
            input_kind="manual_text",
            run_preview=True,
        ),
    )
    investigation_id = str(preview.investigation_id)
    client_selected_ids = list(reversed(preview.selected_preview_ids))
    client_preview_bundle = deepcopy(preview.preview_bundle)
    client_preview_bundle["primary_node_preview"]["preview_confidence"] = 1
    assert client_selected_ids == ["preview-derived", "preview-primary"]
    assert isinstance(preview.preview_bundle["primary_node_preview"]["preview_confidence"], float)
    assert isinstance(client_preview_bundle["primary_node_preview"]["preview_confidence"], int)
    with use_runtime_brain(brain):
        stored_before = sqlite_store.fetch_local_grow_v2_preview(
            brain_id=brain["brain_id"],
            investigation_id=investigation_id,
        )
    assert stored_before["state"] == "active"

    response = grow_router._grow_source_apply(
        "grow_source_apply",
        grow_router.McpGrowApplyRequest(
            brain_id=brain["brain_id"],
            confirm_apply=True,
            investigation_id=investigation_id,
            investigation_version=preview.investigation_version,
            preview_bundle=client_preview_bundle,
            selected_preview_ids=client_selected_ids,
            source_formation_contract=preview.source_formation_contract,
            source_investigation=preview.source_investigation,
        ),
    )

    with use_runtime_brain(brain):
        stored_after = sqlite_store.fetch_local_grow_v2_preview(
            brain_id=brain["brain_id"],
            investigation_id=investigation_id,
        )

    assert response.status == "applied"
    assert response.persist_result["persisted_node_ids"] == ["persisted-preview-derived"]
    assert response.persist_result["selected_preview_ids"] == client_selected_ids
    assert stored_after["state"] == "consumed"
    assert stored_after["apply_result"]["selected_preview_ids"] == client_selected_ids
    assert calls == {"persist": 1}
...
def test_short_claim_specific_question_persists_resumable_clarification(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    brain = _brain(tmp_path, "brain-short-claim-question")
    _patch_brain_resolution(
        monkeypatch,
        {brain["brain_id"]: brain},
        active_brain_id=brain["brain_id"],
    ...
    question = {
        "schema_version": "agvm.grow_clarification_set.v1",
        "question_id": "q1_user_preference_detail_001",
        "question_text": "What specifically defines the user's preference for short updates?",
        "affected_claim_ids": ["claim-user-preference"],
        "required_for_preview": True,
        "decision_effect": "utility",
    ...
    class ClarifyingEngine:
        def preview(self, **_: Any) -> dict[str, Any]:
            return {
                "schema_version": "agvm.grow_engine_result.v3",
                "preview_bundle": None,
                "clarification_questions": [deepcopy(question)],
                "maintenance_feedback": [],
                "investigation": {
                    "schema_version": "agvm.grow_investigation.v3",
                    ...
                    "pending_questions": [deepcopy(question)],
                },
                "investigation_session": {
                    "schema_version": "agvm.investigation_session.v3",
    ...
    monkeypatch.setattr(grow_router, "_GROW_ENGINE", ClarifyingEngine())
    preview = grow_router._grow_source_preview(
        "grow_source_preview",
        grow_router.McpGrowSourceRequest(
            brain_id=brain["brain_id"],
            raw_input="I prefer short factual release updates.",
            input_kind="manual_text",
            source_label="Short preference",
            run_preview=True,
        ),
    )

    assert preview.status == "asking_clarification"
    assert preview.clarification_questions[0]["question"] == question["question_text"]
    with use_runtime_brain(brain):
        persisted = sqlite_store.fetch_grow_investigation(
            brain_id=brain["brain_id"],
            investigation_id=str(preview.investigation_id),
            resume_token=str(preview.resume_token),
        )
        assert persisted is not None
        assert persisted["state"] == "awaiting_clarification"
        resumed = sqlite_store.resume_grow_investigation(
            brain_id=brain["brain_id"],
            investigation_id=str(preview.investigation_id),
            resume_token=str(preview.resume_token),
            clarification_answers={question["question_id"]: "Three to five factual bullets."},
            expected_version=preview.investigation_version,
        )
    assert resumed["state"] == "investigating"
    assert resumed["investigation"]["clarification_answers"] == {
        question["question_id"]: "Three to five factual bullets."
    }


def test_semantic_provider_failure_preserves_execution_accounting(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    brain = _brain(tmp_path, "brain-provider-accounting")
    _patch_brain_resolution(monkeypatch, {brain["brain_id"]: brain}, active_brain_id=brain["brain_id"])
    ledger = [
        {
            "role": "grow_semantic",
    ...
    class PartiallyCompletedEngine:
        def preview(self, **kwargs: Any) -> dict[str, Any]:
            investigation_id = str(kwargs["source_investigation_id"])
            return {
                "schema_version": "agvm.grow_engine_result.v3",
                ...
                "investigation": {
                    ...
                    "failure": {
                        "code": "provider_timeout",
                        "detail": "planner timed out after semantic compilation",
                    },
                },
                "investigation_session": {
                    "schema_version": "agvm.investigation_session.v3",
                    "status": "incomplete",
                    "provider_attested": True,
                    "provider_failure": {"code": "provider_timeout"},
                },
                "usage": {"input_tokens": 100, "output_tokens": 25, "total_tokens": 125},
                "apply_ready": False,
            }

    monkeypatch.setattr(grow_router, "_GROW_ENGINE", PartiallyCompletedEngine())
    monkeypatch.setattr(grow_router, "llm_runtime_status", lambda: {})
    result = grow_router._grow_source_preview(
        "grow_source_preview",
        grow_router.McpGrowSourceRequest(
            brain_id=brain["brain_id"],
            raw_input="Provider-backed semantic Grow must expose consumed token accounting on failure.",
            input_kind="manual_text",
            run_preview=True,
            options={"semantic_preview": True},
        ),
    )

    assert result.status == "blocked"
    assert result.can_apply_now is False
    assert result.preview_bundle is None
    assert result.memory_operation_lifecycle_contract["blocked_reason"] == "blocked_ai_provider_timeout"
    assert result.investigation_id
    assert result.ai_execution_attestation["provider_executed"] is True
    assert result.ai_execution_ledger == ledger
    assert result.investigation["failure"]["code"] == "provider_timeout"
    assert result.investigation_session["provider_failure"]["code"] == "provider_timeout"
