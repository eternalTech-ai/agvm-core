...
def test_public_v2_spatial_execution_keeps_ai_coordinate_and_full_mission_binding(
    monkeypatch,
) -> None:
    monkeypatch.setattr(retrieval, "_search_semantic_authority_v2_enabled", lambda: True)
    monkeypatch.setattr(retrieval, "_search_plan_first_v3_enabled", lambda: True)
    ai_coordinate = {"x": 0.72, "y": -0.35, "z": 0.11}
    contract = {
        "materialized": True,
        "certifiable": True,
        "inverse_answer_paths": [
            {
                "path_id": "P1",
                "mission_id": "mission_P1",
                "strand_id": "strand-projects",
                "goal": "Find reviewed project evidence",
                "answer_field": "projects",
                "answer_hypothesis": "Reviewed sources support recent project claims.",
                "confidence": 0.93,
                "destinations": [
                    {
                        "destination_id": "primary-project-evidence",
                        "region_ref": "project-evidence",
                        "coordinate": ai_coordinate,
                        "radius": 0.18,
                        "execution_role": "primary",
                    },
                    {
                        "destination_id": "reserve-project-evidence",
                        "region_ref": "project-evidence",
                        "coordinate": {"x": -0.4, "y": 0.2, "z": 0.3},
                        "execution_role": "reserve",
                        "activates_for_mission_id": "P1",
                        "activation_predicate": "no_evidence",
                    },
                ],
                "conditional_extension": {
                    "destination_id": "extension-project-evidence",
                    "region_ref": "project-evidence",
                    "coordinate": {"x": 0.3, "y": 0.4, "z": -0.2},
                    "activates_for_mission_id": "P1",
                    "activation_predicate": "mission_unresolved",
                },
            }
        ],
    }

    materialized = retrieval._build_ai_spatial_probes_from_contract(
        query=retrieval.RetrieveRequest(
            query_text="Find recent reviewed projects.",
            retrieval_mode="balanced",
    ...
    probe = materialized["probes"][0]
    primary = probe["plan_first_primary_destinations"][0]
    extension = probe["plan_first_conditional_extension_destinations"][0]
    reserve = probe["plan_first_reserve_destinations"][0]
    assert probe["mission_id"] == "mission_P1"
    assert probe["landing_position"] == ai_coordinate
    assert probe["spatial_snap"]["source"] == "ai_coordinate"
    assert probe["spatial_snap"]["backend_changed_coordinate"] is False
    assert primary["ai_spatial_coordinate"] == ai_coordinate
    assert extension["activates_for_mission_id"] == "mission_P1"
    assert reserve["activates_for_mission_id"] == "mission_P1"
