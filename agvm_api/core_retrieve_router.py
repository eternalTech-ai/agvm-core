                            ...
                            payload = {
                                ...
                                "payload": {
                                    ...
                                    "terminal_for_client": bool(
                                        fallback_result.get("terminal_for_client")
                                        or dict(fallback_result.get("mcp_delivery_contract") or {}).get(
                                            "terminal_for_client"
                                        )
                                    ),
                                    ...
                                    "completion_state": fallback_result.get("completion_state"),
                                    "canonical_search_state": fallback_result.get("canonical_search_state"),
                                },
    ...
    try:
            ...
            for key in (
                *_MCP_SURFACE_FIELDS,
                ...
                if key in persisted and persisted.get(key) is not None:
                    enriched_event_payload[key] = persisted.get(key)
        ...
        persist_lifecycle_event(
            "result_ready",
            {
                ...
                "result_ready_terminal": True,
                "terminal_for_client": bool(
                    result.get("terminal_for_client")
                    or dict(result.get("mcp_delivery_contract") or {}).get(
                        "terminal_for_client"
                    )
                ),
                "client_terminal": bool(
                ...
                "canonical_search_state": result.get("canonical_search_state"),
            },
        )
        return result
    except Exception as exc:  # noqa: BLE001
    ...
    *,
    ...
    return {
        **dict(event_payload or {}),
        ...
        "ai_spatial_landing_contract": dict(
            event_payload.get("ai_spatial_landing_contract")
            or plan.get("ai_spatial_landing_contract")
            or planner_runtime.get("ai_spatial_landing_contract")
            or {}
        ),
        "ai_spatial_landing_contract_runtime": dict(
            event_payload.get("ai_spatial_landing_contract_runtime")
            or plan.get("ai_spatial_landing_contract_runtime")
            or planner_runtime.get("ai_spatial_landing_contract_runtime")
            or {}
        ),
        "path_mission_contract": dict(
            event_payload.get("path_mission_contract")
            or plan.get("path_mission_contract")
            or planner_runtime.get("path_mission_contract")
            or {}
        ),
        "path_missions": [
            dict(item)
            for item in list(
                event_payload.get("path_missions")
                or plan.get("path_missions")
                or planner_runtime.get("path_missions")
                or []
            )
            if isinstance(item, dict)
        ],
        "mission_aware_merge_summary": dict(
            event_payload.get("mission_aware_merge_summary")
            or plan.get("mission_aware_merge_summary")
            or planner_runtime.get("mission_aware_merge_summary")
            or planner_runtime.get("ai_spatial_merge_summary")
            or {}
        ),
        ...
        "landing_metadata": [
            dict(item)
            for item in list(
                event_payload.get("landing_metadata")
                or plan.get("landing_metadata")
                or []
            )
            if isinstance(item, dict)
        ],
        "search_map_2d_truth": dict(
            event_payload.get("search_map_2d_truth")
            or plan.get("search_map_2d_truth")
...
def _mcp_plan_storage_snapshot(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "ai_spatial_landing_contract": dict(plan.get("ai_spatial_landing_contract") or {}),
        "ai_spatial_landing_contract_runtime": dict(
            plan.get("ai_spatial_landing_contract_runtime") or {}
        ),
        "path_mission_contract": dict(plan.get("path_mission_contract") or {}),
        "path_missions": [
            dict(item)
            for item in list(plan.get("path_missions") or [])
            if isinstance(item, dict)
        ],
        "mission_aware_merge_summary": dict(
            plan.get("mission_aware_merge_summary")
        ...
        "mission_evidence_ledger": dict(
            plan.get("mission_evidence_ledger")
            or planner_runtime.get("mission_evidence_ledger")
            or {}
        ),
    ...
    *,
    ...
    append_search_event(
        ...
        "planning_complete",
        {
            ...
            "mission_aware_merge_summary": dict(
                plan.get("mission_aware_merge_summary")
                or dict(plan.get("planner_runtime") or {}).get("mission_aware_merge_summary")
                or dict(plan.get("planner_runtime") or {}).get("ai_spatial_merge_summary")
                or {}
            ),
            "planner_runtime": dict(plan.get("planner_runtime") or {}),
            "probes": list(plan.get("probes") or []),
        ...
        try:
            ...
            if search_id:
                try:
                    ...
                    persisted_result = dict(session.get("result") or {})
                except Exception:  # noqa: BLE001 - preserve the fail-closed MCP boundary
                    persisted_result = {}
            terminal_search_id = (
                search_id
                or str(persisted_result.get("search_id") or "").strip()
            )
            if not persisted_result:
                persisted_result = _build_planning_failed_search_result(
                    terminal_search_id,
                    request,
                    exc,
                    elapsed_ms=0.0,
            ...
            result = _attach_brain_metadata(
                normalize_retrieve_response_payload(
                    {
                        **persisted_result,
                        "search_id": terminal_search_id or None,
                    }
                )
            )
            ...
            output = build_mcp_retrieval_tool_output(
                ...
                include_answer_demo=payload.include_answer_demo,
                include_raw_text=(
                    payload.include_raw_text
                    or payload.document_text_policy in {"top_raw", "all_raw"}
                ),
            )
            return McpToolExecutionResponse(**_attach_tool_brain_metadata(output))
