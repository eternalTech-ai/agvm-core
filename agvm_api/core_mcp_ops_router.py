...
# This is the outer budget for the complete interactive workflow (planner,
# optional child Search, and final review), not a per-provider-call timeout.
# Individual investigator calls remain bounded by GrowInvestigationBudget.
_GROW_INTERACTIVE_COMPILER_TIMEOUT_SECONDS = 600.0
_GROW_INTERACTIVE_HARD_TIMEOUT_SECONDS = 630.0
...
try:
    ...
    def build_source_investigation_package(
        ...
        *,
        ...
        raw_options = deepcopy(dict(options or {}))
        ...
        manual_source = source_kind == "manual_text" and not external_source
        allowed_source_purposes = {
            "auto",
            "self_memory",
            "project_workspace",
            "public_dossier",
        ...
        requested_treat_as = str(raw_options.get("treat_as") or "auto").strip()
        requested_source_trust = str(raw_options.get("source_trust") or "unknown").strip()
        canonical_treat_as = (
            requested_treat_as if requested_treat_as in allowed_source_purposes else "auto"
        )
        canonical_source_trust = (
            requested_source_trust
            if requested_source_trust in allowed_source_trust
            else "unknown"
        )
        canonical_requested_options = {
            **raw_options,
            "treat_as": canonical_treat_as,
            "source_trust": canonical_source_trust,
        }
        content_digest = f"sha256:{hashlib.sha256(evidence_text.encode('utf-8')).hexdigest()}"
        source_unit = {
            "unit_id": unit_id,
            ...
            source_unit.update(
                {
                    ...
                    "acquired_at": resolved_created_at,
                    ...
                    "provenance": {
                        ...
                        "source_trust": canonical_source_trust,
                        "hash": content_digest,
                        "retrieved_at": resolved_created_at,
                    },
                    "extraction_trace": {
            ...
            section.update(
                {
                    ...
                    "acquired_at": resolved_created_at,
                    "acquisition_method": "manual_text_passthrough",
                    "acquisition_proof": deepcopy(source_unit["acquisition_proof"]),
                    "provenance": deepcopy(source_unit["provenance"]),
                    "extraction_trace": deepcopy(source_unit["extraction_trace"]),
                }
            )
        compiler_handoff = {
        ...
        if manual_source:
            compiler_handoff.update(
                {
                    "handoff_version": "agvm.compiler_handoff.v1",
                    "recommended_source_type": "source_investigation",
                    "preview_eligible": bool(evidence_text),
                    "source_purpose": canonical_treat_as,
                }
            )
        package = {
            ...
            package.update(
                {
                    "source_request": {
                        ...
                        "user_instruction": user_instruction,
                        "options": {
                            "requested": canonical_requested_options,
                            "effective": deepcopy(canonical_requested_options),
                        },
                    },
                    "source_purpose": {
                        "purpose": canonical_treat_as,
                        "confidence": 0.92 if canonical_treat_as != "auto" else 0.0,
                        "decision_source": (
                            "user_option"
                            if canonical_treat_as != "auto"
                            else "public_core_default_non_authoritative"
                        ),
                        "requires_confirmation": canonical_treat_as == "auto",
                        "reasons": (
                            [f"explicit_treat_as:{canonical_treat_as}"]
                            if canonical_treat_as != "auto"
                            else ["no_explicit_treat_as"]
                        ),
                    },
                }
            )
...
def _node_change_preview_package(output: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "agvm.mcp_node_change_preview.v1",
        "status": output.get("status"),
        "brain_id": output.get("brain_id"),
        "node_id": output.get("node_id"),
        "preview_id": output.get("preview_id"),
        "before_brain_revision": output.get("before_brain_revision"),
        "expected_brain_revision": output.get("expected_brain_revision"),
        "can_apply_now": bool(output.get("can_apply_now")),
        "mutates_memory": bool(output.get("mutates_memory")),
        "blocked_reasons": list(output.get("blocked_reasons") or []),
        "before": dict(output.get("before") or {}),
        "after": dict(output.get("after") or {}),
        "apply_contract": dict(output.get("apply_contract") or {}),
    }


def _node_delete_preview_package(output: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "agvm.mcp_node_delete_preview.v1",
        "status": output.get("status"),
        "brain_id": output.get("brain_id"),
        "node_ids": list(output.get("node_ids") or []),
        "preview_id": output.get("preview_id"),
        "before_brain_revision": output.get("before_brain_revision"),
        "expected_brain_revision": output.get("expected_brain_revision"),
        "can_apply_now": bool(output.get("can_apply_now")),
        "mutates_memory": bool(output.get("mutates_memory")),
        "blocked_reasons": list(output.get("blocked_reasons") or []),
        "missing_node_ids": list(output.get("missing_node_ids") or []),
        "protected_node_ids": list(output.get("protected_node_ids") or []),
        "nodes": list(output.get("nodes") or []),
        "edges_to_remove": int(output.get("edges_to_remove") or 0),
        "apply_contract": dict(output.get("apply_contract") or {}),
    }
...
def _mcp_change_node_content(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    with use_runtime_brain(brain_record):
        ...
        if not node:
            ...
            blocked["node_change_preview"] = _node_change_preview_package(blocked)
            return blocked
        ...
        preview["node_change_preview"] = _node_change_preview_package(preview)
        ...
        if idempotency_key:
            for receipt in receipts:
                if isinstance(receipt, dict) and receipt.get("idempotency_key") == idempotency_key:
                    ...
                    replayed["node_change_preview"] = _node_change_preview_package(replayed)
                    return replayed
        graph["nodes"] = [
        ...
        applied["node_change_preview"] = _node_change_preview_package(applied)
        return applied
...
def _mcp_delete_node(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    with use_runtime_brain(brain_record):
        ...
        preview["node_delete_preview"] = _node_delete_preview_package(preview)
        ...
        if expected_revision and expected_revision != before_revision:
            ...
            blocked["node_delete_preview"] = _node_delete_preview_package(blocked)
            return blocked
        receipts = _graph_receipts(graph)
        if idempotency_key:
            for receipt in receipts:
                if isinstance(receipt, dict) and receipt.get("idempotency_key") == idempotency_key:
                    ...
                    replayed["node_delete_preview"] = _node_delete_preview_package(replayed)
                    return replayed
        graph["nodes"] = [
            node
            for node in list(graph.get("nodes") or [])
        ...
        applied["node_delete_preview"] = _node_delete_preview_package(applied)
        return applied
...
def _preview_node_ids(bundle: dict[str, Any]) -> list[str]:
...
def _selected_preview_ids(bundle: dict[str, Any], payload_ids: list[str]) -> list[str]:
    ...
    apply_eligible_ids = {
        ...
        for node in nodes
        if not (
            node.get("apply_eligible") is False
            or str(node.get("apply_eligible")).strip().lower() == "false"
        )
    }
    ids = _normalized_grow_ids(payload_ids)
    if ids:
        return [node_id for node_id in ids if node_id in apply_eligible_ids]
    selected_ids = _normalized_grow_ids(
        [
            str(node.get("id") or "")
            for node in nodes
            if bool(node.get("selected_by_default", True))
            and str(node.get("id") or "") in apply_eligible_ids
        ]
    )
    return selected_ids or [
        node_id for node_id in _preview_node_ids(bundle) if node_id in apply_eligible_ids
    ]
...
def _grow_contract_fingerprint(value: Any) -> str:
    canonical = json.dumps(
        _canonical_grow_contract_value(value),
        ensure_ascii=False,
        sort_keys=True,
        separators=(",", ":"),
        default=str,
...
def _canonical_grow_contract_value(value: Any) -> Any:
    """Keep preview fingerprints stable across a JSON/JavaScript round trip."""
    if isinstance(value, dict):
        return {key: _canonical_grow_contract_value(item) for key, item in value.items()}
    if isinstance(value, list):
        return [_canonical_grow_contract_value(item) for item in value]
...
def _validated_handoff_source_type_for_grow(
    source_package: dict[str, Any],
) -> str | None:
    if source_package.get("schema_version") != "agvm.source_investigation.v1":
        return None
    handoff = dict(source_package.get("compiler_handoff") or {})
    handoff_schema = str(
        handoff.get("schema_version") or handoff.get("handoff_version") or ""
    ).strip()
    recommended_source_type = str(handoff.get("recommended_source_type") or "").strip()
    if (
        handoff_schema != "agvm.compiler_handoff.v1"
        or recommended_source_type != "source_investigation"
        or handoff.get("preview_eligible") is not True
        or not str(handoff.get("mega_text") or "").strip()
    ):
        return None
    ...
    source_unit_ids = {
        ...
        and str(unit.get("raw_text") or unit.get("text") or "").strip()
    }
    structured_section_unit_ids: set[str] = set()
    for section in list(handoff.get("structured_sections") or []):
        ...
        if section_id:
            structured_section_unit_ids.add(section_id)
        structured_section_unit_ids.update(
            str(unit_id or "").strip()
            for unit_id in list(section.get("source_unit_ids") or [])
            if str(unit_id or "").strip()
        )
    if not source_unit_ids or not source_unit_ids.issubset(structured_section_unit_ids):
        return None
    return recommended_source_type
...
def _source_type_for_grow(
    ...
    recommended_source_type = _validated_handoff_source_type_for_grow(source_package)
    if recommended_source_type is not None:
        return recommended_source_type
...
def _grow_source_package_primary_raw_text(source_package: dict[str, Any]) -> str:
    """Return the canonical source text that must back the primary raw anchor."""
    ...
    seen: set[str] = set()
    chunks: list[str] = []
    for collection_key in ("source_units",):
        for item in list(source_package.get(collection_key) or []):
...
def _grow_source_bound_preview_with_claim_authority(
    ...
    *,
    ...
    if not raw_nodes:
        return bundle
    source_request = dict(source_package.get("source_request") or {})
    source_raw_anchor_text = _grow_source_package_primary_raw_text(source_package)
    ...
    for index, raw_node in enumerate(raw_nodes):
        ...
        if index == 0 and structural_node and source_raw_anchor_text:
            current_raw_text = str(node.get("raw_text") or node.get("summary") or "").strip()
            if source_raw_anchor_text != current_raw_text and len(source_raw_anchor_text) > len(current_raw_text):
                node["raw_text"] = source_raw_anchor_text
                preservation = dict(node.get("source_raw_text_preservation") or {})
                preservation.update(
                    {
                        "schema_version": "agvm.grow_source_raw_text_preservation.v1",
                        "source": "compiler_handoff",
                        "preserved": True,
                        "original_raw_text_chars": len(current_raw_text),
                        "preserved_raw_text_chars": len(source_raw_anchor_text),
                    }
                )
                node["source_raw_text_preservation"] = preservation
...
def _grow_question_is_required_claim_clarification(question: dict[str, Any]) -> bool:
    """Keep provider questions that are bound to an investigated claim.

    Source-intake questions may be defaulted when Core already has enough source
    material to continue.  Provider-generated claim questions are different:
    their answer is explicitly required for the semantic memory decision, even
    when the source statement itself is short.
    """

    if question.get("required_for_preview") is False:
        return False
    return any(
...
def _filter_non_blocking_grow_source_questions(
    ...
    *,
    ...
    for raw_question in questions:
        ...
        if _grow_question_is_required_claim_clarification(question):
...
def _grow_v3_source_without_legacy_semantic_questions(
    ...
    if formation:
        ...
        if not pending_questions and str(formation.get("state") or "").strip() in {
            "questions_pending",
            "paused_for_clarification",
            "awaiting_clarification",
        }:
            formation["state"] = "preview_ready"
        apply_contract = deepcopy(dict(formation.get("apply_contract") or {}))
        if apply_contract and not pending_questions:
            blocked_reasons = [
                str(reason)
                for reason in list(apply_contract.get("blocked_reasons") or [])
                if str(reason) not in {"pending_guided_grow_questions", "clarification_required"}
            ]
            apply_contract["blocked_reasons"] = blocked_reasons
            if not blocked_reasons:
                apply_contract["can_apply_now"] = True
            formation["apply_contract"] = apply_contract
    ...
    if guided_grow:
        ...
        guided_pending, defaulted_guided = _filter_non_blocking_grow_source_questions(
            raw_guided_pending,
            source_material_ready=source_material_ready,
            source_identity_ready=source_identity_ready,
        )
        ...
        if len(guided_pending) != len(raw_guided_pending):
            guided_grow["pending_questions"] = guided_pending
            guided_grow["pending_count"] = len(guided_pending)
            guided_grow["defaulted_count"] = int(guided_grow.get("defaulted_count") or 0) + len(defaulted_guided)
            guided_grow["defaulted_questions"] = [
                *[
                    dict(item)
                    for item in list(guided_grow.get("defaulted_questions") or [])
                    if isinstance(item, dict)
                ],
                *[
                    {
                        "question_id": item.get("question_id") or item.get("id"),
                        "default_action": item.get("default_action_if_unanswered") or "store_uncertain",
                    }
                    for item in defaulted_guided
                ],
            ]
            if guided_pending:
                guided_grow["state"] = "paused_for_clarification"
                guided_grow["can_resume"] = True
            else:
                guided_grow["state"] = "defaults_applied"
...
def _grow_provider_failure_is_source_bound_recoverable(
    ...
    return bool(
        ...
        and (
            ...
            or "context_length_exceeded" in failure_text
            or "context window" in failure_text
            or "context_budget" in failure_text
            or "context guard" in failure_text
            or "compaction_overflow" in failure_text
            or "coverage_incomplete" in failure_text
            or "provider_did_not_cover_all_fact_eligible_source_units" in failure_text
            or "input exceeds" in failure_text
        )
    )


def _grow_provider_failure_is_context_window_exceeded(
    engine_result: dict[str, Any],
    investigation: dict[str, Any],
) -> bool:
    failure = {
    ...
    return bool(
        "context_length_exceeded" in failure_text
        or "context window" in failure_text
        or "context_budget" in failure_text
        or "context guard" in failure_text
        or "compaction_overflow" in failure_text
...
def _grow_provider_failure_block_reason(
    ...
    if (
        ...
        return "blocked_ai_provider_context_window"
    ...
    if any(
        ...
        for marker in (
            "coverage_incomplete",
            "provider_did_not_cover_all_fact_eligible_source_units",
        )
    ):
        return "blocked_ai_provider_coverage_incomplete"
...
def _grow_provider_failure_recovery_reason(
    ...
    if block_reason == "blocked_ai_provider_context_window":
        return "semantic_provider_context_window_exceeded"
...
def _grow_options_with_effective_source_package_timeout(
    options: Any,
    source_package: dict[str, Any],
) -> Any:
    """Use source-package effective options for the semantic Grow engine budget.

    Public Grow builds a source package before the semantic investigator.  That
    package normalizes interactive/manual runs to the guided compiler budget, but
    the request model can still carry its raw schema default.  The engine and the
    outer watchdog must share the effective source-package timeout, otherwise
    Grow can fail after one short investigator turn even though the public source
    contract advertises the guided budget.
    """

    effective_options = dict(
        dict(dict(source_package.get("source_request") or {}).get("options") or {}).get(
            "effective"
        )
        or {}
    )
    requested = effective_options.get("compiler_preview_timeout_seconds")
    if requested is None:
        return options
    try:
        effective_timeout = float(requested)
    except (TypeError, ValueError):
        return options
    if effective_timeout <= 0:
        return options
    try:
        current_timeout = float(getattr(options, "compiler_preview_timeout_seconds", 45.0) or 45.0)
    except (TypeError, ValueError):
        current_timeout = 45.0
    if math.isclose(effective_timeout, current_timeout):
        return options
    if hasattr(options, "model_copy"):
        return options.model_copy(
            update={"compiler_preview_timeout_seconds": effective_timeout}
        )
    return options


def _grow_options_with_interactive_timeout(
    options: Any,
    source_package: dict[str, Any],
    *,
    clarification_answers: dict[str, str],
) -> Any:
    source_request_options = dict(
        dict(source_package.get("source_request") or {}).get("options") or {}
    )
    effective_options = dict(source_request_options.get("effective") or {})
    guided_grow = dict(source_package.get("guided_grow") or {})
    question_gate = dict(
        dict(source_package.get("source_formation_contract") or {}).get("question_gate")
        or {}
    )
    interactive = bool(
        clarification_answers
        or source_package.get("clarification_questions")
        or source_package.get("open_questions")
        or source_package.get("questions_and_answers")
        or guided_grow
        or question_gate.get("pending_questions")
        or source_request_options.get("pause_on_questions")
        or source_request_options.get("semantic_preview")
        or effective_options.get("pause_on_questions")
        or effective_options.get("semantic_preview")
    )
    if not interactive:
        return options
    try:
        current_timeout = float(
            getattr(options, "compiler_preview_timeout_seconds", 45.0) or 45.0
        )
    except (TypeError, ValueError):
        current_timeout = 45.0
    if current_timeout >= _GROW_INTERACTIVE_COMPILER_TIMEOUT_SECONDS:
        return options
    if hasattr(options, "model_copy"):
        return options.model_copy(
            update={
...
def _grow_source_bound_recovery_preview_response(
    ...
    *,
    ...
    preview_bundle["parent_graph_revision_id"] = preview_brain_revision
    warning_notice = {
    ...
    source_formation_contract = {
        **build_source_formation_contract(recovery_source_package, preview_bundle),
        ...
        "parent_graph_revision_id": preview_brain_revision,
...
def _grow_source_preview_v3(
    ...
    *,
    ...
    options = _grow_options_with_effective_source_package_timeout(options, source_package)
    options = _grow_options_with_interactive_timeout(
        options,
        source_package,
        clarification_answers=clarification_answers,
    )
    ...
    try:
    ...
    except (ValueError, RuntimeError, AiModuleContractError) as exc:
        reason = str(getattr(exc, "code", "") or exc or "grow_ai_unavailable")
        failure_engine_result = {
            "failure": {
                "code": "provider_error",
                "detail": reason,
            }
        ...
        if (
            ...
            and _grow_provider_failure_is_source_bound_recoverable(
                failure_engine_result,
                failure_investigation,
            )
        ):
            return _grow_source_bound_recovery_preview_response(
                ...
                source_package=source_package,
                engine_result=failure_engine_result,
    ...
    if (
        ...
        if (
            not semantic_provider_required
            or _grow_provider_failure_is_context_window_exceeded(
                engine_result,
                investigated,
            )
        ):
        ...
        return _grow_blocked(
            ...
            ai_execution_ledger=[
                deepcopy(dict(item))
                for item in list(
                    engine_result.get("ai_execution_ledger")
                    or investigated.get("ai_execution_ledger")
                    or []
                )
                if isinstance(item, dict)
            ],
            investigation=deepcopy(investigated),
            investigation_session=deepcopy(
                dict(engine_result.get("investigation_session") or {})
            ),
            usage=deepcopy(
                dict(engine_result.get("usage") or investigated.get("usage") or {})
            ),
        )
    ...
    bundle["parent_graph_revision_id"] = preview_brain_revision
    try:
        with use_runtime_brain(brain_record):
...
def _grow_source_preview(tool_name: str, payload: McpGrowSourceRequest) -> McpGrowToolExecutionResponse:
    ...
    with use_runtime_brain(brain_record):
        ...
        atlas_payload = fetch_atlas()
        options = payload.options
        options = _grow_options_with_effective_source_package_timeout(
            options,
            source_package,
        )
        try:
...
def _grow_source_apply_locked(tool_name: str, payload: McpGrowApplyRequest) -> McpGrowToolExecutionResponse:
    ...
    if stored_investigation:
        ...
        tokenless_exact_local_recovery = bool(
            ...
            and sorted(requested_recovery_ids) == sorted(stored_recovery_ids)
        )
    ...
    if (
        payload.preview_bundle is not None
        and _grow_contract_fingerprint(payload.preview_bundle)
        != _grow_contract_fingerprint(bundle)
    ):
    ...
    available_ids = _preview_node_ids(bundle)
...
def _grow_blocked(
    ...
    *,
    ...
    investigation_id: str | None = None,
    ai_execution_attestation: dict[str, Any] | None = None,
    ai_execution_ledger: list[dict[str, Any]] | None = None,
    investigation: dict[str, Any] | None = None,
    investigation_session: dict[str, Any] | None = None,
    ...
    return McpGrowToolExecutionResponse(
        ...
        preview_bundle=preview_bundle,
        ai_execution_attestation=deepcopy(dict(ai_execution_attestation or {})),
        ai_execution_ledger=[
            deepcopy(dict(item))
            for item in list(ai_execution_ledger or [])
            if isinstance(item, dict)
        ],
        investigation=deepcopy(dict(investigation or {})),
        investigation_session=deepcopy(dict(investigation_session or {})),
