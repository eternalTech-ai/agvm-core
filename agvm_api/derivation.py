...
# The default compiler model (gpt-4o-mini) accepts at most 16,384 output
# tokens. Asking the Responses API for 32,000 fails before inference and makes
# hosted Bootstrap surface the generic semantic-compiler-unavailable error.
# Keep a little headroom while retaining enough space for the largest batch.
_SOURCE_SEMANTIC_COMPILER_MAX_OUTPUT_TOKENS = 16_000
# A compiled node carries 24 routing values plus provenance and temporal
# fields. Rich real-world statements can make 24+ nodes exhaust even the
# model's full output window, so keep each independently attested request
# small and aggregate the batches below.
_SOURCE_SEMANTIC_COMPILER_BATCH_SIZE = 12
...
def _compiler_prompt_source_context(
    source_context: Mapping[str, Any] | None,
    *,
    max_serialized_chars: int = 24_000,
) -> dict[str, Any]:
    """Keep compiler authority while excluding bulky investigator/Search payloads.

    Grow already supplies the exact compiler text, identity nucleus, and nearby
    graph context separately.  Passing the full investigator ledger here can
    duplicate terminal Search packages and overflow the provider context window
    even for a one-line self memory.
    """

    context = dict(source_context or {})
    try:
        if len(json.dumps(context, ensure_ascii=False, default=str)) <= max_serialized_chars:
            return context
    except (TypeError, ValueError):
        pass
    ...
    for key in (
        ...
        "source_unit_formation_status",
    ):
        if key in context:
            compact[key] = context[key]

    source_request = dict(context.get("source_request") or {})
    if source_request:
        request_summary = {
        ...
        request_options = dict(source_request.get("options") or {})
        summarized_options: dict[str, Any] = {}
        for bucket_name in ("requested", "effective"):
            bucket = dict(request_options.get(bucket_name) or {})
            summarized_options[bucket_name] = {
                key: bucket[key]
                for key in (
                    "treat_as",
                    "source_trust",
                    "pause_on_questions",
                    "clarification_default_policy",
                )
                if key in bucket
            }
        if any(summarized_options.values()):
            request_summary["options"] = summarized_options
        compact["source_request"] = request_summary

    for key in ("provenance", "compiler_authority", "clarification_answers"):
        value = context.get(key)
        if isinstance(value, Mapping):
            compact[key] = dict(value)

    formation = context.get("source_unit_formation")
    if isinstance(formation, Mapping):
        compact["source_unit_formation"] = {
            key: formation[key]
            for key in (
                "schema_version",
                "status",
    ...
    for raw_unit in list(context.get("source_units") or [])[:16]:
    ...
    if compact_units:
        compact["source_units"] = compact_units

    investigation = context.get("grow_investigation")
    if isinstance(investigation, Mapping):
        ...
        for collection_key in ("decisions", "compiler_claims"):
            summarized_items: list[dict[str, Any]] = []
            for raw_item in list(investigation.get(collection_key) or [])[:24]:
            ...
            if summarized_items:
                compact_investigation[collection_key] = summarized_items
        compact["grow_investigation"] = compact_investigation

    compact["prompt_compaction"] = {
        "applied": True,
        "omitted": "bulky investigator/Search evidence; exact source text is supplied separately",
    }
    return compact
...
def llm_memory_compile(
    ...
    *,
    ...
    compiler_source_context = _compiler_prompt_source_context(source_context)
    user_prompt = (
        f"Input mode: {input_mode}\n"
        f"Source text:\n{text}\n\n"
        f"Metamemory role: compiler\n\n"
        f"Source context:\n{compiler_source_context}\n\n"
        f"Identity nucleus:\n{identity_nucleus}\n\n"
        f"Nearby context:\n{nearby_context}\n"
    )
    payload, error = structured_json(
...
def _llm_source_unit_semantic_compile_batch(
    *,
    ...
    metamemory = build_metamemory_package("compiler")
    source_unit_ids = _unique_strings([section.get("source_unit_id") for section in sections], limit=len(sections))
    schema = {
        "type": "object",
        "additionalProperties": False,
            ...
            "derived_nodes": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "raw_text": {"type": "string", "minLength": 36},
                        "summary": {"type": "string"},
                        "routing_semantic_scores": {
                            "type": "object",
                            "additionalProperties": False,
    ...
    source_context = {
        ...
        "allowed_source_unit_ids": source_unit_ids,
    }
    system_prompt = (
        ...
        "- Write raw_text as a complete, self-contained declarative sentence of at least 36 characters with an explicit subject and verb. "
        "Expand slash-separated or telegraphic source wording into fluent language without adding facts; never echo a heading or keyword list.\n"
    ...
    payload, error = structured_json(
        ...
        role="compiler",
        max_output_tokens=_SOURCE_SEMANTIC_COMPILER_MAX_OUTPUT_TOKENS,
        api_key_override=api_key_override,
        execution_metadata=execution_metadata,
    )
    if not payload:
...
def llm_source_unit_semantic_compile(
    *,
    ...
    batch_size = _SOURCE_SEMANTIC_COMPILER_BATCH_SIZE
...
_SOURCE_CLAIM_ACTION_RE = re.compile(
    ...
    r"shapes?|shaped|believes?|believed|wants?|wanted|prefers?|preferred|am|"
    r"requires?|required|depends?|depended|consumes?|consumed|fails?|failed|"
    r"must|should|conducts?|conducted|allows?|allowed|ensures?|ensured|"
...
_SOURCE_LOW_VALUE_SENTENCE_RE = re.compile(
    ...
    r"menu|search\s+(?:box|field|bar|results?)|skip to|copyright|all rights reserved|zoom|"
...
def _preview_node_apply_eligible(node: Mapping[str, Any]) -> bool:
    value = node.get("apply_eligible")
    return not (value is False or str(value).strip().lower() == "false")


def _source_preview_node_text_key(node: Mapping[str, Any]) -> str:
    return normalize_preview_text(str(node.get("raw_text") or node.get("summary") or ""))


def _source_preview_node_source_ref(node: Mapping[str, Any]) -> str:
    provenance = dict(node.get("provenance") or {})
    return str(
        node.get("source_unit_id")
        or provenance.get("source_unit_id")
...
def _source_preview_is_structural_anchor(node: Mapping[str, Any]) -> bool:
    return bool(
        node.get("is_document_anchor")
        or str(node.get("document_role") or "").strip() == "anchor"
        or str(node.get("memory_type") or "").strip() == "document_anchor"
        or str(node.get("node_kind") or "").strip() == "document_anchor"
        or str(node.get("compiler_binding_role") or "").strip() == "source_document_structure"
    )


def _source_preview_same_source_family(
    primary_preview: Mapping[str, Any],
    derived_node: Mapping[str, Any],
) -> bool:
    primary_ref = _source_preview_node_source_ref(primary_preview)
    derived_ref = _source_preview_node_source_ref(derived_node)
    if primary_ref and derived_ref and primary_ref == derived_ref:
        return True
    derived_anchor = str(derived_node.get("document_anchor_id") or "").strip()
    primary_id = str(primary_preview.get("id") or "").strip()
    primary_anchor = str(primary_preview.get("document_anchor_id") or "").strip()
    return bool(
        derived_anchor
        and derived_anchor in {primary_id, primary_anchor, "preview_primary"}
    )


def _suppress_identical_structural_anchor_apply(
    *,
    primary_preview: dict[str, Any],
    derived_nodes: list[dict[str, Any]],
    source_type: str | None,
    ...
    if not _is_source_investigation_scope(source_type=source_type, source_sections=source_sections):
        return primary_preview, derived_nodes
    if not _source_preview_is_structural_anchor(primary_preview):
        return primary_preview, derived_nodes
    primary_text_key = _source_preview_node_text_key(primary_preview)
    if not primary_text_key:
        return primary_preview, derived_nodes
    duplicate_child_ids = [
        str(node.get("id") or "")
        for node in derived_nodes
        if _source_preview_node_text_key(node) == primary_text_key
        and _source_preview_same_source_family(primary_preview, node)
    ]
    duplicate_child_ids = _unique_strings(duplicate_child_ids, limit=16)
    if not duplicate_child_ids:
        return primary_preview, derived_nodes
    primary_refs = {
        str(primary_preview.get("id") or "").strip(),
        str(primary_preview.get("document_anchor_id") or "").strip(),
        "preview_primary",
    }
    duplicate_child_id_set = set(duplicate_child_ids)
    normalized_derived_nodes: list[dict[str, Any]] = []
    for node in derived_nodes:
        node_id = str(node.get("id") or "")
        if node_id not in duplicate_child_id_set:
            normalized_derived_nodes.append(node)
            continue
        normalized_node = dict(node)
        if str(normalized_node.get("document_anchor_id") or "").strip() in primary_refs:
            normalized_node["document_anchor_id"] = None
        if str(normalized_node.get("derived_from_preview_id") or "").strip() in primary_refs:
            normalized_node["derived_from_preview_id"] = None
        normalized_derived_nodes.append(normalized_node)
    primary_preview = {
        **primary_preview,
        "selected_by_default": False,
        "apply_eligible": False,
        "apply_suppression_reason": "identical_structural_anchor_duplicate_of_child",
        "apply_suppressed_by_preview_ids": duplicate_child_ids,
    }
    return primary_preview, normalized_derived_nodes
...
def _source_section_short_self_memory_preview_items(
    *,
    source_text: str,
    source_sections: list[dict[str, Any]] | None,
    source_investigation_id: str | None = None,
    ...
    for section in [dict(item) for item in list(source_sections or []) if isinstance(item, dict)]:
        ...
        if len(claim_text) < 28 or len(claim_text) > 160:
            continue
        if _SOURCE_MOJIBAKE_RE.search(claim_text):
            continue
        if _SOURCE_METADATA_FRAGMENT_START_RE.search(claim_text):
            continue
        if _SOURCE_LOW_VALUE_SENTENCE_RE.search(claim_text):
            continue
        if not _SOURCE_CLAIM_ACTION_RE.search(claim_text):
            continue
        provider_normalized_self_subject = bool(
            re.search(
                r"\b(?:the\s+user|user['’]s|utente)\b|\bl['’]utente\b",
                claim_text,
                re.IGNORECASE,
            )
        )
        if not _source_has_memory_anchor(claim_text) and not provider_normalized_self_subject:
            continue
        key = normalize_preview_text(claim_text)
        if not key or key in seen:
            continue
        seen.add(key)
        start, end = _span_bounds_in_lower_source(source_text_lower, claim_text[: min(len(claim_text), 160)])
        claim_type = _source_claim_type_for_source_fact(claim_text)
        confidence = 0.84
        retrieval_aliases = _source_question_variants_for_claim(
            claim_text=claim_text,
        ...
        output.append(
            {
                ...
                "source_grounding": {
                    ...
                    "supported": True,
                    "score": 1.0,
                    "reason": "short_self_memory_claim_from_source_unit",
                },
                "retrieval_affordance": {
                    "schema_version": "agvm.source_claim_retrieval_affordance.v1",
                    "question": retrieval_aliases[0] if retrieval_aliases else "What personal preference is stated here?",
                    "answer_claim": claim_text,
                    "purpose": "preserve_short_user_asserted_self_memory_claim",
                },
                "retrieval_aliases": retrieval_aliases,
            }
        )
...
def _source_context_purpose_text(value: Any) -> str:
    if isinstance(value, dict):
        return " ".join(
...
def _source_context_self_memory_requested(source_context: Mapping[str, Any]) -> bool:
    source_purpose_text = _source_context_purpose_text(
        source_context.get("source_purpose")
    )
    operator_instruction = str(source_context.get("operator_instruction") or "")
    source_request = dict(source_context.get("source_request") or {})
    requested_options = dict(dict(source_request.get("options") or {}).get("requested") or {})
    effective_options = dict(dict(source_request.get("options") or {}).get("effective") or {})
    option_text = " ".join(
        str(
            bucket.get(key)
            or ""
        )
        for bucket in (source_request, requested_options, effective_options)
        for key in ("treat_as", "source_purpose", "purpose", "user_instruction")
    )
    return bool(
        re.search(
            r"\b(?:self[-_ ]memory|personal\s+preference|preference)\b",
            f"{source_purpose_text} {operator_instruction} {option_text}",
            re.IGNORECASE,
        )
    )


def _source_context_user_asserted_or_first_person(
    *,
    source_trust: str | None,
    ...
    trust_values = {
        ...
        for value in (
            source_trust,
            source_context.get("source_trust"),
            source_request.get("source_trust"),
            requested_options.get("source_trust"),
            effective_options.get("source_trust"),
            dict(source_context.get("provenance") or {}).get("source_trust"),
        )
    }
    if "user_asserted" in trust_values:
        return True
    for section in [dict(item) for item in list(source_sections or []) if isinstance(item, dict)]:
        if re.search(
            r"\b(?:i|my|mine|me|we|our|ours|sono|mio|mia|miei|mie|noi|nostro|nostra)\b",
            _source_section_text(section),
            re.IGNORECASE,
        ):
            return True
    return False
...
def preview_bundle(
    ...
    *,
    ...
    if source_investigation_scope and compiled_derived:
        # In a source investigation, provenance is a server-issued capability:
        # a non-empty provider value is not sufficient.  Only section ids that
        # were actually handed to this compilation may enter the preview.  In
        # Grow v3 those ids are the investigator claim ids, so accepting an
        # arbitrary/stale source_unit_id here leaks an unbindable semantic node
        # into the otherwise claim-bound bundle.
        allowed_source_unit_ids = set(source_sections_by_id)
        source_scoped_compiler_nodes = []
        unknown_source_unit_count = 0
        missing_source_unit_count = 0
        for item in compiled_derived:
            source_unit_id = str(item.get("source_unit_id") or "").strip()
            if source_unit_id and source_unit_id in allowed_source_unit_ids:
                source_scoped_compiler_nodes.append(item)
            elif source_unit_id:
                unknown_source_unit_count += 1
            else:
            ...
            warnings.append(
                {
                    ...
                    "message": (
                        f"Suppressed {suppressed_free_nodes} compiler-derived source node(s) without a "
                        "server-issued source section binding "
                        f"(missing={missing_source_unit_count}, unknown={unknown_source_unit_count}). "
                        "Real source Grow can persist only raw anchors, source facts and source chunks with "
                        "explicit provenance."
                    ),
                }
            )
    ...
    primary_seed = build_seed(
        ...
        memory_type_override=(
            ...
            else map_runtime_memory_type(primary_compiled.get("memory_type"))
        ),
        guide_area_override=(
            "documents"
            if source_investigation_scope
            else primary_compiled.get("guide_area") or infer_guide_area(text)
        ),
    ...
    if require_ai and source_investigation_scope and source_sections:
        ...
        for repair_attempt in range(2):
            ...
            repair_items, _, repair_error = llm_source_unit_semantic_compile(
                ...
                operator_instruction=(
                    ...
                    "Each raw_text must be a complete, self-contained declarative sentence with an explicit "
                    "subject and action verb. Expand slash-separated or telegraphic source wording into fluent "
                    "language without adding facts. Do not echo headings or keyword lists."
                ),
                identity_nucleus=identity_nucleus,
                nearby_context=nearby_context,
                source_unit_formation=source_unit_formation,
        ...
        if missing_source_unit_ids:
            coverage_recoverable_error = (
                not source_semantic_error
                or source_semantic_error == "llm_disabled"
                or "provider_did_not_cover_all_fact_eligible_source_units" in str(source_semantic_error)
            )
            if coverage_recoverable_error:
                missing_sections = [
                    dict(section)
                    for section in source_sections
                    if isinstance(section, dict)
                    ...
                    warnings.append(
                        {
                            ...
                            "message": (
                                ...
                                "source-bound vectors because the per-unit semantic compiler "
                                f"{'is disabled' if source_semantic_error == 'llm_disabled' else 'returned incomplete source-unit coverage'}."
                            ),
                        }
                    )
                missing_source_unit_ids = sorted(required_source_unit_ids - set(source_ai_items_by_unit))
                if missing_source_unit_ids:
                    ...
                    missing_source_unit_ids = sorted(required_source_unit_ids - set(source_ai_items_by_unit))
                short_self_memory_coverage_allowed = bool(
                    missing_source_unit_ids
                    and _source_context_user_asserted_or_first_person(
                        source_trust=source_trust,
                        source_context=resolved_source_context,
                        source_sections=[
                            dict(section)
                    ...
                    and _source_context_self_memory_requested(resolved_source_context)
                )
                if short_self_memory_coverage_allowed:
                    missing_sections = [
                        dict(section)
                        for section in source_sections
                        if isinstance(section, dict)
                    ...
                    short_items = _source_section_short_self_memory_preview_items(
                        source_text=text,
                        source_sections=missing_sections,
                        source_investigation_id=source_investigation_id,
                    )
                    stabilized_short_items: list[dict[str, Any]] = []
                    for item in short_items:
                        claim_text = str(item.get("raw_text") or item.get("summary") or "").strip()
                        if not claim_text:
                            continue
                        claim_type = str(item.get("claim_type") or _source_claim_type_for_source_fact(claim_text))
                        stabilized_semantics = _stabilize_compiled_semantics(
                            raw_text=claim_text,
                            input_mode=input_mode,
                            summary=str(item.get("summary") or claim_text),
                            memory_type=str(item.get("memory_type") or map_runtime_memory_type(None, claim_type=claim_type)),
                            guide_area=None,
                            routing_scores=dict(item.get("routing_semantic_scores") or {}),
                            routing_facets=dict(item.get("routing_facets") or {}),
                            granularity=None,
                        ...
                        stabilized_short_items.append(
                            {
                                **item,
                                "claim_type": claim_type if claim_type in CLAIM_TYPES else _source_claim_type_for_source_fact(claim_text),
                                "memory_type": stabilized_semantics["memory_type"],
                                "routing_semantic_scores": stabilized_semantics["routing_semantic_scores"],
                                "routing_facets": stabilized_semantics["routing_facets"],
                                "semantic_vector_source": "source_bound_short_self_memory_coverage",
                            }
                        )
                    compiled_derived, short_fallback_count = _append_source_section_preview_items(
                        compiled_derived,
                        stabilized_short_items,
                    )
                    for semantic_item in stabilized_short_items:
                        semantic_unit_id = str(semantic_item.get("source_unit_id") or "").strip()
                        if semantic_unit_id and has_ai_semantic_vector(semantic_item):
                            source_ai_items_by_unit[semantic_unit_id].append(semantic_item)
                    if short_fallback_count:
                        warnings.append(
                            {
                                "code": "source_unit_short_self_memory_coverage",
                                "message": (
                                    f"Covered {short_fallback_count} short user-asserted self-memory source unit(s) "
                                    "with source-bound semantic vectors because provider source-unit coverage was incomplete."
                                ),
                            }
                        )
                    missing_source_unit_ids = sorted(required_source_unit_ids - set(source_ai_items_by_unit))
    ...
    primary_preview, derived_nodes = _suppress_identical_structural_anchor_apply(
        primary_preview=primary_preview,
        derived_nodes=derived_nodes,
        source_type=source_type,
        source_sections=source_sections,
...
def resolve_persist_selection(
    ...
    *,
    ...
    primary_preview = dict(normalized_bundle.get("primary_node_preview") or {})
    primary_preview_id = str(primary_preview.get("id") or "").strip()
    if include_primary and primary_preview_id and _preview_node_apply_eligible(primary_preview):
    ...
    resolved_ids = [
        ...
        and str(node.get("id") or "") in persistable_ids
        and _preview_node_apply_eligible(node)
    ]
