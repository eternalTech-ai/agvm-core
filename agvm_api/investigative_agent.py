...
PostToolBatchTerminalizer = Callable[
...
def _call_provider_with_hard_timeout(
    ...
    *,
    ...
    future = executor.submit(context.run, provider, request)
    try:
...
def run_investigative_agent(
    *,
    ...
    post_tool_batch_terminalizer: PostToolBatchTerminalizer | None = None,
    ...
    while provider_calls < max_provider_calls:
        ...
        if tool_calls:
            ...
            if post_tool_batch_terminalizer is not None:
                try:
                    terminal_candidate = post_tool_batch_terminalizer(
                        normalized_payload,
                        tool_calls,
                        [
                    ...
                    return incomplete("server_policy_failed", str(exc))
                if terminal_candidate is not None:
                    terminal_payload, terminal_error = turn_validator(
                        dict(terminal_candidate),
                        turn_number,
                    )
                    if terminal_error or terminal_payload is None:
                        return incomplete(
                            "server_policy_failed",
                            str(terminal_error or "terminal_payload_invalid"),
                        )
                    if (
                        str(terminal_payload.get("status") or "") != "complete"
                        or terminal_payload.get("tool_calls")
                        or terminal_payload.get("questions")
                    ):
                        return incomplete(
                            "server_policy_failed",
                            "terminal_payload_not_complete",
                        )
                    normalized_payload = terminal_payload
                    last_payload = dict(terminal_payload)
                    tool_calls = []
