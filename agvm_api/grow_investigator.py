...
_GROW_PROVIDER_CONTEXT_GUARD_TOKENS = 16_000
_GROW_PROVIDER_CONTEXT_OVERHEAD_TOKENS = 512
_GROW_PROVIDER_MAX_OUTPUT_TOKENS = 4_000
_GROW_PROVIDER_MIN_OUTPUT_TOKENS = 768
_GROW_PROVIDER_PROMPT_COMPACTION_MIN_OUTPUT_TOKENS = 1_024
_GROW_PROVIDER_ESTIMATED_CHARS_PER_TOKEN = 3
...
def _grow_search_worker_count(
    *, requested_concurrency: int, search_call_count: int
) -> int:
    """Keep child Search waves within the process-wide provider capacity."""

    if search_call_count <= 0:
        return 0
    return min(
        max(1, int(requested_concurrency)),
        int(search_call_count),
        max(1, int(llm_provider_concurrency_limit())),
    )


def _grow_provider_request_budget(request: Mapping[str, Any]) -> dict[str, Any]:
    """Bound structured Grow output against a conservative 16k provider window."""

    system_prompt = str(request.get("system_prompt") or "")
    user_prompt = str(request.get("user_prompt") or "")
    schema_json = json.dumps(
        _dict(request.get("schema")),
        ensure_ascii=True,
        sort_keys=True,
        separators=(",", ":"),
    )
    input_chars = len(system_prompt) + len(user_prompt) + len(schema_json)
    estimated_input_tokens = (
        input_chars + _GROW_PROVIDER_ESTIMATED_CHARS_PER_TOKEN - 1
    ) // _GROW_PROVIDER_ESTIMATED_CHARS_PER_TOKEN
    available_output_tokens = (
        _GROW_PROVIDER_CONTEXT_GUARD_TOKENS
        - _GROW_PROVIDER_CONTEXT_OVERHEAD_TOKENS
        - estimated_input_tokens
    )
    max_output_tokens = min(
        _GROW_PROVIDER_MAX_OUTPUT_TOKENS,
        max(0, available_output_tokens),
    )
    return {
        "schema_version": "agvm.grow_provider_request_budget.v1",
        "system_prompt_chars": len(system_prompt),
        "user_prompt_chars": len(user_prompt),
        "schema_chars": len(schema_json),
        "input_chars": input_chars,
        "estimated_input_tokens": estimated_input_tokens,
        "context_guard_tokens": _GROW_PROVIDER_CONTEXT_GUARD_TOKENS,
        "context_overhead_tokens": _GROW_PROVIDER_CONTEXT_OVERHEAD_TOKENS,
        "max_output_tokens": max_output_tokens,
        "within_context_guard": max_output_tokens >= _GROW_PROVIDER_MIN_OUTPUT_TOKENS,
    }
...
def _child_search_query_authority_cap_seconds(
    budget: "GrowInvestigationBudget",
) -> float:
    """Bound source-blind query composition outside the child Search budget."""

    return max(0.001, min(45.0, float(budget.provider_timeout_seconds)))


def _child_search_batch_cap_seconds(
    budget: "GrowInvestigationBudget",
    retrieval_mode: str,
) -> float:
    """Cover query authority plus one complete child Search lifecycle.

    The source-blind query is authored by Grow before Search starts.  Charging
    that provider latency to Search's own mode deadline can leave the required
    coordinate-first landing planner with only a fraction of its configured
    window.  The parent Grow deadline and final-review reserve remain the hard
    outer authorities.
    """

    return (
        _child_search_query_authority_cap_seconds(budget)
        + _child_search_budget_cap_seconds(retrieval_mode)
    )
...
class GrowInvestigationBudget:
    ...
    wall_budget_seconds: int = 600
    provider_timeout_seconds: int = 180
    ai_review_reserve_seconds: int = 180
    ...
    def from_env(cls) -> "GrowInvestigationBudget":
        ...
        return cls(
            ...
            wall_budget_seconds=bounded("AGVM_GROW_AI_INVESTIGATOR_WALL_SECONDS", 600, 10, 600),
            provider_timeout_seconds=bounded("AGVM_GROW_AI_INVESTIGATOR_TIMEOUT_SECONDS", 180, 1, 180),
            ai_review_reserve_seconds=bounded("AGVM_GROW_AI_REVIEW_RESERVE_SECONDS", 180, 90, 180),
...
_DECISION_SCHEMA = {
    ...
    "properties": {
        ...
        "target_node_ids": {"type": "array", "items": {"type": "string"}, "maxItems": 1},
...
def _defer_attestation_repair_schema(
    *, decision_enum: Sequence[str] | None = None, forbid_defer: bool = False
) -> dict[str, Any]:
    """Narrow defer-attestation repair to clarification or attested decisions."""
    ...
    for field in ("claims", "answer_claims", "exclusions", "tool_calls"):
        properties[field] = {**_dict(properties.get(field)), "maxItems": 0}
    decisions = _dict(properties.get("decisions"))
    decision_items = _dict(decisions.get("items"))
    decision_properties = _dict(decision_items.get("properties"))
    if decision_enum is not None:
        narrowed_decisions = sorted(set(_strings(decision_enum, limit=16)) & DECISION_VALUES)
        decision_properties["decision"] = {
            **_dict(decision_properties.get("decision")),
            "enum": narrowed_decisions,
        }
        if narrowed_decisions == ["source_only"]:
            decision_properties["target_node_ids"] = {
                **_dict(decision_properties.get("target_node_ids")),
                "maxItems": 0,
            }
    elif forbid_defer:
        decision_properties["decision"] = {
            **_dict(decision_properties.get("decision")),
            "enum": sorted(DECISION_VALUES - {"defer"}),
        }
    decision_properties["no_clarification_can_change_decision"] = {
        **_dict(decision_properties.get("no_clarification_can_change_decision")),
        "enum": [True],
    }
    decision_properties["no_clarification_explanation"] = {
        **_dict(decision_properties.get("no_clarification_explanation")),
        "minLength": 1,
    }
    decision_items["properties"] = decision_properties
    decision_items["required"] = sorted(
        set(_strings(decision_items.get("required"), limit=32))
        | {"no_clarification_can_change_decision", "no_clarification_explanation"}
    )
...
def _source_only_personal_repair_schema() -> dict[str, Any]:
    """Narrow a personal source-only repair so decide/defer cannot repeat."""
...
def _initial_source_binding_repair_schema() -> dict[str, Any]:
    """Narrow initial source-binding repair to claim rebinding plus Search."""
    ...
    properties["claims"] = {
        **_dict(properties.get("claims")),
        "minItems": 1,
    }
    properties["exclusions"] = {
        **_dict(properties.get("exclusions")),
        "maxItems": 0,
    }
    properties["tool_calls"] = {
        **_dict(properties.get("tool_calls")),
        "minItems": 1,
    }
    for field in ("answer_claims", "questions", "decisions"):
        properties[field] = {**_dict(properties.get(field)), "maxItems": 0}
    schema["properties"] = properties
    return schema


def _missing_first_search_repair_schema() -> dict[str, Any]:
    """Narrow missing-first-search repair to a Search wave only."""
    ...
    tool_items = _dict(properties["tool_calls"].get("items"))
    tool_properties = _dict(tool_items.get("properties"))
    tool_properties["tool_name"] = {
        **_dict(tool_properties.get("tool_name")),
        "enum": ["search_brain"],
    }
    tool_items["properties"] = tool_properties
    properties["tool_calls"]["items"] = tool_items
    for field in ("claims", "answer_claims", "exclusions", "questions", "decisions"):
        properties[field] = {**_dict(properties.get(field)), "maxItems": 0}
    schema["properties"] = properties
    return schema


def _closed_gap_decision_repair_schema(*, defer_only: bool = False) -> dict[str, Any]:
    ...
    if defer_only:
        decision_items = _dict(properties["decisions"].get("items"))
        decision_properties = _dict(decision_items.get("properties"))
        decision_properties["decision"] = {
            **_dict(decision_properties.get("decision")),
            "enum": ["defer"],
        ...
        decision_items["properties"] = decision_properties
        properties["decisions"]["items"] = decision_items
    schema["properties"] = properties
    return schema


def _source_only_authority_repair_schema() -> dict[str, Any]:
    """Narrow a source-only authority repair so the invalid action cannot repeat."""

    schema = json.loads(json.dumps(GROW_INVESTIGATOR_RESPONSE_SCHEMA))
    properties = _dict(schema.get("properties"))
    for field in ("claims", "answer_claims", "exclusions"):
        properties[field] = {**_dict(properties.get(field)), "maxItems": 0}
    decisions = _dict(properties.get("decisions"))
    decision_items = _dict(decisions.get("items"))
    decision_properties = _dict(decision_items.get("properties"))
    decision_properties["decision"] = {
        **_dict(decision_properties.get("decision")),
        "enum": sorted(DECISION_VALUES - {"source_only"}),
    }
    decision_items["properties"] = decision_properties
    decisions["items"] = decision_items
    properties["decisions"] = decisions
...
def _receipt_payload_integrity_valid(value: Mapping[str, Any]) -> bool:
    proof = _dict(value.get("search_authority_proof"))
    if proof.get("payload_integrity_valid") is True:
        return True
    integrity = _dict(value.get("payload_integrity"))
    return integrity.get("passed") is True or integrity.get("valid") is True


def _receipt_is_attested_terminal_review_boundary(value: Mapping[str, Any]) -> bool:
    """Recognize a completed AI Search attempt without granting evidence authority."""

    if not _is_v3_search_receipt(value) or not _receipt_has_semantic_firewall(value):
        return False
    attestation = _dict(value.get("search_execution_attestation"))
    return bool(
        str(value.get("terminal_state") or "").strip().casefold()
        == "review_required"
        and value.get("terminal_for_client") is True
        and value.get("evidence_usable") is False
        and value.get("question_usable") is False
        and value.get("decision_usable") is False
        and (
            value.get("reviewable") is True
            or _receipt_payload_integrity_valid(value)
        )
        and str(attestation.get("status") or "").strip().casefold()
        in {"completed", "review_required"}
        and attestation.get("provider_executed") is True
        and attestation.get("fallback_used") is not True
    )


def _receipt_is_attested_nondispositive_question_boundary(
    value: Mapping[str, Any],
) -> bool:
    """Recognize an attested Search result that can ask but cannot decide."""
    ...
    return bool(
        ...
        == "review_required"
        and value.get("terminal_for_client") is True
        and value.get("reviewable") is True
        and value.get("question_usable") is True
        and value.get("decision_usable") is False
        and str(attestation.get("status") or "").strip().casefold()
        in {"completed", "review_required"}
        and attestation.get("provider_executed") is True
...
def _receipt_is_attested_terminal_question_boundary(value: Mapping[str, Any]) -> bool:
    """Recognize a terminal Search gap with no independently usable evidence."""

    return bool(
        value.get("evidence_usable") is False
        and _receipt_is_attested_nondispositive_question_boundary(value)
    )
...
def _provider_prompt_json(value: Mapping[str, Any]) -> str:
    return json.dumps(value, ensure_ascii=True, sort_keys=True, default=str)


def _grow_provider_user_prompt_char_budget(
    *,
    system_prompt: str,
    ...
    max_input_chars = (
        _GROW_PROVIDER_CONTEXT_GUARD_TOKENS
        - _GROW_PROVIDER_CONTEXT_OVERHEAD_TOKENS
        - _GROW_PROVIDER_PROMPT_COMPACTION_MIN_OUTPUT_TOKENS
    ) * _GROW_PROVIDER_ESTIMATED_CHARS_PER_TOKEN
    schema_chars = len(
        json.dumps(
            _dict(schema),
    ...
    return max(1_000, max_input_chars - len(system_prompt) - schema_chars - 512)


def _compact_search_master_judgement_for_provider_prompt(
    value: Any,
    *,
    char_limit: int = 500,
) -> Any:
    ...
    ai_decision = _dict(master.get("ai_master_decision"))
    sufficiency_decision = _dict(
        _dict(master.get("sufficiency_judge")).get("ai_master_decision")
    )
    accepted_support_ids: list[str] = []
    for container in (master, ai_decision, sufficiency_decision):
        validation = _dict(container.get("mission_entailment_validation"))
        accepted_support_ids.extend(
            _strings(validation.get("accepted_support_ids"), limit=24)
        )
    for decision in (ai_decision, sufficiency_decision):
        for assessment in _dicts(decision.get("mission_evidence_assessments"), limit=24):
            if assessment.get("entailed") is True:
                accepted_support_ids.extend(
                    _strings(assessment.get("evidence_node_ids"), limit=24)
                )

    field_limit = max(1, int(char_limit))

    def text(*values: Any, limit: int = 240) -> str | None:
        resolved_limit = max(1, min(int(limit), field_limit))
        for item in values:
            candidate = str(item or "").strip()
            if candidate:
                return candidate[:resolved_limit]
        return None

    return {
        "schema_version": "agvm.grow_search_master_prompt_projection.v1",
        "master_judgement_sha256": stable_digest(master),
        "master_state": text(master.get("master_state"), limit=120),
        "decision": text(master.get("decision"), limit=120),
        "unresolved_gap": text(
        ...
        "next_evidence_action": text(
            master.get("next_evidence_action"),
            continuation.get("next_evidence_action"),
            continuation.get("reason"),
            limit=360,
        ),
        "accepted_support_ids": list(dict.fromkeys(accepted_support_ids))[:24],
    }


def _compact_search_authority_proof_for_provider_prompt(value: Any) -> dict[str, Any]:
    proof = _dict(value)
    if not proof:
        return {}
    if (
        proof.get("provider_prompt_compacted") is True
        and str(proof.get("search_authority_proof_sha256") or "").strip()
    ):
        return proof
    projected: dict[str, Any] = {
        ...
        for key in (
            ...
            "plan_first_metadata_present",
            "plan_first_metadata_valid",
            "ledger_digest",
            "master_ledger_digest",
            "strong_no_match_contract",
            "document_context_usable",
            "question_usable",
            "partial_useful_matches",
        )
        if key in proof
    }
    projected["search_authority_proof_sha256"] = stable_digest(proof)
    projected["provider_prompt_compacted"] = True
    return projected


def _compact_search_query_review_for_provider_prompt(value: Any) -> dict[str, Any]:
    review = _dict(value)
    if not review:
        return {}
    firewall = _dict(review.get("semantic_firewall"))
    compacted_firewall = {
        key: firewall.get(key)
        for key in (
            "schema_version",
            "source_claim_visible_to_search_runtime",
            "investigator_query_visible_to_search_runtime",
            "source_blind_query_authority_used",
            "single_provider_query_authority",
            "pre_search_provider_repair_call_count",
            "neutral_subject_anchors_sha256",
            "subject_anchor_verification_sha256",
            "isolated_query_boundary_sha256",
        )
        if key in firewall
    }
    return {
        "semantic_firewall": compacted_firewall,
        "query_review_sha256": stable_digest(review),
        "provider_prompt_compacted": True,
    }


def _compact_document_references_for_provider_prompt(
    value: Any,
    *,
    limit: int,
    text_chars: int,
    ...
    for reference in _dicts(value, limit=max(0, int(limit))):
        references.append(
            {
                key: _compact_source_blind_text(raw_value, limit=max(0, int(text_chars)))
                if isinstance(raw_value, str)
                else raw_value
                for key, raw_value in {
                    "document_id": reference.get("document_id"),
                    "anchor_node_id": reference.get("anchor_node_id"),
...
def _compact_search_receipt_for_provider_prompt(
    ...
    *,
    ...
    include_document_references: bool,
    document_reference_limit: int = 8,
) -> dict[str, Any]:
    item = _dict(receipt)
    receipt_sha256 = str(item.get("search_receipt_sha256") or "").strip() or stable_digest(item)
    projected: dict[str, Any] = {
        key: item.get(key)
        for key in (
            "schema_version",
            ...
            "terminal_state",
            "terminal_for_client",
            "grow_target_mutation_usable",
            "grow_receipt_authority_sha256",
            "wave",
            "provider_call_ref",
            "server_call_id",
            "server_call_authority_sha256",
        )
        if key in item
    }
    projected["search_receipt_sha256"] = receipt_sha256
    projected["affected_claim_ids"] = _strings(
        projected.get("affected_claim_ids"),
        limit=128,
    )
    projected["evidence_node_ids"] = _strings(
        item.get("evidence_node_ids"),
        limit=max(1, int(evidence_ref_limit)),
    )
    projected["query_text"] = _compact_source_blind_text(
        projected.get("query_text"),
        limit=max(120, min(700, int(context_excerpt_chars) * 4)),
    )
    projected["purpose"] = _compact_source_blind_text(
        projected.get("purpose"),
        limit=max(100, min(500, int(context_excerpt_chars) * 3)),
    )
    projected["context_excerpt"] = _compact_source_blind_text(
        item.get("context_excerpt"),
        limit=max(0, int(context_excerpt_chars)),
    )
    projected["master_judgement"] = _compact_search_master_judgement_for_provider_prompt(
        item.get("master_judgement"),
        char_limit=max(0, int(master_chars)),
    )
    projected["search_authority_proof"] = _compact_search_authority_proof_for_provider_prompt(
        item.get("search_authority_proof")
    )
    projected["query_review"] = _compact_search_query_review_for_provider_prompt(
        item.get("query_review")
    )
    continuation = _search_continuation_brief(item)
    projected["continuation_brief"] = {
        key: (str(value)[: max(0, int(continuation_chars))] if isinstance(value, str) else value)
        for key, value in continuation.items()
        if value not in (None, "", [])
    }
    projected["document_references"] = (
        _compact_document_references_for_provider_prompt(
            item.get("document_references"),
            limit=document_reference_limit,
            text_chars=max(80, min(240, int(context_excerpt_chars))),
        )
...
def _compact_hydrated_evidence_for_provider_prompt(
    ...
    *,
    ...
    for receipt in receipts:
        ordered_ids.extend(_strings(receipt.get("evidence_node_ids"), limit=512))
    ordered_ids.extend(str(node_id) for node_id in hydrated_evidence.keys())
    visible_ids = [
        node_id
        for node_id in dict.fromkeys(ordered_ids)
        if node_id in hydrated_evidence
    ][: max(1, int(max_nodes))]
    compacted: dict[str, dict[str, Any]] = {}
    for node_id in visible_ids:
        material = _dict(hydrated_evidence.get(node_id))
        provenance = _dict(material.get("provenance"))
        canonical_text = str(
            material.get("canonical_text") or material.get("raw_text") or ""
        )
        summary = str(material.get("summary") or "")
        compacted[node_id] = {
            "node_id": str(material.get("node_id") or node_id),
            "canonical_text": canonical_text[: max(0, int(canonical_chars))],
            "summary": summary[: max(0, int(summary_chars))],
            "claim_status": str(material.get("claim_status") or "fact"),
            "source_trust": str(material.get("source_trust") or "unknown"),
            "node_revision": material.get("node_revision"),
            "temporal_role": material.get("temporal_role"),
            "observed_at": material.get("observed_at"),
            "valid_from": material.get("valid_from"),
            "valid_to": material.get("valid_to"),
            "lifecycle_status": str(material.get("lifecycle_status") or "active"),
            "provenance": {
                key: provenance.get(key)
                for key in ("source_type", "source_trust", "source_uri")
                if provenance.get(key) is not None
            },
            "digest": str(material.get("digest") or stable_digest(material)),
            "provider_prompt_compacted": (
                len(canonical_text) > int(canonical_chars)
                or len(summary) > int(summary_chars)
            ),
        }
    return compacted


def _compact_clarification_for_provider_prompt(
    question: Mapping[str, Any],
    *,
    max_claims: int,
    question_text_chars: int,
    answer_chars: int,
    summary_chars: int,
    ...
    choices = [
        _compact_source_blind_text(choice, limit=160)
        for choice in _strings(item.get("choices"), limit=12)
    ]
    compacted: dict[str, Any] = {
        "schema_version": str(
            item.get("schema_version") or GROW_CLARIFICATION_SET_SCHEMA_VERSION
        ),
        "question_id": str(item.get("question_id") or ""),
        "provider_question_id": str(item.get("provider_question_id") or "")
        or None,
        "question_sha256": stable_digest(
            {
                key: item.get(key)
                for key in (
                    "question_text",
        ...
        "question_text": _compact_source_blind_text(
            question_text,
            limit=max(0, int(question_text_chars)),
        ),
        "answer_type": str(item.get("answer_type") or ""),
        "choices": choices,
        "required_for_preview": bool(item.get("required_for_preview")),
        "answer_state": str(item.get("answer_state") or "unanswered"),
        "answer": _compact_source_blind_text(answer, limit=max(0, int(answer_chars))),
        "answer_sha256": stable_digest(answer) if answer else None,
        "affected_claim_ids": _strings(item.get("affected_claim_ids"), limit=max_claims),
        "decision_effect": str(item.get("decision_effect") or ""),
        "source_claim_summary": _compact_source_blind_text(
            item.get("source_claim_summary"),
            limit=max(0, int(summary_chars)),
        ),
        "brain_evidence_summary": _compact_source_blind_text(
            item.get("brain_evidence_summary"),
            limit=max(0, int(summary_chars)),
        ),
        "comparison": _compact_source_blind_text(
            item.get("comparison"),
            limit=max(0, int(explanation_chars)),
        ),
        "reason": _compact_source_blind_text(
            item.get("reason"),
            limit=max(0, int(explanation_chars)),
        ),
        "impact": _compact_source_blind_text(
            item.get("impact"),
            limit=max(0, int(explanation_chars)),
        ),
        "next_step": _compact_source_blind_text(
            item.get("next_step"),
            limit=max(0, int(explanation_chars)),
        ),
        "evidence_refs": _strings(item.get("evidence_refs"), limit=32),
        "provider_prompt_compacted": (
            len(question_text) > int(question_text_chars)
            or len(answer) > int(answer_chars)
            or any(
                len(str(item.get(key) or ""))
                > (
                    int(summary_chars)
                    if key in {"source_claim_summary", "brain_evidence_summary"}
                    else int(explanation_chars)
                )
                for key in (
                    "source_claim_summary",
                    "brain_evidence_summary",
...
def _compact_clarifications_for_provider_prompt(
    ...
    *,
    ...
    return [
        _compact_clarification_for_provider_prompt(
            question,
            max_claims=max_claims,
            question_text_chars=question_text_chars,
            answer_chars=answer_chars,
            summary_chars=summary_chars,
            explanation_chars=explanation_chars,
        )
        for question in list(questions or [])[: max(1, int(max_questions))]
        if isinstance(question, Mapping)
    ]


def _closed_clarification_gaps_for_provider_prompt(
    questions: Sequence[Mapping[str, Any]],
    *,
    max_claims: int,
    max_questions: int,
    ...
    return [
        ...
        for item in _compact_clarifications_for_provider_prompt(
            [
                question
                for question in questions
                if str(question.get("question_id") or "")
                and str(question.get("answer_state") or "unanswered")
...
def _compact_clarification_answers_for_provider_prompt(
    answers: Mapping[str, Any],
    *,
    answer_chars: int,
) -> dict[str, Any]:
    compacted: dict[str, Any] = {}
    for question_id, answer in _dict(answers).items():
        normalized_id = str(question_id or "").strip()
        if not normalized_id:
            continue
        if isinstance(answer, Mapping):
            answer_mapping = _dict(answer)
            compacted[normalized_id] = {
                "state": str(answer_mapping.get("state") or ""),
                "answer": _compact_source_blind_text(
                    answer_mapping.get("answer")
                    or answer_mapping.get("text")
                    or answer_mapping.get("value")
                    or "",
                    limit=max(0, int(answer_chars)),
                ),
                "answer_sha256": stable_digest(answer_mapping),
            }
        else:
            answer_text = str(answer or "")
            compacted[normalized_id] = _compact_source_blind_text(
                answer_text,
                limit=max(0, int(answer_chars)),
            )
    return compacted
...
def _compact_claim_ledger_for_provider_prompt(
    claims: Sequence[Mapping[str, Any]],
    *,
    max_claims: int,
    quote_chars: int,
    claim_chars: int,
    investigation_chars: int,
) -> list[dict[str, Any]]:
    ...
    for claim in list(claims or [])[: max(1, int(max_claims))]:
        ...
        natural_claim = str(item.get("natural_language_claim") or "")
        investigation_need = str(item.get("investigation_need") or "")
        temporal_scope = item.get("temporal_scope")
        if isinstance(temporal_scope, Mapping):
            scope = _dict(temporal_scope)
            temporal_scope = {
                key: scope.get(key)
                for key in (
                    "schema_version",
            ...
            temporal_scope["temporal_scope_sha256"] = stable_digest(scope)
        compacted.append(
            {
                key: value
                for key, value in {
                    ...
                    "exact_quote": _compact_source_blind_text(
                        exact_quote,
                        limit=max(0, int(quote_chars)),
                    ),
                    "exact_quote_sha256": stable_digest(exact_quote)
                    if exact_quote
                    else None,
                    "subject_anchor_start": item.get("subject_anchor_start"),
                    "subject_anchor_end": item.get("subject_anchor_end"),
                    "neutral_subject_anchor": item.get("neutral_subject_anchor"),
                    "natural_language_claim": _compact_source_blind_text(
                        natural_claim,
                        limit=max(0, int(claim_chars)),
                    ),
                    "natural_language_claim_sha256": stable_digest(natural_claim)
                    if natural_claim
                    else None,
                    "subject_hypotheses": _strings(
                        item.get("subject_hypotheses"),
                        limit=4,
                    ),
                    "temporal_scope": temporal_scope,
                    "epistemic_posture": item.get("epistemic_posture"),
                    "investigation_need": _compact_source_blind_text(
                        investigation_need,
                        limit=max(0, int(investigation_chars)),
                    ),
                    "investigation_need_sha256": stable_digest(
                        investigation_need
                    )
                    if investigation_need
                    else None,
                    "claim_sha256": stable_digest(item),
                    "provider_prompt_compacted": True,
                }.items()
                if value not in (None, "", [])
            }
...
def _compact_review_action_contract_for_provider_prompt(
    ...
    *,
    ...
    if not item:
        return {}
    question_policy = _dict(item.get("question_id_policy"))
    hydration_policy = _dict(item.get("hydration_policy"))
    hydration_repair = _dict(item.get("decision_target_hydration_repair"))
    return {
        key: value
        for key, value in {
            "schema_version": "agvm.grow_review_action_contract_prompt_projection.v1",
            "review_action_contract_sha256": stable_digest(item),
            "allowed_actions": _strings(item.get("allowed_actions"), limit=16),
            "question_only_claim_ids": _strings(
                item.get("question_only_claim_ids"),
                limit=max_claims,
            ),
            "question_only_receipt_ids": _strings(
                item.get("question_only_receipt_ids"),
                limit=evidence_ref_limit,
            ),
            "source_only_eligible_claim_ids": _strings(
                item.get("source_only_eligible_claim_ids"),
                limit=max_claims,
            ),
            "self_attested_personal_claim_ids": _strings(
                item.get("self_attested_personal_claim_ids"),
                limit=max_claims,
            ),
            "post_hydration_question_phase": item.get(
                "post_hydration_question_phase"
            ),
            "initial_search_policy": item.get("initial_search_policy"),
            "first_search_wave_call_cap": item.get("first_search_wave_call_cap"),
            "post_answer_search_call_cap": item.get("post_answer_search_call_cap"),
            "post_answer_search_policy": item.get("post_answer_search_policy"),
            "semantic_repair_search_allowed": item.get(
                "semantic_repair_search_allowed"
            ),
            "keyword_fallback_allowed": item.get("keyword_fallback_allowed"),
            "fixed_questionnaire_allowed": item.get("fixed_questionnaire_allowed"),
            "question_id_policy": {
                key: value
                for key, value in {
                    "reuse_known_question_ids": question_policy.get(
                        "reuse_known_question_ids"
                    ),
                    "answered_or_deferred_questions_are_closed": question_policy.get(
                        "answered_or_deferred_questions_are_closed"
                    ),
                    "server_canonicalizes_accidental_new_question_id_collisions": (
                        question_policy.get(
                            "server_canonicalizes_accidental_new_question_id_collisions"
                        )
                    ),
                    "known_question_ids": _strings(
                        question_policy.get("known_question_ids"),
                        limit=evidence_ref_limit,
                    ),
                    "closed_question_ids": _strings(
                        question_policy.get("closed_question_ids"),
                        limit=evidence_ref_limit,
                    ),
                }.items()
                if value not in (None, "", [])
            ...
            "hydration_policy": {
                key: value
                for key, value in {
                    "explicit_ai_review_action_required": hydration_policy.get(
                        "explicit_ai_review_action_required"
                    ),
                    "server_auto_hydrates_provisional_decision_targets": (
                        hydration_policy.get(
                            "server_auto_hydrates_provisional_decision_targets"
                        )
                    ),
                    "provisional_target_decision_requires_post_hydration_review": (
                        hydration_policy.get(
                            "provisional_target_decision_requires_post_hydration_review"
                        )
                    ),
                    "only_search_returned_ids": hydration_policy.get(
                        "only_search_returned_ids"
                    ),
                    "max_nodes_per_review_state": hydration_policy.get(
                        "max_nodes_per_review_state"
                    ),
                    "current_review_state_key": hydration_policy.get(
                        "current_review_state_key"
                    ),
                    "hydration_wave_available": hydration_policy.get(
                        "hydration_wave_available"
                    ),
                }.items()
                if value not in (None, "", [])
            },
            "decision_target_hydration_repair": {
                key: value
                for key, value in {
                    "repair_sha256": stable_digest(hydration_repair)
                    if hydration_repair
                    else None,
                    "required_node_ids": _strings(
                        hydration_repair.get("required_node_ids")
                        or hydration_repair.get("target_node_ids"),
                        limit=evidence_ref_limit,
                    ),
                    "affected_claim_ids": _strings(
                        hydration_repair.get("affected_claim_ids"),
                        limit=max_claims,
                    ),
                    "reason": _compact_source_blind_text(
                        hydration_repair.get("reason"),
                        limit=160,
                    ),
                }.items()
                if value not in (None, "", [])
...
def _compact_protocol_feedback_for_provider_prompt(
    ...
    *,
    ...
    if not item:
        return None
    compacted: dict[str, Any] = {
        "schema_version": "agvm.grow_protocol_feedback_prompt_projection.v1",
        "protocol_feedback_sha256": stable_digest(item),
        "detail": str(item.get("detail") or ""),
        "instruction": _compact_source_blind_text(
            item.get("instruction"),
            limit=max(0, int(instruction_chars)),
        ),
        "allowed_actions": _strings(item.get("allowed_actions"), limit=16),
        "forbidden_actions": _strings(item.get("forbidden_actions"), limit=16),
        "allowed_terminal_actions": _strings(
            item.get("allowed_terminal_actions"),
            limit=16,
        ),
        ...
        "required_decision_claim_ids": _strings(
            item.get("required_decision_claim_ids"),
            limit=max_claims,
        ),
        "affected_claim_ids": _strings(
        ...
        "reviewable_receipt_ids": _strings(
            item.get("reviewable_receipt_ids"),
            limit=evidence_ref_limit,
        ),
        "observed_search_receipt_ids": _strings(
            item.get("observed_search_receipt_ids"),
            limit=evidence_ref_limit,
        ),
        "allowed_document_reference_ids": _strings(
            item.get("allowed_document_reference_ids"),
            limit=evidence_ref_limit,
        ),
        "search_returned_memory_node_ids": _strings(
            item.get("search_returned_memory_node_ids"),
            limit=evidence_ref_limit,
        ),
        "forbidden_tool_name": item.get("forbidden_tool_name"),
        "required_tool_name": item.get("required_tool_name"),
        ...
        "remaining_search_calls": item.get("remaining_search_calls"),
        "first_search_wave_call_cap": item.get("first_search_wave_call_cap"),
        "post_answer_search_call_cap": item.get("post_answer_search_call_cap"),
        "provider_prompt_compacted": True,
    }
    for key in (
        "review_action_exclusivity",
        "decision_attestation_requirements",
        ...
        if isinstance(value, Mapping):
            compacted[key] = {
                ...
                for inner_key, inner_value in _dict(value).items()
                if inner_value not in (None, "", [])
            }
    hydration_required = _dict(item.get("decision_target_hydration_required"))
    if hydration_required:
        compacted["decision_target_hydration_required"] = {
            key: value
            for key, value in {
                "required_node_ids": _strings(
                    hydration_required.get("required_node_ids")
                    or hydration_required.get("target_node_ids"),
                    limit=evidence_ref_limit,
                ),
                "affected_claim_ids": _strings(
                    hydration_required.get("affected_claim_ids"),
                    limit=max_claims,
                ),
                "reason": _compact_source_blind_text(
                    hydration_required.get("reason"),
                    limit=160,
                ),
                "sha256": stable_digest(hydration_required),
            }.items()
            if value not in (None, "", [])
        }
    return {key: value for key, value in compacted.items() if value not in (None, "", [], {})}
...
def _compact_question_evidence_authority_for_provider_prompt(
    value: Any,
    *,
    max_claims: int,
    evidence_ref_limit: int,
    ...
    compacted: list[dict[str, Any]] = []
    for authority_item in _dicts(value, limit=max(1, int(evidence_ref_limit))):
        compacted.append(
            {
                key: field_value
                for key, field_value in {
                    "schema_version": authority_item.get("schema_version"),
                    "receipt_id": authority_item.get("receipt_id"),
                    "affected_claim_ids": _strings(
                        authority_item.get("affected_claim_ids"),
                        limit=max_claims,
                    ),
                    "gap_authority_refs": _strings(
                        authority_item.get("gap_authority_refs"),
                        limit=evidence_ref_limit,
                    ),
                    "supported_node_ids": _strings(
                        authority_item.get("supported_node_ids"),
                        limit=evidence_ref_limit,
                    ),
                    "supported_document_ids": _strings(
                        authority_item.get("supported_document_ids"),
                        limit=evidence_ref_limit,
                    ),
                    "allowed_evidence_refs": _strings(
                        authority_item.get("allowed_evidence_refs"),
                        limit=evidence_ref_limit,
                    ),
                    "search_master_authority_present": authority_item.get(
                        "search_master_authority_present"
                    ),
                    "diagnostic_only_refs_executable": authority_item.get(
                        "diagnostic_only_refs_executable"
                    ),
                    "non_entailed_refs_executable": authority_item.get(
                        "non_entailed_refs_executable"
                    ),
                    "question_evidence_authority_sha256": stable_digest(
                        authority_item
                    ),
                    "provider_prompt_compacted": True,
                }.items()
...
def _compact_provider_notebook_for_context_guard(
    ...
    *,
    ...
    pre_compaction_chars: int,
    level: str,
) -> dict[str, Any]:
    """Project verbose review notebooks into a deterministic provider prompt."""

    item = _dict(notebook)
    strict = str(level or "") == "minimum"
    quote_chars = 56 if strict else 180
    claim_chars = 72 if strict else 220
    investigation_chars = 48 if strict else 140
    receipt_chars = 16 if strict else 80
    evidence_limit = max(1, min(8 if strict else 24, int(evidence_ref_limit)))
    question_chars = 40 if strict else 90
    answer_chars = 40 if strict else 90
    explanation_chars = 20 if strict else 45
    compacted: dict[str, Any] = {
        "runtime_phase": item.get("runtime_phase"),
        "phase_instruction": (
            "AI_REVIEW: preserve server-owned claim ledger; return claims=[], exclusions=[]; "
            "choose exactly one allowed action. Use Search/hydrate only when review_action_contract allows it; "
            "ask evidence-bound questions only from question_evidence_authority; use source_only only for eligible IDs."
        )
        if item.get("runtime_phase") == "AI_REVIEW"
        else (
            "AI_FRAME_AND_PLAN: extract source-bound atomic claims and emit one or two neutral Search calls "
            "covering the claim batch; no decisions or questions."
        ),
        "review_action_contract": _compact_review_action_contract_for_provider_prompt(
            _dict(item.get("review_action_contract")),
            max_claims=max_claims,
            evidence_ref_limit=evidence_limit,
        ),
        "protocol_feedback": _compact_protocol_feedback_for_provider_prompt(
            item.get("protocol_feedback"),
            max_claims=max_claims,
            evidence_ref_limit=evidence_limit,
            instruction_chars=180 if strict else 320,
        ),
        "brain_revision": item.get("brain_revision"),
        "semantic_authority": item.get("semantic_authority"),
        "source_authority": {
            key: value
            for key, value in _dict(item.get("source_authority")).items()
            if key
            in {
                "source_purpose",
                "source_purpose_decision_source",
                "source_trust",
                "verified_manual_self_memory_unit_ids",
                "personal_self_attestation_requires_provider_posture",
                "external_fact_corroboration_policy",
            }
        ...
        "claim_ledger": _compact_claim_ledger_for_provider_prompt(
            _dicts(item.get("claim_ledger"), limit=max_claims),
            max_claims=max_claims,
            quote_chars=quote_chars,
            claim_chars=claim_chars,
            investigation_chars=investigation_chars,
        ),
        "search_receipts": [
            _compact_search_receipt_for_provider_prompt(
                receipt,
                context_excerpt_chars=receipt_chars,
                master_chars=receipt_chars,
                continuation_chars=receipt_chars,
                evidence_ref_limit=evidence_limit,
                include_document_references=False,
            )
            for receipt in _dicts(item.get("search_receipts"), limit=evidence_limit)
        ],
        "search_continuation_briefs": [
            {
                key: (str(value)[:receipt_chars] if isinstance(value, str) else value)
            ...
            for brief in _dicts(item.get("search_continuation_briefs"), limit=2)
        ],
        "document_evidence_receipts": [],
        "hydrated_evidence": _compact_hydrated_evidence_for_provider_prompt(
            _dict(item.get("hydrated_evidence")),
            _dicts(item.get("search_receipts"), limit=evidence_limit),
            max_nodes=2 if strict else 4,
            canonical_chars=40 if strict else 80,
            summary_chars=40 if strict else 60,
        ),
        "decisions": [
            {
                key: (
                    _compact_source_blind_text(value, limit=120 if strict else 180)
                ...
                in {
                    ...
                    "impact",
                    "next_step",
                    "no_clarification_can_change_decision",
                    "no_clarification_explanation",
                    "evidence_refs",
                }
                and value not in (None, "", [])
            }
            for decision in _dicts(item.get("decisions"), limit=max_claims)
        ],
        "clarifications": _compact_clarifications_for_provider_prompt(
            _dicts(item.get("clarifications"), limit=max_questions),
            max_claims=max_claims,
            max_questions=max_questions,
            question_text_chars=question_chars,
            answer_chars=answer_chars,
            summary_chars=explanation_chars,
            explanation_chars=explanation_chars,
        ),
        "clarification_answers": _compact_clarification_answers_for_provider_prompt(
            _dict(item.get("clarification_answers")),
            answer_chars=answer_chars,
        ),
        "question_evidence_authority": _compact_question_evidence_authority_for_provider_prompt(
            item.get("question_evidence_authority"),
            max_claims=max_claims,
            evidence_ref_limit=evidence_limit,
        ),
        "question_policy": {
            "maximum_pending_questions": _dict(item.get("question_policy")).get(
                "maximum_pending_questions"
            ),
        ...
        "clarification_answer_policy": {
            ...
            "closed_clarification_gaps": _closed_clarification_gaps_for_provider_prompt(
                _dicts(item.get("clarifications"), limit=max_questions),
                max_claims=max_claims,
                max_questions=max_questions,
                question_text_chars=question_chars,
        ...
        "turn": item.get("turn"),
        "response_phase": item.get("response_phase"),
        "provider_prompt_compaction": {
            "schema_version": "agvm.grow_provider_prompt_compaction.v1",
            "reason": "provider_context_guard",
            "level": level,
            "pre_compaction_chars": pre_compaction_chars,
            "target_user_prompt_chars": prompt_budget_chars,
            "static_policy_projection": True,
            "claim_ledger_projected": True,
            "full_receipts_and_hydrated_evidence_remain_in_investigation_receipt": True,
            "full_clarifications_remain_in_investigation_receipt": True,
        },
    }
    return {
        key: value
...
def _canonical_source_bound_temporal_day(value: str) -> str | None:
    """Return an ISO day only for explicit, source-bound day-level strings."""
    ...
    iso_match = re.fullmatch(r"(\d{4})-(\d{2})-(\d{2})(?:[tT].*)?", lowered)
    if iso_match:
        ...
        return None
    normalized = re.sub(r"\b(\d{1,2})(st|nd|rd|th)\b", r"\1", lowered)
    ...
    month: int
    year: int
    if parts[0].isdigit() and parts[1] in _TEMPORAL_MONTH_INDEX and parts[2].isdigit():
        day = int(parts[0])
        month = _TEMPORAL_MONTH_INDEX[parts[1]]
        year = int(parts[2])
    elif parts[0] in _TEMPORAL_MONTH_INDEX and parts[1].isdigit() and parts[2].isdigit():
        month = _TEMPORAL_MONTH_INDEX[parts[0]]
        day = int(parts[1])
        year = int(parts[2])
    else:
        return None
...
def _source_bound_temporal_days_in_text(value: str) -> list[str]:
    text = str(value or "")
    days: list[str] = []
    for match in re.finditer(r"\b\d{4}-\d{2}-\d{2}\b", text):
        day = _canonical_source_bound_temporal_day(match.group(0))
        if day:
            days.append(day)
    month_names = "|".join(re.escape(month) for month in _TEMPORAL_MONTH_INDEX)
    natural_patterns = (
...
def _temporal_field_value_is_source_bound(
    *,
    ...
    if not normalized_value:
        return False
    if normalized_value == exact_text:
        return True
    first = basis_text.find(exact_text)
    if first < 0 or first != basis_text.rfind(exact_text):
        return False
    normalized_day = _canonical_source_bound_temporal_day(normalized_value)
    exact_day = _canonical_source_bound_temporal_day(exact_text)
    if normalized_day is None:
        return False
    if exact_day == normalized_day:
        return True
    exact_text_days = _source_bound_temporal_days_in_text(exact_text)
    return exact_text_days == [normalized_day]
...
def _normalize_temporal_scope(
    ...
    *,
    ...
    This deliberately does not infer a temporal role or source a date from
    metadata.  The model owns the semantic interpretation; the server owns
    whether every cited temporal mention is an exact substring of the source, an
    answered human clarification, or Search-returned hydrated brain evidence.
    Day-level normalized values are accepted only when they are a deterministic
    equivalent of one uniquely source-bound cited span.
    """
    ...
    for mention in raw_mentions:
        ...
        if not basis_ref or not exact_text:
            return None, "temporal_mention_exact_span_invalid"
        exact_span_matches = (
            0 <= span_start < span_end <= len(basis_text)
            and basis_text[span_start:span_end] == exact_text
        )
        if not exact_span_matches:
            rebound_start = basis_text.find(exact_text)
            if rebound_start < 0 or rebound_start != basis_text.rfind(exact_text):
                return None, "temporal_mention_exact_span_invalid"
            span_start = rebound_start
            span_end = rebound_start + len(exact_text)
        for field in ("observed_at", "valid_from", "valid_to"):
            if field in supports_fields and not _temporal_field_value_is_source_bound(
...
def _source_attribution(
    ...
    requested_source_options = _dict(source_options.get("requested"))
    effective_source_options = _dict(source_options.get("effective"))
    source_uri = str(
    ...
    source_trust = str(
        ...
        or effective_source_options.get("source_trust")
        or requested_source_options.get("source_trust")
...
def default_grow_investigator_provider(request: dict[str, Any]) -> tuple[dict[str, Any] | None, str | None, dict[str, Any]]:
    try:
        ...
        request_budget = _grow_provider_request_budget(request)
        if request_budget["within_context_guard"] is not True:
            return (
                None,
                (
                    "grow_provider_context_budget_exceeded:"
                    f"estimated_input_tokens={request_budget['estimated_input_tokens']};"
                    f"input_chars={request_budget['input_chars']}"
                ),
                {
        ...
        payload, error = structured_json(
            ...
            role="grow_semantic",
            max_output_tokens=int(request_budget["max_output_tokens"]),
            api_key_override=api_key_override,
            execution_metadata=provider_metadata,
            retry_on_max_output_tokens=False,
        )
...
def _system_prompt() -> str:
    return (
        ...
        "For an explicitly user-selected self_memory source, or an exact-span verified manual source that directly states the user's own first-person "
        "personal preference, "
        "independent Search means an independent lookup for an existing, duplicate, or conflicting "
        "brain memory; it does not mean that the user's own preference, opinion, or instruction needs third-party corroboration. Classify those personal "
        "self-attestations as epistemic_posture=opinion or instruction and keep the user attribution explicit. Never use this personal authority for an "
        "asserted_fact, reported_claim, hypothesis, public event, company fact, or other externally verifiable proposition, even when supplied through a "
        "self_memory request. "
        "A direct user-authored preference, opinion, or instruction in a user_asserted manual source is primary evidence for that preference itself: "
        "do not ask the user to independently confirm that stated preference; ask only for a genuinely missing value, scope, or ambiguity not present in the source span. "
...
def _repair_personal_self_subject_anchor(
    *,
    ...
    """Rebind obvious self-memory subject anchors without inventing source text.

    The server still refuses fuzzy subject binding for ordinary factual claims.
    This narrowly handles a common provider failure mode for direct personal
    preferences: the provider emits a semantic identifier such as
    ``user_release_status_update_preferences`` as ``neutral_subject_anchor``
    even though the source quote itself contains an exact first-person/user
    subject.  Requiring another provider repair for that byte-span typo makes
    valid BYOK Grow runs fail closed after spending tokens.
    """

    claim_text = " ".join(
        str(value or "")
        for value in (
            raw_claim.get("natural_language_claim"),
            raw_claim.get("exact_quote"),
            exact_quote,
        )
    ).casefold()
    posture = str(raw_claim.get("epistemic_posture") or "").strip().casefold()
    user_instruction = str(
        _dict(_dict(source_investigation.get("source_request")).get("source_request")).get(
            "user_instruction"
        )
        or _dict(source_investigation.get("source_request")).get("user_instruction")
        or ""
    ).casefold()
    is_personal_self_memory = bool(
        posture in {"opinion", "preference", "personal_preference", "instruction"}
        or "self memory" in user_instruction
        or "personal preference" in user_instruction
        or "the user prefers" in claim_text
        or "user prefers" in claim_text
        or "i prefer" in claim_text
        or "my preference" in claim_text
    )
    if not is_personal_self_memory:
        return None
    ...
    for candidate in (
        ...
        relative_match = exact_quote.find(candidate)
        if relative_match < 0:
            continue
        if exact_quote.find(candidate, relative_match + 1) >= 0:
            continue
        start = quote_start + relative_match
        end = start + len(candidate)
        if quote_start <= start < end <= quote_end:
            return candidate, start, end
    return None


def _initial_claim_quote_is_atomic(exact_quote: str) -> bool:
    """Reject an obviously aggregated initial claim without inventing semantics.

    Grow still delegates semantic claim extraction to the provider.  The server
    only enforces a narrow structural invariant here: one initial claim cannot
    cite three or more clearly delimited sentences as its assertion span.  Two
    sentences remain allowed because a short follow-up sentence may qualify or
    scope the first assertion.
    """

    text = str(exact_quote or "").strip()
    if not text:
        return True
    sentence_count = 1
...
def _bind_initial_claims(
    ...
    *,
    ...
    for raw_claim in raw_claims:
        ...
        if not _initial_claim_quote_is_atomic(exact_quote):
            return [], [], {}, f"claim_not_atomic:{claim_key}"
        ...
        if not anchor_is_server_bound:
            ...
            if not anchor_is_server_bound:
                repaired_anchor = _repair_personal_self_subject_anchor(
                    raw_claim=raw_claim,
                ...
                if repaired_anchor is not None:
                    (
                        neutral_subject_anchor,
                        subject_anchor_start,
                        subject_anchor_end,
        ...
        if supplied_source_sha256 and supplied_source_sha256 != source_unit_content_sha256:
            # The provider-authored source-unit hash is not source authority on
            # the initial turn.  At this point the server has already rebound
            # source_unit_id, exact_quote, quote offsets, and subject anchor
            # against canonical raw_text.  Preserve fail-closed source binding by
            # deriving the immutable content digest from that canonical unit
            # instead of spending an AI repair on a stale/advisory hash.
            pass
        ...
        result.append(
            {
                ...
                "epistemic_posture": claim.get("epistemic_posture"),
...
def run_grow_investigation(
    ...
    *,
    ...
    effective_search_concurrency = min(
        max(1, int(resolved_budget.search_concurrency)),
        max(1, int(llm_provider_concurrency_limit())),
    )
    ...
    repairing_complete_without_decisions = False
    repairing_unanswered_clarification_claim = False
    repairing_forbidden_review_search = False
    repairing_search_call_budget = False
    repairing_memory_hydration_binding = False
    repairing_document_hydration_binding = False
    repairing_decision_target_hydration = False
    repairing_decision_target_receipt_binding = False
    repairing_decision_targets_not_allowed = False
    repairing_decision_single_target = False
    repairing_source_only_authority = False
    repairing_unusable_decision_receipt = False
    repairing_defer_attestation = False
    repairing_defer_attestation_for_source_only_personal = False
    repairing_receipt_authority_for_source_only_personal = False
    ...
    repairing_decision_evidence_refs = False
    ...
    def claim_has_exact_source_span(claim_id: str) -> bool:
    ...
    def explicit_self_memory_instruction() -> bool:
        source_request = _dict(source_investigation.get("source_request"))
        instruction = str(
            source_request.get("user_instruction")
            or source_investigation.get("operator_instruction")
            or ""
        ).strip()
        folded = " ".join(
            instruction.casefold().replace("-", " ").replace("_", " ").split()
        )
        tokens = {
            token.strip(".,:;!?()[]{}\"'")
        ...
        first_token = (
            folded.split()[0].strip(".,:;!?()[]{}\"'") if folded else ""
        )
        ambiguous_tokens = {
            "if",
            "unless",
        ...
        return bool(
            0 < len(instruction) <= 500
            and "?" not in instruction
            and first_token in {"create", "store", "save", "record"}
            and "self memory" in folded
            and not tokens.intersection(ambiguous_tokens)
        )

    def explicit_self_memory_source_marker() -> bool:
        source_request = _dict(source_investigation.get("source_request"))
        if str(source_request.get("input_kind") or "").strip() != "manual_text":
            return False
        for unit in fact_units_by_id.values():
            raw_text = " ".join(str(unit.get("raw_text") or "").casefold().split())
            prefix = raw_text[:240]
            if (
                prefix.startswith("direct user memory:")
                or prefix.startswith("self memory:")
                or prefix.startswith("personal preference:")
            ):
                return True
        return False

    def explicit_user_selected_self_memory() -> bool:
        source_purpose = _dict(source_investigation.get("source_purpose"))
        source_request = _dict(source_investigation.get("source_request"))
        source_options = _dict(source_request.get("options"))
        requested_options = _dict(source_options.get("requested"))
        effective_options = _dict(source_options.get("effective"))
        if str(source_request.get("input_kind") or "").strip() != "manual_text":
            return False
        source_trust = str(
            effective_options.get("source_trust")
            or requested_options.get("source_trust")
            or ""
        ).strip()
        requested_treat_as = str(
            effective_options.get("treat_as")
            or requested_options.get("treat_as")
            or ""
        ).strip()
        if (
            str(source_purpose.get("purpose") or "").strip() == "self_memory"
            and source_purpose.get("requires_confirmation") is not False
        ):
            return False
        if source_trust == "user_asserted":
            decision_source = str(source_purpose.get("decision_source") or "").strip()
            if decision_source == "user_option" and requested_treat_as == "self_memory":
                return True
            if requested_treat_as == "auto" and explicit_self_memory_instruction():
                return True
        if explicit_self_memory_source_marker():
            return True

        return False

    def verified_manual_unit(unit: Mapping[str, Any]) -> bool:
        """Verify exact manual acquisition without upgrading its trust label."""

        provenance = _dict(unit.get("provenance"))
        acquisition_proof = _dict(unit.get("acquisition_proof"))
        extraction_trace = _dict(unit.get("extraction_trace"))
        raw_text = str(unit.get("raw_text") or "")
        unit_digest = _canonical_upstream_digest(
            unit.get("content_digest")
            or acquisition_proof.get("content_digest")
            or provenance.get("hash")
        )
        acquisition_method = str(
            unit.get("acquisition_method")
            or acquisition_proof.get("method")
            or extraction_trace.get("method")
            or ""
        ).strip()
        return bool(
            str(source_investigation.get("investigation_id") or "").strip()
            and str(unit.get("kind") or "").strip() == "manual_block"
            and str(provenance.get("source_type") or "").strip() == "manual_text"
            and acquisition_proof.get("verified") is True
            and str(acquisition_proof.get("kind") or "").strip() == "manual_input"
            and acquisition_method == "manual_text_passthrough"
            and unit_digest == _content_sha256(raw_text)
        )

    def verified_user_asserted_manual_unit(unit: Mapping[str, Any]) -> bool:
        unit_attribution = _source_attribution(source_investigation, unit)
        if (
            str(unit_attribution.get("source_trust") or "").strip()
            != "user_asserted"
            and not explicit_self_memory_source_marker()
        ):
            return False
        return verified_manual_unit(unit)

    def inferred_direct_personal_preference_claim(claim_id: str) -> bool:
        claim = _dict(claims_by_id.get(claim_id))
        source_request = _dict(source_investigation.get("source_request"))
        source_options = _dict(source_request.get("options"))
        requested_options = _dict(source_options.get("requested"))
        ...
        posture = str(claim.get("epistemic_posture") or "").strip().casefold()
        if (
            posture not in {"opinion", "instruction"}
            or str(source_request.get("input_kind") or "").strip() != "manual_text"
            or requested_treat_as != "auto"
        ):
            return False
        exact_quote = " ".join(str(claim.get("exact_quote") or "").casefold().split())
        if not exact_quote:
            return False
        direct_preference_markers = (
            r"^i prefer\b",
            r"^i currently prefer\b",
            r"^i would prefer\b",
            r"^i'd prefer\b",
            r"^my preference\b",
            r"^my preferred\b",
        ...
        return any(re.search(marker, exact_quote) for marker in direct_preference_markers)

    def inferred_user_asserted_personal_preference_claim(claim_id: str) -> bool:
        claim = _dict(claims_by_id.get(claim_id))
        posture = str(claim.get("epistemic_posture") or "").strip().casefold()
        if posture not in {"opinion", "instruction"}:
            return False
        source_request = _dict(source_investigation.get("source_request"))
        source_options = _dict(source_request.get("options"))
        requested_options = _dict(source_options.get("requested"))
        ...
        if (
            str(source_request.get("input_kind") or "").strip() != "manual_text"
            or source_trust != "user_asserted"
        ):
            return False
        texts = [
            ...
            for value in (
                ...
                claim.get("natural_language_claim"),
            )
            if str(value or "").strip()
        ]
        if not texts:
            return False
        user_asserted_preference_markers = (
            r"^preferenza\b",
            r"^preferenza operativa dichiarata\b",
            r"^preferenza dichiarata\b",
            r"^l'utente preferisce\b",
            r"^l’utente preferisce\b",
            r"^l utente preferisce\b",
            r"^l'utente vuole\b",
            r"^l’utente vuole\b",
            r"^the user prefers\b",
            r"^user prefers\b",
            r"^the user wants\b",
            r"^user wants\b",
            r"^the user would like\b",
            r"^user would like\b",
        )
        return any(
            re.search(marker, text)
            for text in texts
            for marker in user_asserted_preference_markers
        )

    def self_attested_personal_claim_eligible(claim_id: str) -> bool:
        """Authorize only provenance-verified personal assertions, never external facts."""
        ...
        if (
            unit is None
            or not claim_has_exact_source_span(claim_id)
        ):
            return False
        # Opinion/instruction is the provider's explicit semantic assertion that
        # the memory records the user's own preference or direction.  Factual,
        # reported, and hypothetical claims still require independent evidence.
        if str(claim.get("epistemic_posture") or "").strip() not in {
            "opinion",
            "instruction",
        }:
            return False
        if explicit_user_selected_self_memory() and verified_user_asserted_manual_unit(unit):
            return True
        if (
            verified_user_asserted_manual_unit(unit)
            and inferred_user_asserted_personal_preference_claim(claim_id)
        ):
            return True
        return bool(
            inferred_direct_personal_preference_claim(claim_id)
            and all(
                inferred_direct_personal_preference_claim(other_claim_id)
                for other_claim_id in claims_by_id
            )
            and verified_manual_unit(unit)
        )

    def claim_has_verified_user_asserted_manual_source(claim_id: str) -> bool:
        claim = _dict(claims_by_id.get(claim_id))
        unit_id = str(claim.get("source_unit_id") or "").strip()
        unit = fact_units_by_id.get(unit_id)
        return bool(
            unit is not None
            and claim_has_exact_source_span(claim_id)
            and verified_user_asserted_manual_unit(unit)
        )

    def personal_absence_answer_closes_gap(
        *,
        closed_question: Mapping[str, Any],
        candidate_claim_ids: set[str],
    ) -> bool:
        """Treat explicit personal/no-proof answers as closed gaps, not re-questions.

        This does not create a memory decision. It only routes a repeated
        clarification into the existing terminal provider repair path.
        """

        if str(closed_question.get("answer_state") or "unanswered") not in {
            "answered",
            "deferred",
        }:
            return False
        closed_claim_ids = set(
            _strings(closed_question.get("affected_claim_ids"), limit=resolved_budget.max_claims)
        )
        matching_claim_ids = sorted(candidate_claim_ids.intersection(closed_claim_ids))
        if (
            not matching_claim_ids
            or set(candidate_claim_ids) - closed_claim_ids
            or not all(
                claim_has_verified_user_asserted_manual_source(claim_id)
                for claim_id in matching_claim_ids
            )
        ):
            return False
        answer = closed_question.get("answer")
        ...
        absence_markers = {
            ...
            "does not have external proof",
            "does not have independent proof",
            "assenza di prova",
            "assenza di prove",
            "nessuna prova",
            "nessuna evidenza",
            "nessuna prova esterna",
            "nessuna evidenza esterna",
            "nessuna prova indipendente",
            "nessuna evidenza indipendente",
            "senza prova esterna",
            "senza prove esterne",
            "non è corroborata",
            "non è corroborato",
        ...
        return bool(
            any(marker in normalized_answer for marker in absence_markers)
            and any(marker in normalized_answer for marker in personal_markers)
        )

    def receipt_is_bound_personal_search_boundary(
        claim_id: str,
        receipt: Mapping[str, Any],
    ) -> bool:
        """Bind one terminal Search review to a verified personal claim.

        This proves only that required Search ran against the current brain. It
        does not promote the receipt to evidence/question/decision authority.
        """

        return bool(
            self_attested_personal_claim_eligible(claim_id)
            and claim_id in list(receipt.get("affected_claim_ids") or [])
            and str(receipt.get("brain_revision") or "") == normalized_revision
            and (
                not str(receipt.get("grow_receipt_authority_sha256") or "").strip()
                or str(receipt.get("grow_receipt_authority_sha256") or "")
                == grow_receipt_authority_sha256
            )
            and _receipt_is_attested_terminal_review_boundary(receipt)
        )
    ...
    def source_only_claim_boundary_eligible(claim_id: str) -> bool:
        ...
        unit_id = str(claim.get("source_unit_id") or "").strip()
        unit = fact_units_by_id.get(unit_id)
        if unit is None or not claim_has_exact_source_span(claim_id):
            return False
        unit_attribution = _source_attribution(source_investigation, unit)
        ...
        if not str(source_uri or "").strip():
            ...
            if not local_document_identity and not self_attested_personal_claim_eligible(
                claim_id
            ):
                return False
        if any(
        ...
        return any(
            ...
            and (
                (
                    (
                        not str(
                            receipt.get("grow_receipt_authority_sha256") or ""
                        ).strip()
                        or str(receipt.get("grow_receipt_authority_sha256") or "")
                        == grow_receipt_authority_sha256
                    )
                    and (
                        _receipt_evidence_usable(receipt)
                        or _receipt_question_usable(receipt)
                    )
                )
                or receipt_is_bound_personal_search_boundary(claim_id, receipt)
            )
            for receipt in receipts
        )
    ...
    def personal_source_only_receipt_ids(claim_id: str) -> list[str]:
        if not self_attested_personal_claim_eligible(claim_id):
            return []
        return sorted(
            {
                str(receipt.get("receipt_id") or "").strip()
                for receipt in receipts
                if claim_id in list(receipt.get("affected_claim_ids") or [])
                and str(receipt.get("brain_revision") or "") == normalized_revision
                and _is_v3_search_receipt(receipt)
                and _receipt_has_semantic_firewall(receipt)
    ...
    def build_personal_source_only_decision(
        claim_id: str,
        receipt_ids: Sequence[str],
        *,
        base: Mapping[str, Any] | None = None,
    ) -> dict[str, Any] | None:
        normalized_receipt_ids = _strings(receipt_ids, limit=16)
        claim = _dict(claims_by_id.get(claim_id))
        candidate = {
            **_dict(base),
            "claim_id": claim_id,
            ...
            "target_node_ids": [],
            "evidence_receipt_ids": normalized_receipt_ids,
            "compatible_but_distinct": False,
            "rationale": (
                "The provenance-verified manual self-memory is authoritative for "
                "the user's own preference, opinion, or instruction. The required "
                "same-revision Search boundary found no decision-usable existing target."
            ),
            "source_claim_summary": str(
                claim.get("natural_language_claim")
                or _dict(base).get("source_claim_summary")
                or ""
            ),
            "brain_evidence_summary": (
                "Search completed an attested same-revision review boundary without "
                "a decision-usable existing target."
            ),
            "comparison": (
                "The verified source records the user's own personal assertion; "
                "Search supplied no authoritative target to update or conflict to resolve."
            ),
            "reason": (
                "A user's own preference, opinion, or instruction is self-attested "
                "and does not require third-party corroboration."
            ),
            "impact": (
                "The source-bound memory can be presented for explicit review without "
                "claiming independent factual verification."
            ),
            "next_step": (
                "Review the source-only preview and apply it explicitly if it matches "
                "the user's intent."
            ),
            "evidence_refs": normalized_receipt_ids,
        }
        return candidate if source_only_decision_authorized(candidate) else None

    def personal_review_receipt_ids(claim_id: str) -> list[str]:
        if not self_attested_personal_claim_eligible(claim_id):
            return []
        return sorted(
            {
                ...
                and (
                    _receipt_evidence_usable(receipt)
                    or _receipt_question_usable(receipt)
                    or _receipt_decision_usable(receipt)
                    or receipt_is_bound_personal_search_boundary(claim_id, receipt)
                )
                and str(receipt.get("receipt_id") or "").strip()
            }
    ...
    def build_personal_fail_closed_defer_decision(
        claim_id: str,
        receipt_ids: Sequence[str],
    ) -> dict[str, Any] | None:
        normalized_receipt_ids = _strings(receipt_ids, limit=16)
        claim = _dict(claims_by_id.get(claim_id))
        if not normalized_receipt_ids or not self_attested_personal_claim_eligible(claim_id):
            return None
        return {
            "claim_id": claim_id,
            "decision": "defer",
            ...
            "compatible_but_distinct": False,
            "rationale": (
                "The provider completed review of a provenance-verified personal assertion "
                "but did not emit a safe memory mutation decision after repair. The server "
                "records a fail-closed non-applicable defer instead of applying memory."
            ),
            "no_clarification_can_change_decision": True,
            "no_clarification_explanation": (
                "The review already had same-revision Search evidence; without a provider-authored "
                "target or source-only decision, local Grow must not mutate memory."
            ),
            "source_claim_summary": str(claim.get("natural_language_claim") or ""),
            "brain_evidence_summary": (
                "Same-revision Search completed, but no safe provider-authored write decision remained."
            ),
            "comparison": (
                "The source is a self-attested personal assertion; existing Search evidence did not "
                "lead to a valid target-bound mutation in the repaired response."
            ),
            "reason": "Fail-closed local Grow review preserves safety and avoids invalid apply-ready output.",
            "impact": "No memory mutation is proposed for this claim in the current Grow run.",
            "next_step": "Provide a more distinct source or explicit clarification if a new memory is required.",
            "evidence_refs": normalized_receipt_ids,
        }

    def personal_source_only_terminal_payload() -> dict[str, Any] | None:
        decided_claim_ids = {
            str(decision.get("claim_id") or "") for decision in normalized_decisions
        }
        unresolved_claim_ids = sorted(set(claims_by_id) - decided_claim_ids)
        if not unresolved_claim_ids or any(
            bool(question.get("required_for_preview"))
            and str(question.get("answer_state") or "unanswered") == "unanswered"
        ...
        for claim_id in unresolved_claim_ids:
            if any(
                claim_id in list(receipt.get("affected_claim_ids") or [])
                and str(receipt.get("brain_revision") or "") == normalized_revision
                and bool(_strings(receipt.get("evidence_node_ids"), limit=96))
                for receipt in receipts
            ):
                # A discovered target still needs provider comparison and exact
                # hydration; source-only is reserved for a genuinely targetless
                # Search boundary.
                return None
            receipt_ids = personal_source_only_receipt_ids(claim_id)
            decision = build_personal_source_only_decision(claim_id, receipt_ids)
            if decision is None:
                return None
            policy_decisions.append(decision)
        return {
            "schema_version": GROW_INVESTIGATOR_TURN_SCHEMA_VERSION,
            "status": "complete",
            "claims": [],
            "answer_claims": [],
            "exclusions": [],
            ...
            "summary": (
                "Server policy prepared source-only previews for provenance-verified "
                "personal assertions after same-revision Search found no decision target."
            ),
        }

    def personal_optional_question_source_only_decisions(
        question_claim_ids: set[str],
        *,
        allow_evidence_usable_receipts: bool = False,
    ) -> list[dict[str, Any]] | None:
        decided_claim_ids = {
            str(decision.get("claim_id") or "") for decision in normalized_decisions
        }
        unresolved_claim_ids = set(claims_by_id) - decided_claim_ids
        if not unresolved_claim_ids or question_claim_ids != unresolved_claim_ids:
            return None
        policy_decisions: list[dict[str, Any]] = []
        for claim_id in sorted(unresolved_claim_ids):
            if not self_attested_personal_claim_eligible(claim_id):
                return None
            receipt_ids = sorted(
                {
                    ...
                    for receipt in receipts
                    if claim_id in list(receipt.get("affected_claim_ids") or [])
                    and str(receipt.get("brain_revision") or "")
                    == normalized_revision
                    and (
                        not str(
                            receipt.get("grow_receipt_authority_sha256") or ""
                    ...
                    and (
                        _receipt_is_attested_nondispositive_question_boundary(receipt)
                        if allow_evidence_usable_receipts
                        else _receipt_is_attested_terminal_question_boundary(receipt)
                    )
                    and str(receipt.get("receipt_id") or "").strip()
                }
            )
    ...
    def explicit_personal_store_instruction() -> bool:
        source_request = _dict(source_investigation.get("source_request"))
        instruction = str(
            source_request.get("user_instruction")
            or source_investigation.get("operator_instruction")
        ...
        return bool(
            0 < len(instruction) <= 500
            and "?" not in instruction
            and first_token in {"add", "create", "make", "store", "save", "record"}
            and "self memory" in folded
            and not tokens.intersection(ambiguous_tokens)
        )

    def explicit_personal_truth_question(
        question: Mapping[str, Any],
    ) -> bool:
        """Recognize a non-dispositive truth question by authority, not wording."""
        ...
        return bool(
            str(question.get("decision_effect") or "").strip() == "truth"
            and affected_claim_ids
            and all(
                self_attested_personal_claim_eligible(claim_id)
    ...
    def normalize_personal_self_attestation_defer(
        decision: Mapping[str, Any],
    ) -> dict[str, Any]:
        """Convert one semantically inconsistent personal defer to source-only.

        The provider must already have classified the exact-span claim as a
        personal opinion/instruction, cited a completed same-revision Search
        boundary, and emitted no target.  This policy never applies to external
        facts, and the general defer validator still requires explicit
        no-clarification attestation for every defer that is not converted.
        """

        normalized = dict(decision)
        claim_id = str(normalized.get("claim_id") or "").strip()
        receipt_ids = _strings(normalized.get("evidence_receipt_ids"), limit=16)
        if (
            str(normalized.get("decision") or "").strip() != "defer"
            or _strings(normalized.get("target_node_ids"), limit=32)
            or not receipt_ids
            or not self_attested_personal_claim_eligible(claim_id)
        ):
            return normalized
        return (
            build_personal_source_only_decision(
                claim_id,
                receipt_ids,
                base=normalized,
            )
    ...
    def self_attested_personal_decision_authorized(
        decision: Mapping[str, Any],
    ) -> bool:
        """Allow bounded personal-memory updates from self attestation after Search."""

        claim_id = str(decision.get("claim_id") or "").strip()
        decision_value = str(decision.get("decision") or "").strip()
        if (
            decision_value not in {"enrich_existing", "evolve_existing"}
            or not self_attested_personal_claim_eligible(claim_id)
        ):
            return False
        target_ids = _strings(decision.get("target_node_ids"), limit=32)
        receipt_ids = _strings(decision.get("evidence_receipt_ids"), limit=16)
        if len(target_ids) != 1 or not receipt_ids:
            return False
        ...
        for receipt_id in receipt_ids:
            ...
            if str(receipt.get("brain_revision") or "") != normalized_revision:
                return False
            if not _is_v3_search_receipt(receipt) or not _receipt_has_semantic_firewall(
                receipt
            ):
                return False
            if (
                str(receipt.get("grow_receipt_authority_sha256") or "").strip()
                and str(receipt.get("grow_receipt_authority_sha256") or "")
                != grow_receipt_authority_sha256
            ):
                return False
            if not (
                ...
                return False
            receipt_node_ids.update(
                str(node_id)
                for node_id in list(receipt.get("evidence_node_ids") or [])
            )
        return target_ids[0] in receipt_node_ids
    ...
    def context_builder(
        ...
        nonlocal repairing_forbidden_review_search, repairing_memory_hydration_binding
        nonlocal repairing_document_hydration_binding
        nonlocal repairing_complete_without_decisions
        nonlocal repairing_search_call_budget
        nonlocal repairing_decision_target_hydration
        nonlocal repairing_decision_target_receipt_binding
        nonlocal repairing_decision_targets_not_allowed
        nonlocal repairing_decision_single_target
        nonlocal repairing_source_only_authority
        nonlocal repairing_unusable_decision_receipt
        nonlocal repairing_defer_attestation
        nonlocal repairing_defer_attestation_for_source_only_personal
        nonlocal repairing_receipt_authority_for_source_only_personal
        ...
        nonlocal repairing_decision_evidence_refs
        ...
        verified_manual_self_memory_unit_ids = sorted(
            str(unit.get("unit_id") or "")
            for unit in fact_units
            if explicit_user_selected_self_memory()
            and verified_user_asserted_manual_unit(unit)
        ...
        source_only_claim_ids = sorted(
            claim_id
            for claim_id in claims_by_id
            if source_only_claim_boundary_eligible(claim_id)
            and (
                claim_id in question_only_claim_ids
                or self_attested_personal_claim_eligible(claim_id)
            )
        )
        self_attested_personal_claim_ids = sorted(
            claim_id
            for claim_id in claims_by_id
            if self_attested_personal_claim_eligible(claim_id)
            and any(
                claim_id in list(receipt.get("affected_claim_ids") or [])
                and str(receipt.get("brain_revision") or "") == normalized_revision
                and _is_v3_search_receipt(receipt)
                ...
                and (
                    ...
                    or _receipt_question_usable(receipt)
                    or _receipt_decision_usable(receipt)
                    or receipt_is_bound_personal_search_boundary(
                        claim_id,
                        receipt,
                    )
                )
        ...
        self_attested_personal_claim_set = set(self_attested_personal_claim_ids)
        non_personal_question_only_claim_ids = [
            claim_id
            for claim_id in question_only_claim_ids
            if claim_id not in self_attested_personal_claim_set
        ]
        ...
        repairing_complete_without_decisions = bool(
            protocol_feedback
            and str(protocol_feedback.get("detail") or "").startswith(
                "complete_claim_decisions_missing:"
            )
            and bool(claims)
        )
        ...
        repairing_memory_hydration_binding = bool(
            protocol_feedback
            and str(protocol_feedback.get("detail") or "").startswith(
                "hydration_node_not_returned_by_search:"
            )
            and bool(receipts)
        )
        ...
        repairing_decision_target_receipt_binding = bool(
            protocol_feedback
            and str(protocol_feedback.get("detail") or "").startswith(
                "decision_target_not_in_search_receipt:"
            )
            and bool(receipts)
        )
        repairing_decision_targets_not_allowed = bool(
            protocol_feedback
            and str(protocol_feedback.get("detail") or "").startswith(
                "decision_targets_not_allowed:"
            )
            and bool(receipts)
        )
        repairing_decision_single_target = bool(
            protocol_feedback
            and str(protocol_feedback.get("detail") or "").startswith(
                "decision_single_target_required:"
            )
            and bool(receipts)
        )
        repairing_source_only_authority = bool(
            protocol_feedback
            and str(protocol_feedback.get("detail") or "").startswith(
                "source_only_source_bound_authority_invalid:"
            )
            and bool(receipts)
        )
        ...
        repairing_decision_evidence_refs = bool(
            protocol_feedback
            and str(protocol_feedback.get("detail") or "")
            == "decision_evidence_refs_invalid"
            and bool(receipts)
        )
        ...
        repairing_defer_attestation = bool(
            protocol_feedback
            and str(protocol_feedback.get("detail") or "")
            == "defer_requires_no_clarification_attestation"
            and bool(receipts)
        )
        repairing_defer_attestation_for_source_only_personal = bool(
            repairing_defer_attestation
            and claims_by_id
            and set(claims_by_id).issubset(self_attested_personal_claim_set)
            and set(claims_by_id).issubset(set(source_only_claim_ids))
        )
        repairing_receipt_authority_for_source_only_personal = bool(
            repairing_unusable_decision_receipt
            and claims_by_id
            and set(claims_by_id).issubset(self_attested_personal_claim_set)
            and set(claims_by_id).issubset(set(source_only_claim_ids))
        ...
        decision_receipt_repair_requires_questions = bool(
            repairing_unusable_decision_receipt
            and bool(non_personal_question_only_claim_ids)
            and (
        ...
        if repairing_closed_clarification_gap_reopened and question_only_claim_ids:
            ...
            repairing_defer_attestation_for_source_only_personal
            or repairing_receipt_authority_for_source_only_personal
        ):
            review_allowed_actions = ["ask_questions", "source_only"]
        elif repairing_defer_attestation and question_only_claim_ids and source_only_claim_ids:
            review_allowed_actions = ["ask_questions", "source_only", "defer"]
        elif repairing_defer_attestation and question_only_claim_ids:
            review_allowed_actions = ["ask_questions", "defer"]
        elif repairing_defer_attestation:
            review_allowed_actions = ["ask_questions", "decide", "defer"]
        ...
        elif question_only_claim_ids and self_attested_personal_claim_ids:
            review_allowed_actions = [
                "hydrate",
                "ask_questions",
                "decide",
        ...
        elif self_attested_personal_claim_ids:
            review_allowed_actions = [
                "hydrate",
                "ask_questions",
                "decide",
        ...
        if repairing_initial_source_binding:
            protocol_feedback["instruction"] = (
                ...
                "translate, normalize, fuzzy-match, or reconstruct source text. Preserve opaque identifiers exactly, "
                "including hyphens, slashes, capitalization, punctuation, and spacing, by copying from raw_text instead "
                "of retyping from memory. Recalculate zero-based quote_start "
                ...
                "do not return exclusions, answer_claims, questions, or decisions in this repair turn, and do not wait "
                "for a separate first-search repair turn. Keep claim_id and source_unit_content_sha256 "
            ...
            if initial_binding_detail.startswith("claim_not_atomic:"):
                protocol_feedback["instruction"] += (
                    " The reported claim_not_atomic error means one claim cited three or more independently "
                    "delimited source sentences. Split that span into separate atomic claims, each bound to the "
                    "smallest exact quote and exact subject anchor that expresses one assertion. Preserve every "
                    "factual statement, personal preference, and instruction up to max_claims; do not replace them "
                    "with one aggregate summary."
                )
                protocol_feedback["claim_atomicity_requirements"] = {
                    "one_assertion_per_claim": True,
                    "smallest_exact_source_span": True,
                    "aggregate_multi_sentence_summary_forbidden": True,
                }
            if repairing_initial_temporal_binding:
                protocol_feedback["instruction"] += (
                    ...
                    "canonical source unit named by the claim's source_unit_id. If the existing atomic exact_quote "
                    "already contains the necessary temporal text, correct only the temporal mention span. If the "
                    "date or interval is semantically necessary and belongs to the same atomic source assertion but "
                    "sits outside the claim quote, keep the mention bound to the canonical source unit and expand "
                    "exact_quote only when the assertion would otherwise be non-atomic or ambiguous. If no temporal "
                    "assertion is semantically necessary, set temporal_scope to null. Never copy a date from source "
                    "metadata, infer one from audit time, or bind a mention to a different source unit."
                )
            ...
            if repairing_initial_temporal_binding:
                protocol_feedback["temporal_binding_requirements"] = {
                    "source_mentions_bound_to_claim_source_unit": True,
                    "exact_zero_based_offsets": True,
                    "temporal_scope_may_be_null": True,
                    "atomic_quote_may_expand_for_ambiguity_only": True,
            ...
            protocol_feedback["first_search_wave_call_cap"] = effective_search_concurrency
            ...
            if answers:
                protocol_feedback["instruction"] = (
                    "The previous answer_claim was not validly bound to the answered clarification. Repair the turn "
                    "without adding an answer claim: return answer_claims=[], claims=[] and exclusions=[]. A confirming "
                    "answer, a negative answer, or an answer saying that there are no updates closes the existing gap; "
                    "it does not create a new atomic fact. Preserve the canonical claim ledger and finish with an "
                    "evidence-led decision or defer, using only exact claim_ids and receipt/node evidence refs already "
                    "present in the notebook. Do not reopen the answered gap or ask a catch-all reconfirmation."
                )
            else:
                protocol_feedback["instruction"] = (
        ...
        elif repairing_memory_hydration_binding:
            invalid_hydration_node_ids = _strings(
                str(protocol_feedback.get("detail") or "").split(":", 1)[1].split(","),
                limit=96,
            )
            protocol_feedback["instruction"] = (
                "The previous hydrate_memory_objects call targeted an ID that was not returned by a bound Search "
                "receipt. Preserve the canonical claim ledger by returning claims=[] and exclusions=[]. Do not "
                "Search again and do not invent, abbreviate, or use placeholder node IDs. If full memory text is "
                "needed, return status=continue with hydrate_memory_objects using only exact IDs from "
                "search_returned_memory_node_ids_by_claim for the affected claims. Otherwise ask evidence-bound "
                "human questions or defer. A reviewable or question-usable receipt does not authorize a memory "
                "decision until its required target evidence is hydrated."
            )
            ...
            protocol_feedback["search_returned_memory_node_ids_by_claim"] = {
                claim_id: sorted(
                    {
                        str(node_id or "").strip()
                        for receipt in receipts
                        if claim_id
                        in _strings(receipt.get("affected_claim_ids"), limit=128)
                        for node_id in list(receipt.get("evidence_node_ids") or [])
                        if str(node_id or "").strip()
                    }
                )
            ...
            protocol_feedback["invalid_hydration_node_ids"] = invalid_hydration_node_ids
        ...
        elif repairing_decision_targets_not_allowed:
            affected_claim_ids = [
                claim_id
                for claim_id in _strings(
                    str(protocol_feedback.get("detail") or "").split(":", 1)[1].split(","),
            ...
            protocol_feedback["instruction"] = (
                "The previous AI_REVIEW attached target_node_ids to a target-free new_memory, source_only or defer "
                "decision. Preserve the canonical claim ledger by returning claims=[] and exclusions=[]. Do not "
                "Search or hydrate again, and do not silently change the semantic decision merely to retain a target. "
                "Re-evaluate the affected claim from the unchanged receipt and hydrated evidence. If the intended "
                "decision remains new_memory, source_only or defer, return it with target_node_ids=[]. If the evidence "
                "instead requires a target-bound mutation, return the correct target decision with exactly one "
                "already-hydrated ID that belongs to the affected claim's cited Search receipt. Otherwise ask an "
                "evidence-bound clarification. Every repaired decision remains subject to the normal authority, "
                "novelty, receipt, attestation and explanation checks."
            )
            protocol_feedback["affected_claim_ids"] = affected_claim_ids
            protocol_feedback["hydrated_search_target_ids_by_claim"] = {
                claim_id: sorted(
                    {
                        str(node_id or "").strip()
                        for receipt in receipts
                        ...
                        in _strings(receipt.get("affected_claim_ids"), limit=128)
                        for node_id in list(receipt.get("evidence_node_ids") or [])
                        if str(node_id or "").strip() in hydrated_evidence
                    }
                )
                for claim_id in affected_claim_ids
            }
            protocol_feedback["allowed_actions"] = [
                "target_free_decision_without_targets",
                "single_hydrated_search_target_decision",
                "ask_clarification",
            ]
            protocol_feedback["forbidden_actions"] = [
                "search",
                "hydrate",
                "target_on_target_free_decision",
                "target_outside_claim_receipt",
                "preview",
                "apply",
            ]
        elif repairing_decision_single_target:
            protocol_feedback["instruction"] = (
                "The previous AI_REVIEW proposed a memory mutation with more than one target. Preserve the canonical "
                "claim ledger by returning claims=[] and exclusions=[]. Do not Search or hydrate again, and never "
                "choose a target by list position. Return exactly one safe terminal action: emit the mutation with "
                "exactly one already-hydrated target only when the cited evidence uniquely supports that target; ask "
                "an evidence-bound clarification when identity or target choice is genuinely unresolved; or use "
                "source_only/new_memory only when the existing source-bound or novelty authority rules allow it. "
                "A decision-usable receipt does not authorize converting a multi-target mutation to source_only."
            )
        ...
        elif repairing_source_only_authority:
            protocol_feedback["instruction"] = (
                "The previous AI_REVIEW converted a claim to source_only using a Search receipt that is decision-usable "
                "or otherwise not a targetless source boundary. Preserve the canonical claim ledger by returning claims=[] "
                "and exclusions=[]. Do not Search again and do not repeat source_only for this claim. If a memory mutation "
                "is justified, emit exactly one decision with either no target for a distinct new_memory, or exactly one "
                "already hydrated target returned by the cited Search receipt for enrich_existing/evolve_existing. If the "
                "target is not hydrated yet, return status=continue with hydrate_memory_objects for a small motivated "
                "subset of Search-returned evidence_node_ids. If no safe mutation is available, ask an evidence-bound "
                "clarification or defer."
            )
            protocol_feedback["allowed_actions"] = [
                "hydrate_search_returned_targets",
                "target_free_new_memory",
                "single_target_personal_update",
                "ask_clarification",
                "defer",
            ]
            protocol_feedback["forbidden_actions"] = [
                "search",
                "source_only_with_decision_usable_receipt",
                "multi_target_mutation",
                "target_outside_search_receipt",
                "preview",
                "apply",
            ]
            protocol_feedback["search_returned_memory_node_ids"] = sorted(
        ...
        elif repairing_decision_target_receipt_binding:
            protocol_feedback["instruction"] = (
                "The previous AI_REVIEW chose target_node_ids that were not returned by the cited Search receipt. "
                "Preserve the canonical claim ledger by returning claims=[] and exclusions=[]. Do not Search again, "
                "do not hydrate, and do not cite or select any target that is absent from the existing receipt evidence_node_ids. "
                "Repair the result by returning status=complete with decisions=[] only if no decision is now safe, "
                "or with an evidence-bound target-free decision such as new_memory/source_only/defer when the cited Search "
                "receipt and human explanation fields justify it. If the receipt supports only a human choice, return "
                "status=needs_clarification with evidence-bound questions. Set answer_claims=[] and tool_calls=[]."
            )
            protocol_feedback["allowed_actions"] = [
                "decide_without_target",
                "ask_clarification",
                "defer",
            ]
            protocol_feedback["forbidden_actions"] = [
            ...
            protocol_feedback["instruction"] = (
                ...
                "clarification can change the closed missing-detail gap. When required_decision is defer, every "
                "decision must use decision=defer. Do not ask any question, Search, hydrate, preview, apply, or "
                "invent a fallback."
            )
            ...
            if question_only_claim_ids:
                protocol_feedback["required_decision"] = "defer"
        ...
        elif repairing_decision_evidence_refs:
            all_review_receipts = [*receipts, *document_receipts]
            protocol_feedback["instruction"] = (
                "The previous AI_REVIEW cited decision evidence_refs outside the exact receipt authority. Preserve "
                "the canonical claim ledger by returning claims=[] and exclusions=[]. Do not Search or hydrate again. "
                "Return the same evidence-led review action, but cite only exact receipt IDs or evidence node IDs from "
                "decision_evidence_refs_by_claim for each affected claim. Do not invent, abbreviate, translate, or "
                "reuse a reference from another claim. All ordinary decision, target, novelty and authority checks "
                "still apply."
            )
            protocol_feedback["decision_evidence_refs_by_claim"] = {
                claim_id: sorted(
                    {
                        str(ref or "").strip()
                        for receipt in all_review_receipts
                        if claim_id
                        in _strings(receipt.get("affected_claim_ids"), limit=128)
                        for ref in [
                            receipt.get("receipt_id"),
                            *list(receipt.get("evidence_node_ids") or []),
                        ]
                        if str(ref or "").strip()
                    }
                )
            ...
            protocol_feedback["forbidden_actions"] = [
                ...
                "hydrate",
                "invent_evidence_ref",
                "cross_claim_evidence_ref",
                "preview",
                "apply",
            ]
        elif repairing_defer_attestation:
            if repairing_defer_attestation_for_source_only_personal:
                protocol_feedback["instruction"] = (
                    "The previous AI_REVIEW deferred a provenance-verified personal self-memory claim even though the "
                    "review contract lists every unresolved claim in both self_attested_personal_claim_ids and "
                    "source_only_eligible_claim_ids. Preserve the canonical claim ledger by returning claims=[] and "
                    "exclusions=[]. Do not Search, hydrate, preview, apply, invent tool results, or return another "
                    "defer decision in this repair. Lack of third-party corroboration is not maintenance uncertainty "
                    "for the user's own stated preference, opinion, or instruction. Choose exactly one schema-valid "
                    "terminal action from the unchanged notebook: return status=needs_clarification with evidence-bound "
                    "questions only if a specific human answer can change whether this is the user's own current "
                    "preference or instruction, or return status=complete with one provider-authored source_only "
                    "decision per unresolved claim when the source-bound Search boundary supports a target-free "
                    "source-only preview."
                )
            else:
                protocol_feedback["instruction"] = (
                    "The previous AI_REVIEW returned one or more defer decisions without the required "
                    "no-clarification-can-change-decision attestation. Preserve the canonical claim ledger by returning "
                    "claims=[] and exclusions=[]. Do not Search, hydrate, preview, apply, or invent tool results. Choose "
                    "exactly one schema-valid terminal action from the unchanged notebook: return status=needs_clarification "
                    "with evidence-bound questions if a human answer can change the outcome, or return status=complete "
                    "with one evidence-bound decision per unresolved claim. If any decision is defer, it must include "
                    "no_clarification_can_change_decision=true and a non-empty no_clarification_explanation explaining "
                    "why no human clarification can change that maintenance decision. Non-defer decisions remain allowed "
                    "only when the existing receipt and hydrated evidence authorize them."
                )
            protocol_feedback["allowed_actions"] = review_allowed_actions
            protocol_feedback["forbidden_actions"] = [
                "search",
            ...
            protocol_feedback["decision_attestation_requirements"] = {
                "defer_requires_no_clarification_can_change_decision": True,
                "defer_requires_nonempty_no_clarification_explanation": True,
                "schema_requires_attestation_fields_on_returned_decisions": True,
            }
        elif repairing_unusable_decision_receipt:
            if repairing_receipt_authority_for_source_only_personal:
                protocol_feedback["instruction"] = (
                    "The previous AI_REVIEW chose a receipt-authority-invalid memory decision for a "
                    "provenance-verified personal self-memory claim. The Search receipt is not decision_usable, "
                    "but the review contract lists every unresolved claim in both self_attested_personal_claim_ids "
                    "and source_only_eligible_claim_ids. Preserve the canonical claim ledger by returning claims=[] "
                    "and exclusions=[]. Do not Search, hydrate, preview, apply, invent tool results, return defer, "
                    "or emit target-bound/generic memory decisions. Choose exactly one schema-valid terminal action "
                    "from the unchanged notebook: return status=needs_clarification with evidence-bound questions "
                    "only if a specific human answer can change whether this is the user's own current preference "
                    "or instruction, or return status=complete with one provider-authored target-free source_only "
                    "decision per unresolved claim."
                )
                protocol_feedback["required_output_shape"] = {
                    "claims": [],
                    "exclusions": [],
                    "answer_claims": [],
                    "tool_calls": [],
                    "questions_or_source_only_decisions": True,
                }
                protocol_feedback["allowed_terminal_actions"] = [
                    "ask_questions",
                    "source_only",
                ]
            elif decision_receipt_repair_requires_questions:
                protocol_feedback["instruction"] = (
        ...
        elif repairing_complete_without_decisions:
            protocol_feedback["instruction"] = (
                "The previous AI_REVIEW returned status=complete without one evidence-bound decision per unresolved "
                "claim. An empty complete response is invalid. Preserve the canonical claim ledger by returning "
                "claims=[] and exclusions=[]. Choose exactly one valid action from review_action_contract: return "
                "status=continue with only an allowed hydration tool call when target evidence must be inspected; "
                "return status=needs_clarification with only evidence-bound questions when a human answer can change "
                "the outcome; or return status=complete with exactly one evidence-bound decision for every ID in "
                "required_decision_claim_ids. Do not Search again, invent evidence, preview, apply, or return another "
                "empty terminal response."
            )
            missing_detail = str(protocol_feedback.get("detail") or "").split(":", 1)[-1]
            protocol_feedback["required_decision_claim_ids"] = [
                claim_id
                for claim_id in _strings(
                    missing_detail.split(","),
                    limit=resolved_budget.max_claims,
                )
                if claim_id in claims_by_id
            ]
        ...
        notebook = {
            ...
            "phase_instruction": (
                ...
                "When source authority identifies an explicit, provenance-verified self_memory request, represent the user's own stated preference, "
                "opinion, or instruction with epistemic_posture=opinion or instruction and preserve that attribution. Do not apply those postures to "
                "externally verifiable facts merely because the request says self_memory. "
                "use no more than first_search_wave_call_cap search_brain tool calls; combine compatible natural-language "
                "information needs when provider capacity is one, and ensure their "
                ...
                "target. Explain that this is source-bound memory, not independent verification. For claim IDs listed in "
                "self_attested_personal_claim_ids, the verified manual source is authoritative for the user's own preference, "
                "opinion, or instruction: do not defer merely because no independent third party can corroborate it. Use "
                "source_only to create when Search finds no decision-usable target, or enrich_existing/evolve_existing for a "
                "Search-returned target after its canonical text is hydrated. This authority never applies to external facts."
                + (
            ...
            "review_action_contract": {
                ...
                "initial_search_policy": "one_search_wave_with_up_to_provider_capacity_calls_covering_claim_batch",
                "first_search_wave_call_cap": effective_search_concurrency,
                ...
                "self_attested_personal_claim_ids": self_attested_personal_claim_ids,
                "self_attested_personal_policy": {
                    "source_purpose": "explicit_user_selected_self_memory",
                    "source_trust": "user_asserted",
                    "direct_verified_manual_personal_preference_allowed": True,
                    "requires_verified_manual_input_digest": True,
                    "allowed_epistemic_postures": ["opinion", "instruction"],
                    "independent_corroboration_required": False,
                    "search_boundary_required": True,
                    "target_update_requires_search_returned_hydrated_target": True,
                    "allowed_target_decisions": ["enrich_existing", "evolve_existing"],
                    "external_fact_authority": False,
                },
                "source_only_policy": {
                    "decision": "source_only",
                    "requires_server_bound_exact_source_span": True,
                    "requires_durable_source_identity_and_trust_attribution": True,
                    "source_uri_exception": "verified_manual_self_attested_personal_memory",
                ...
                "hydration_policy": {
                    ...
                    "server_auto_hydrates_provisional_decision_targets": True,
                    "provisional_target_decision_requires_post_hydration_review": True,
            ...
            "semantic_authority": authority,
            "source_authority": {
                "source_purpose": _dict(
                    source_investigation.get("source_purpose")
                ).get("purpose"),
                "source_purpose_decision_source": _dict(
                    source_investigation.get("source_purpose")
                ).get("decision_source"),
                "source_trust": (
                    _dict(
                        _dict(
                            _dict(source_investigation.get("source_request")).get(
                                "options"
                            )
                        ).get("effective")
                    ).get("source_trust")
                ...
                "external_fact_corroboration_policy": "unchanged_fail_closed",
            },
            "temporal_evidence_policy": {
                "claim_temporal_scope_authority": "provider_evidence_bound",
            ...
            "source_units": [dict(item) for item in source_material] if not claims else [],
            "source_binding_findings": [
                dict(finding)
                for exclusion in exclusions
                if (finding := _dict(exclusion.get("human_finding")))
            ],
            "claim_ledger": [dict(item) for item in claims],
            "search_receipts": [dict(item) for item in receipts],
            "search_continuation_briefs": _search_continuation_briefs(receipts),
            "document_evidence_receipts": [dict(item) for item in document_receipts],
            "hydrated_evidence": {
                str(node_id): dict(material)
                for node_id, material in hydrated_evidence.items()
                if isinstance(material, Mapping)
            },
            "decisions": [dict(item) for item in normalized_decisions],
            "clarifications": [dict(item) for item in normalized_questions],
        ...
        if len(encoded) > resolved_budget.max_notebook_chars:
            notebook["search_receipts"] = [
                _compact_search_receipt_for_provider_prompt(
                    ...
                    master_chars=700,
                    continuation_chars=700,
                    evidence_ref_limit=resolved_budget.max_evidence_references,
                    include_document_references=True,
                )
                for item in receipts
            ]
            notebook["hydrated_evidence"] = (
                _compact_hydrated_evidence_for_provider_prompt(
                    hydrated_evidence,
                    receipts,
                    max_nodes=resolved_budget.max_hydration_nodes_per_review,
                    canonical_chars=2_000,
                    summary_chars=1_000,
                )
            )
            encoded = _provider_prompt_json(notebook)
        provider_safe_notebook_chars = min(
            int(resolved_budget.max_notebook_chars),
            7_500 if protocol_feedback or hydrated_evidence else int(resolved_budget.max_notebook_chars),
        )
        if len(encoded) > provider_safe_notebook_chars:
            for item in notebook.get("source_material") or []:
                if isinstance(item, dict):
                    item["raw_text"] = str(item.get("raw_text") or "")[:600]
                    item["title"] = str(item.get("title") or "")[:200]
            for claim in notebook.get("claim_ledger") or []:
                if isinstance(claim, dict):
                    for key in (
                        ...
                        claim[key] = str(claim.get(key) or "")[:600]
            notebook["source_binding_findings"] = [
                {
                    key: (str(value)[:500] if isinstance(value, str) else value)
                    for key, value in dict(finding).items()
                }
                for finding in list(notebook.get("source_binding_findings") or [])[:4]
            ...
            notebook["search_receipts"] = [
                _compact_search_receipt_for_provider_prompt(
                    ...
                    master_chars=500,
                    continuation_chars=400,
                    evidence_ref_limit=resolved_budget.max_evidence_references,
                    include_document_references=True,
                )
                for receipt in list(notebook.get("search_receipts") or [])
                if isinstance(receipt, Mapping)
            ]
            notebook["search_continuation_briefs"] = [
                {
                    key: (str(value)[:400] if isinstance(value, str) else value)
                    for key, value in dict(brief).items()
                }
                for brief in list(notebook.get("search_continuation_briefs") or [])[:6]
                if isinstance(brief, dict)
            ]
            for material in _dict(notebook.get("hydrated_evidence")).values():
                if isinstance(material, dict):
                    material["canonical_text"] = str(material.get("canonical_text") or "")[:700]
                    material["summary"] = str(material.get("summary") or "")[:300]
                    provenance = _dict(material.get("provenance"))
                    material["provenance"] = {
                        key: provenance.get(key)
                        for key in ("source_type", "source_trust", "source_uri")
            ...
            notebook["hydrated_evidence"] = (
                _compact_hydrated_evidence_for_provider_prompt(
                    notebook.get("hydrated_evidence") or {},
                    notebook.get("search_receipts") or [],
                    max_nodes=resolved_budget.max_hydration_nodes_per_review,
                    canonical_chars=700,
                    summary_chars=300,
                )
            )
            for decision in notebook.get("decisions") or []:
                if isinstance(decision, dict):
                    for key in (
                        ...
                        decision[key] = str(decision.get(key) or "")[:500]
            notebook["clarifications"] = _compact_clarifications_for_provider_prompt(
                notebook.get("clarifications") or [],
                max_claims=resolved_budget.max_claims,
                max_questions=resolved_question_limit,
                question_text_chars=500,
                answer_chars=500,
                summary_chars=300,
                explanation_chars=300,
            )
            clarification_policy = _dict(notebook.get("clarification_answer_policy"))
            if clarification_policy:
                clarification_policy["instruction"] = (
                    "Answered or deferred clarifications close the exact asked gap. "
                    "Do not ask paraphrases; decide from remaining evidence or defer "
                    "only when no clarification can change the decision."
                )
                clarification_policy["closed_clarification_gaps"] = (
                    _closed_clarification_gaps_for_provider_prompt(
                        notebook.get("clarifications") or [],
                        max_claims=resolved_budget.max_claims,
                        max_questions=resolved_question_limit,
                        question_text_chars=500,
                ...
                notebook["clarification_answer_policy"] = clarification_policy
            notebook["clarification_answers"] = (
                _compact_clarification_answers_for_provider_prompt(
                    notebook.get("clarification_answers") or {},
                    answer_chars=500,
                )
            )
            encoded = json.dumps(notebook, ensure_ascii=False, sort_keys=True, default=str)
        if len(encoded) > provider_safe_notebook_chars:
            for item in notebook.get("source_material") or []:
                if isinstance(item, dict):
                    item["raw_text"] = str(item.get("raw_text") or "")[:300]
            notebook["search_receipts"] = [
                _compact_search_receipt_for_provider_prompt(
                    receipt,
                    context_excerpt_chars=300,
                    master_chars=250,
                    continuation_chars=250,
                    evidence_ref_limit=max(1, min(128, resolved_budget.max_evidence_references)),
                    include_document_references=True,
                    document_reference_limit=4,
                )
                for receipt in list(notebook.get("search_receipts") or [])
                if isinstance(receipt, Mapping)
            ]
            for material in _dict(notebook.get("hydrated_evidence")).values():
                if isinstance(material, dict):
                    material["canonical_text"] = str(material.get("canonical_text") or "")[:300]
                    material["summary"] = str(material.get("summary") or "")[:200]
            notebook["hydrated_evidence"] = (
                _compact_hydrated_evidence_for_provider_prompt(
                    notebook.get("hydrated_evidence") or {},
                    notebook.get("search_receipts") or [],
                    max_nodes=max(1, min(12, resolved_budget.max_hydration_nodes_per_review)),
                    canonical_chars=300,
                    summary_chars=200,
                )
            )
            notebook["source_binding_findings"] = []
        encoded = _provider_prompt_json(notebook)
        provider_user_prompt_budget = _grow_provider_user_prompt_char_budget(
            system_prompt=_system_prompt(),
            schema=GROW_INVESTIGATOR_RESPONSE_SCHEMA,
        )
        if len(encoded) > provider_user_prompt_budget:
            notebook["hydrated_evidence"] = (
                _compact_hydrated_evidence_for_provider_prompt(
                    notebook.get("hydrated_evidence") or {},
                    notebook.get("search_receipts") or [],
                    max_nodes=max(1, min(8, resolved_budget.max_hydration_nodes_per_review)),
                    canonical_chars=160,
                    summary_chars=120,
                )
            )
            for item in notebook.get("source_material") or []:
                if isinstance(item, dict):
                    item["raw_text"] = str(item.get("raw_text") or "")[:180]
            ...
            for claim in notebook.get("claim_ledger") or []:
                if isinstance(claim, dict):
                    for key in (
                        ...
                        claim[key] = str(claim.get(key) or "")[:220]
            notebook["search_receipts"] = [
                _compact_search_receipt_for_provider_prompt(
                    receipt,
                    context_excerpt_chars=160,
                    master_chars=160,
                    continuation_chars=160,
                    evidence_ref_limit=max(1, min(64, resolved_budget.max_evidence_references)),
                    include_document_references=False,
                )
                for receipt in list(notebook.get("search_receipts") or [])
                if isinstance(receipt, Mapping)
            ...
            notebook["clarifications"] = _compact_clarifications_for_provider_prompt(
                ...
                max_claims=resolved_budget.max_claims,
                max_questions=resolved_question_limit,
                question_text_chars=90,
                answer_chars=90,
                summary_chars=45,
                explanation_chars=45,
            )
            clarification_policy = _dict(notebook.get("clarification_answer_policy"))
            if clarification_policy:
                clarification_policy["instruction"] = (
            ...
            notebook["provider_prompt_compaction"] = {
                "schema_version": "agvm.grow_provider_prompt_compaction.v1",
                "reason": "provider_context_guard",
                "pre_compaction_chars": len(encoded),
                "target_user_prompt_chars": provider_user_prompt_budget,
                "full_receipts_and_hydrated_evidence_remain_in_investigation_receipt": True,
                "full_clarifications_remain_in_investigation_receipt": True,
            }
            encoded = _provider_prompt_json(notebook)
        if len(encoded) > provider_user_prompt_budget:
            notebook = _compact_provider_notebook_for_context_guard(
                notebook,
                max_claims=resolved_budget.max_claims,
                max_questions=resolved_question_limit,
                evidence_ref_limit=resolved_budget.max_evidence_references,
                prompt_budget_chars=provider_user_prompt_budget,
                pre_compaction_chars=len(encoded),
                level="bounded",
            )
            encoded = _provider_prompt_json(notebook)
        ...
        if len(encoded) > provider_user_prompt_budget:
            raise RuntimeError(
                "grow_provider_prompt_compaction_overflow:"
                f"encoded_chars={len(encoded)};"
                f"target_user_prompt_chars={provider_user_prompt_budget}"
            )
        return notebook
    ...
    def turn_validator(payload: dict[str, Any], turn: int) -> tuple[dict[str, Any] | None, str | None]:
        ...
        unauthorized_hydration_targets: list[str] = []
        for call in normalized_calls:
            if str(call.get("tool_name") or "") != "hydrate_memory_objects":
                continue
            arguments = _dict(call.get("arguments"))
            affected_claim_ids = set(
                _strings(arguments.get("affected_claim_ids"), limit=128)
            )
            receipt_evidence_ids = {
                str(node_id or "").strip()
                for receipt in receipts
                if (
                    not affected_claim_ids
                    or not affected_claim_ids.isdisjoint(
                        set(_strings(receipt.get("affected_claim_ids"), limit=128))
                    )
                )
                and str(receipt.get("brain_revision") or "") == normalized_revision
                for node_id in list(receipt.get("evidence_node_ids") or [])
            ...
            unauthorized_hydration_targets.extend(
                node_id
                for node_id in _strings(arguments.get("node_ids"), limit=96)
                if node_id not in receipt_evidence_ids
            )
        if unauthorized_hydration_targets:
            policy_payload = personal_source_only_terminal_payload()
            if policy_payload is None:
                return None, (
                    "hydration_node_not_returned_by_search:"
                    + ",".join(sorted(set(unauthorized_hydration_targets)))
                )
        ...
        if (
            ...
            payload["decisions"] = [
                normalize_personal_self_attestation_defer(decision)
                for decision in _dicts(
                    payload.get("decisions"),
                    limit=resolved_budget.max_claims,
                )
            ]
            if not payload["decisions"]:
                undecided_claim_ids = sorted(
                    set(claims_by_id)
                    - {
                        str(decision.get("claim_id") or "")
                        for decision in normalized_decisions
                ...
                source_only_decisions: list[dict[str, Any]] = []
                for claim_id in undecided_claim_ids:
                    decision = build_personal_source_only_decision(
                        claim_id,
                        personal_source_only_receipt_ids(claim_id),
                    )
                    if decision is None:
                        break
                    source_only_decisions.append(decision)
                if len(source_only_decisions) == len(undecided_claim_ids):
                    payload["decisions"] = source_only_decisions
        search_tool_calls = [
            call
            for call in normalized_calls
        ...
        if (
            search_tool_calls
            and not receipts
            and len(search_tool_calls) > effective_search_concurrency
        ):
        ...
        self_attested_question_only_claim_set = {
            claim_id
            for claim_id in question_only_claim_ids
            if self_attested_personal_claim_eligible(claim_id)
        }
        non_personal_question_only_claim_ids = [
            claim_id
            for claim_id in question_only_claim_ids
            if claim_id not in self_attested_question_only_claim_set
        ]
        ...
        raw_question_only_personal_target_decisions = [
            decision
            for decision in _dicts(
                payload.get("decisions"), limit=resolved_budget.max_claims
            )
            if str(decision.get("claim_id") or "") in question_only_claim_set
            and self_attested_personal_decision_authorized(decision)
        ]
        post_hydration_question_phase = bool(
            non_personal_question_only_claim_ids
            and hydrated_evidence
        ...
        if (
            ...
            and any(
                ...
                and not self_attested_personal_decision_authorized(decision)
                for decision in _dicts(payload.get("decisions"), limit=resolved_budget.max_claims)
            )
        ):
        ...
        if (
            ...
            and not raw_question_only_personal_target_decisions
        ):
            return None, f"question_only_receipt_action_invalid:{question_only_claim_ids[0]}"
        ...
        suppressed_self_attested_confirmation_claim_ids: set[str] = set()
        for raw_question in raw_turn_questions:
            ...
            # When a mixed source contains both already authoritative personal
            # preferences and a genuinely unresolved claim, do not make the
            # real question compete with redundant preference confirmations.
            # Single-claim flows retain the non-blocking question in the audit
            # trail and are normalized by the existing optional-question policy.
            if (
                len(claims_by_id) > 1
                and any(
                    not self_attested_personal_claim_eligible(claim_id)
                    for claim_id in claims_by_id
                )
                and explicit_personal_truth_question(
                {
                    **raw_question,
                    "affected_claim_ids": affected,
                ...
                suppressed_self_attested_confirmation_claim_ids.update(affected)
                continue
            ...
            if closed_gap_candidates:
                candidate_claim_ids = set(affected)
                personal_absence_closed_question = next(
                    (
                        question
                        for question in closed_gap_candidates
                        if personal_absence_answer_closes_gap(
                ...
                if personal_absence_closed_question is not None:
                    closed_question_id = str(
                        personal_absence_closed_question.get("question_id") or ""
                    ).strip() or "unknown"
                    return None, f"closed_clarification_gap_reopened:{closed_question_id}"
                if provider is None:
        ...
        personal_optional_question_policy = False
        required_turn_questions = [
            question
            for question in latest_turn_questions
            if question.get("required_for_preview") is True
        ]
        personal_truth_questions = [
            question
            for question in required_turn_questions
            if explicit_personal_truth_question(question)
        ]
        if (
            status == "needs_clarification"
            and latest_turn_questions
            and not normalized_calls
            and not payload.get("decisions")
            and all(
                (
                    str(question.get("decision_effect") or "")
                    in {"scope", "derivation", "utility"}
                    or (
                        str(question.get("decision_effect") or "") == "merge"
                        and explicit_personal_store_instruction()
                    )
                    or explicit_personal_truth_question(question)
                )
                for question in required_turn_questions
            )
        ):
            ...
            policy_decisions = personal_optional_question_source_only_decisions(
                question_claim_ids,
                allow_evidence_usable_receipts=bool(
                    personal_truth_questions
                ),
            )
            if policy_decisions is not None:
                for question in latest_turn_questions:
                    if question.get("required_for_preview") is True:
                        question["required_for_preview"] = False
                        question["decision_effect"] = "utility"
                        question["next_step"] = (
                            "Optionally answer this enrichment question after reviewing "
                            "the source-only preview."
                        )
                payload["status"] = "complete"
                payload["questions"] = []
                payload["decisions"] = policy_decisions
                status = "complete"
                personal_optional_question_policy = True
        if (
            status == "needs_clarification"
            and not latest_turn_questions
            and suppressed_self_attested_confirmation_claim_ids
            and not normalized_calls
            and not payload.get("decisions")
        ):
            policy_decisions = personal_optional_question_source_only_decisions(
                suppressed_self_attested_confirmation_claim_ids,
                allow_evidence_usable_receipts=True,
            )
            if policy_decisions is not None:
                payload["status"] = "complete"
                payload["questions"] = []
        ...
        if (
            status != "needs_clarification"
            and latest_turn_questions
            and not personal_optional_question_policy
        ):
            return None, "questions_require_needs_clarification_status"

        latest_turn_decisions = []
        provisional_hydration_targets_by_claim: dict[str, list[str]] = {}
        for raw_decision in _dicts(payload.get("decisions"), limit=resolved_budget.max_claims):
            ...
            if not evidence_refs or any(ref not in allowed_evidence_refs for ref in evidence_refs):
                if not repairing_decision_evidence_refs:
                    return None, "decision_evidence_refs_invalid"
                # A bounded repair already gave the provider the exact whitelist.
                # Preserve its semantic decision while replacing only invalid
                # citations with already-bound receipt authority.
                evidence_refs = [
                    ref for ref in evidence_refs if ref in allowed_evidence_refs
                ]
                if not evidence_refs:
                    evidence_refs = list(receipt_ids)
                raw_decision = {**raw_decision, "evidence_refs": evidence_refs}
            ...
            if decision in TARGET_DECISIONS and unhydrated_targets:
                bound_search_receipts = [
                    receipts_by_id[receipt_id]
                    for receipt_id in receipt_ids
                    if receipt_id in receipts_by_id
                ]
                search_receipt_node_ids = {
                    str(node_id)
                    for receipt in bound_search_receipts
                    for node_id in list(receipt.get("evidence_node_ids") or [])
                }
                personal_target_authorized = (
                    self_attested_personal_decision_authorized(raw_decision)
                )
                # A target decision is also an explicit AI promotion signal.  If
                # every selected target is bound to decision-usable Search proof,
                # or to the narrower verified self-attested personal authority,
                # the runtime can hydrate that exact bounded subset without a
                # provider repair turn. The premature decision is discarded and
                # must be re-issued after canonical text is visible.
                if (
                    len(bound_search_receipts) == len(receipt_ids)
                    and (
                        all(
                            claim_id
                            in list(receipt.get("affected_claim_ids") or [])
                            and _receipt_decision_usable(receipt)
                            for receipt in bound_search_receipts
                        )
                        or personal_target_authorized
                    )
                    and targets
                    and all(node_id in search_receipt_node_ids for node_id in targets)
                ):
                    provisional_hydration_targets_by_claim[claim_id] = unhydrated_targets
                else:
                    return None, f"decision_target_hydration_required:{claim_id}"
        if provisional_hydration_targets_by_claim:
            provisional_target_ids = sorted(
                {
                    node_id
                    for target_ids in provisional_hydration_targets_by_claim.values()
                    for node_id in target_ids
                }
            )
            hydration_state_key = current_review_state_key()
            auto_hydration_available = bool(
                receipts
                and hydration_state_key not in hydration_review_state_keys
                and len(provisional_target_ids)
                <= resolved_budget.max_hydration_nodes_per_review
            )
            if auto_hydration_available:
                arguments = {
                    "affected_claim_ids": sorted(provisional_hydration_targets_by_claim),
                    "query_text": None,
                    "purpose": (
                        "Hydrate only the Search-returned targets explicitly promoted by the provisional "
                        "decision before any mutation decision is accepted."
                    ),
                    "query_strategy": None,
                    "retrieval_mode": None,
                    "max_matches": None,
                    "node_ids": provisional_target_ids,
                }
                server_call_id = (
                    f"grow-tool::{investigation_id}::run-{run_call_authority_sha256[:16]}::"
                    f"turn-{turn}::auto-decision-target-hydration::"
                    f"{stable_digest(arguments)[:20]}"
                )
                payload["status"] = "continue"
                payload["tool_calls"] = [
                    {
                        "call_id": server_call_id,
                        "tool_name": "hydrate_memory_objects",
                        "server_call_ordinal": len(normalized_calls) + 1,
                        "run_call_authority_sha256": run_call_authority_sha256,
                        "provider_call_ref": "runtime:auto-decision-target-hydration",
                ...
                if merged_review_claims is not None:
                    claims = merged_review_claims
                    claims_by_id = {
                        str(item.get("claim_id") or ""): item for item in claims
                    }
                return payload, None
            repair_claim_id = next(iter(provisional_hydration_targets_by_claim))
    ...
    def execute_search_call(
        ...
        query_text, isolated_purpose, isolated_boundary, boundary_ledger = (
            _compose_source_blind_search_query(
                ...
                query_authority_timeout_seconds=(
                    _child_search_query_authority_cap_seconds(resolved_budget)
                ),
            )
        )
        with search_state_lock:
        ...
        try:
            ...
            # Start Search's mode budget only after Grow has completed its
            # separate source-blind query-authority call.  The previous batch
            # timestamp was created before that call and silently shortened
            # Search's admission/spatial/Master lifecycle on slow providers.
            child_search_seconds = search_deadline.require(
                "grow_child_search_runtime",
                cap_seconds=_child_search_budget_cap_seconds(mode),
            )
            child_search_deadline_at_ms = int(
                (time.time() + child_search_seconds) * 1000
            )
            receipt = dict(
                search_runner(
                    graph_snapshot,
                    ...
                    deadline_at_ms=child_search_deadline_at_ms,
                )
            )
            receipt_brain_id = str(receipt.get("brain_id") or "").strip()
            if resolved_brain_id and receipt_brain_id and receipt_brain_id != resolved_brain_id:
            ...
            if (
                _is_v3_search_receipt(receipt)
                and not evidence_ids
                and claim_ids
                and all(self_attested_personal_claim_eligible(claim_id) for claim_id in claim_ids)
                and _receipt_has_semantic_firewall(receipt)
                and receipt.get("usable") is not True
                and receipt.get("decision_usable") is not True
                and not receipt.get("error")
            ):
                search_attestation = _dict(receipt.get("search_execution_attestation"))
                search_attested = bool(
                    str(search_attestation.get("status") or "").strip().casefold()
                    in {"completed", "review_required"}
                    and search_attestation.get("provider_executed") is True
                    and search_attestation.get("fallback_used") is not True
                    and receipt.get("terminal_for_client") is True
                )
                if search_attested:
                    receipt.update(
                        {
                            ...
                            "contract_outcome": "reviewable_needs_more_search",
                            "authoritative_no_match": False,
                            "evidence_usable": False,
                            "question_usable": False,
            ...
            if (
                _is_v3_search_receipt(receipt)
                and receipt.get("usable") is False
                and receipt.get("reviewable") is not True
                and not evidence_ids
                and all(self_attested_personal_claim_eligible(claim_id) for claim_id in claim_ids)
                and _receipt_is_attested_terminal_review_boundary(receipt)
            ):
                receipt["reviewable"] = True
                receipt["contract_outcome"] = "reviewable_personal_source_only_boundary"
                receipt["grow_personal_source_only_boundary"] = True
    ...
    def tool_batch_executor(calls: list[dict[str, Any]], _turn: int) -> list[dict[str, Any]]:
        ...
        if search_calls:
            ...
            search_worker_count = _grow_search_worker_count(
                requested_concurrency=resolved_budget.search_concurrency,
                search_call_count=len(search_calls),
            )
            search_batch_rounds = (
                len(search_calls) + search_worker_count - 1
            ) // search_worker_count
            child_search_batch_cap_seconds = (
                _child_search_batch_cap_seconds(
                    resolved_budget,
                    child_search_mode,
                )
                * search_batch_rounds
            ...
            deadline.require(
                ...
                cap_seconds=child_search_batch_cap_seconds,
            )
            with search_state_lock:
            ...
            executor = ThreadPoolExecutor(
                max_workers=search_worker_count
            )
    ...
    def post_tool_batch_terminalizer(
        ...
        if calls and all(
            str(call.get("tool_name") or "") == "search_brain" for call in calls
        ):
            return personal_source_only_terminal_payload()
        return None
    ...
    def final_validator(payload: dict[str, Any]) -> str | None:
        ...
        for decision in latest_turn_decisions:
            ...
            target_ids = _strings(decision.get("target_node_ids"), limit=32)
            if (
                repairing_decision_targets_not_allowed
                and decision_value not in TARGET_DECISIONS
                and target_ids
            ):
                # The provider already received one bounded semantic repair and
                # repeated only the structurally forbidden target field. Keep its
                # semantic decision unchanged, discard the non-authoritative field,
                # and run every ordinary authority/novelty/receipt gate below.
                target_ids = []
                decision = {**decision, "target_node_ids": []}
            all_receipts_by_id = {**receipts_by_id, **document_receipts_by_id}
            ...
            personal_target_authorized = self_attested_personal_decision_authorized(
                decision
            )
            source_only_override_required = decision_value == "source_only" and (
                repairing_decision_single_target
                or (
                    self_attested_personal_claim_eligible(claim_id)
                    and any(_receipt_decision_usable(item) for item in bound_receipts)
                )
                or any(not _receipt_decision_usable(item) for item in bound_receipts)
            )
            if source_only_override_required and not source_only_authorized:
                if repairing_decision_single_target:
                    return f"source_only_source_bound_authority_invalid_after_repair:{claim_id}"
                return f"source_only_source_bound_authority_invalid:{claim_id}"
            if (
                decision_value != "defer"
                and not source_only_authorized
                and not personal_target_authorized
                and any(
            ...
            if decision_value in TARGET_DECISIONS:
                ...
                if len(target_ids) != 1:
                    return f"decision_single_target_required:{claim_id}"
            elif target_ids:
        ...
        if missing_decisions:
            source_only_decisions: list[dict[str, Any]] = []
            for claim_id in missing_decisions:
                decision = build_personal_source_only_decision(
                    claim_id,
                    personal_source_only_receipt_ids(claim_id),
                )
                    ...
                    + stable_digest(
                        {
                            ...
                            "decision": "source_only",
                            "targets": [],
                            "receipts": decision["evidence_receipt_ids"],
                            "fallback": "personal_complete_missing_decision_source_only",
                        }
                    )[:24]
                )
                source_only_decisions.append({**decision, "decision_id": decision_id})
            if len(source_only_decisions) == len(missing_decisions):
                normalized_decisions.extend(source_only_decisions)
                for decision in source_only_decisions:
                    claim_id = str(decision.get("claim_id") or "")
                    claims_by_id[claim_id]["status"] = "COMPLETE"
                    claims_by_id[claim_id]["decision_id"] = str(
                        decision.get("decision_id") or ""
                    )
                decision_claim_ids = {
                    str(item.get("claim_id") or "") for item in normalized_decisions
                }
                missing_decisions = sorted(set(claims_by_id) - decision_claim_ids)
        if missing_decisions and repairing_complete_without_decisions:
            fail_closed_decisions: list[dict[str, Any]] = []
            for claim_id in missing_decisions:
                decision = build_personal_fail_closed_defer_decision(
                    claim_id,
                    personal_review_receipt_ids(claim_id),
                )
                if decision is None:
                    break
                decision_id = (
                    ...
                    + stable_digest(
                        {
                            ...
                            "targets": [],
                            "receipts": decision["evidence_receipt_ids"],
                            "fallback": "personal_complete_without_decisions",
                        }
                    )[:24]
                )
                fail_closed_decisions.append({**decision, "decision_id": decision_id})
            if len(fail_closed_decisions) == len(missing_decisions):
                normalized_decisions.extend(fail_closed_decisions)
                for decision in fail_closed_decisions:
                    claim_id = str(decision.get("claim_id") or "")
                    claims_by_id[claim_id]["status"] = "DEFERRED"
                    claims_by_id[claim_id]["decision_id"] = str(
                        decision.get("decision_id") or ""
                    )
                decision_claim_ids = {
    ...
    if provider is not None:
        def provider_for_agent(request: dict[str, Any]) -> Any:
            if (
                ...
                request["schema"] = _missing_first_search_repair_schema()
            if (
                repairing_receipt_authority_for_source_only_personal
                and str(request.get("schema_name") or "") == "agvm_grow_investigator_v3"
            ):
                request["schema"] = _source_only_personal_repair_schema()
            elif (
            ...
            if (
                ...
                request["schema"] = (
                    _source_only_personal_repair_schema()
                    if repairing_defer_attestation_for_source_only_personal
                    else _defer_attestation_repair_schema()
                )
            if (
                repairing_initial_source_binding
                and str(request.get("schema_name") or "") == "agvm_grow_investigator_v3"
            ):
                request["schema"] = _initial_source_binding_repair_schema()
            if (
                ...
                request["schema"] = _closed_gap_decision_repair_schema(
                    defer_only=bool(
                        set(closed_clarification_gap_reopened_claim_ids).intersection(
                            current_question_only_claim_ids()
                        )
                    )
                )
            if (
                repairing_source_only_authority
                and str(request.get("schema_name") or "") == "agvm_grow_investigator_v3"
            ):
                request["schema"] = _source_only_authority_repair_schema()
    ...
    agent_result = run_investigative_agent(
        ...
        post_tool_batch_terminalizer=post_tool_batch_terminalizer,
        ...
        repair_limits_by_detail={
            ...
            "clarification_claim_binding_invalid": 1,
            ...
            "defer_requires_no_clarification_attestation": 1,
            "question_evidence_refs_invalid": 1,
            "decision_evidence_refs_invalid": 1,
            "document_hydration_not_discovered_by_search": 1,
            "hydration_node_not_returned_by_search:*": 1,
            ...
            "decision_target_not_in_search_receipt:*": 1,
            "decision_targets_not_allowed:*": 1,
            "decision_single_target_required:*": 1,
            "source_only_source_bound_authority_invalid:*": 1,
            "decision_receipt_not_usable:*": 1,
            "question_only_receipt_action_invalid:*": 1,
            "complete_claim_decisions_missing:*": 1,
