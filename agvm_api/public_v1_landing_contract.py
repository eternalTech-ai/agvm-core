...
def _path_strand_id(path: Any) -> str:
    payload = _as_dict(path)
    return _text(payload.get("mission_id") or payload.get("strand_id"), limit=120)


def _missing_strand_ids(paths: list[dict[str, Any]], expected_strands: list[str]) -> list[str]:
    actual_strands = {
        strand_id
        for path in paths
        if (strand_id := _path_strand_id(path))
    }
    return [strand for strand in expected_strands if strand not in actual_strands]


def _answer_strands_for_ids(answer_strands: Any, strand_ids: list[str]) -> list[dict[str, Any]]:
    wanted = set(strand_ids)
    selected: list[dict[str, Any]] = []
    for item in _as_list(answer_strands):
        strand = _as_dict(item)
        key = _text(strand.get("mission_id") or strand.get("strand_id"), limit=120)
        if key in wanted:
...
def _merge_repair_paths(
    ...
    *,
    missing_strands: list[str],
) -> tuple[list[dict[str, Any]], int]:
    wanted = set(missing_strands)
    seen = {_path_strand_id(path) for path in paths if _path_strand_id(path)}
    merged = [dict(path) for path in paths]
    recovered = 0
    for path in repair_paths:
        strand_id = _path_strand_id(path)
        if not strand_id or strand_id not in wanted or strand_id in seen:
            continue
        merged.append(dict(path))
        seen.add(strand_id)
        recovered += 1
...
def build_public_v1_landing_contract(
    ...
    **_kwargs: Any,
    ...
    if stage_timeout is None:
        stage_timeout = search_ai_stage_timeout_seconds("planner", mode)
    ...
    retry_policy: dict[str, Any] = {
        "schema_version": "agvm.public_v1_landing_provider_retry.v1",
        "retry_used": False,
    ...
    initial_missing_strands = _missing_strand_ids(paths, expected_strands)
    if initial_missing_strands:
        repair_strands = _answer_strands_for_ids(answer_strands, initial_missing_strands)
        repair_prompt_payload = {
            **prompt_payload,
            "schema_version": "agvm.public_v1_landing_prompt.repair.v1",
            "answer_strands": _bounded_value(repair_strands),
            "repair": {
                "reason": "answer_strand_paths_missing",
                "missing_strands": initial_missing_strands,
                "instruction": (
                    "Return AI-authored inverse_answer_paths only for the missing strands. "
                    "Copy each missing input mission_id and strand_id exactly. Do not reuse, "
                    "renumber, or replace already covered strands."
                ),
            },
        }
        repair_payload, repair_error = structured_json_fn(
            model=ai_spatial_model(),
            system_prompt=(
                "You are repairing AGVM's public coordinate-first landing plan. Do not answer the user. "
                "The previous provider output omitted one or more admitted answer strands. "
                "For each missing strand, reuse its mission_id and strand_id exactly and author one compact "
                "coordinate-first inverse answer path. The backend owns execution; do not create deterministic fallbacks."
            ),
            user_prompt=json.dumps(repair_prompt_payload, ensure_ascii=False, separators=(",", ":")),
            schema_name=_PUBLIC_V1_PROVIDER_SCHEMA_NAME,
            schema=_PUBLIC_V1_PROVIDER_SCHEMA,
            timeout=max(0.1, float(stage_timeout)),
            role="ai_spatial",
        ...
        retry_policy["retry_used"] = True
        retry_policy["retry_reason"] = "answer_strand_paths_missing"
        retry_policy["missing_strands"] = initial_missing_strands
        if repair_error or not isinstance(repair_payload, dict):
            retry_policy["retry_status"] = "failed"
            retry_policy["provider_error"] = _text(repair_error or "invalid_json", limit=220)
        else:
            repair_paths = [
                path
                for path_index, item in enumerate(_as_list(repair_payload.get("inverse_answer_paths")), start=1)
                if (path := _normalize_path(item, index=path_index)) is not None
            ]
            paths, recovered_count = _merge_repair_paths(
                paths,
                repair_paths,
                missing_strands=initial_missing_strands,
            )
            retry_policy["recovered_path_count"] = recovered_count
            retry_policy["retry_status"] = (
                "recovered"
                if not _missing_strand_ids(paths, expected_strands)
                else "failed"
            )
    missing_reasons: list[str] = []
    if not paths:
        missing_reasons.append("inverse_answer_paths_missing")
    missing_strands = _missing_strand_ids(paths, expected_strands)
    if missing_strands:
    ...
    return {
        ...
        "provider_retry_policy": retry_policy,
        "cache": {"status": "disabled", "hit": False},
        "routing_authority": "ai_coordinate_first",
        "fallback_used": False,
        "heuristic_result_exposed": False,
