...
_INTERACTIVE_GROW_MIN_WALL_SECONDS = 600
    ...
    def _preview_v3(
        ...
        *,
        ...
        if compiler_timeout_seconds is not None:
            ...
            if _is_interactive_grow_review(
                ...
                requested_timeout = max(
                    requested_timeout,
                    float(_INTERACTIVE_GROW_MIN_WALL_SECONDS),
                )
        ...
        compiler_source_purpose = _grow_v3_compiler_source_purpose(
            source_purpose,
            direct_compiler_claims,
        )
        execution_metadata: dict[str, Any] = {}
        resolved_source_context = dict(canonical_source)
        if isinstance(compiler_source_purpose, Mapping):
            resolved_source_context["source_purpose"] = dict(compiler_source_purpose)
        ...
        bundle = preview_bundle(
            ...
            atlas_payload,
            source_label=source_label,
            # The compiler input above is no longer the raw source: it is the
            # server-issued, investigator-approved claim ledger.  Always enter
            # the source-investigation compiler path so every semantic child is
            # bound to one of those claim ids.  Reusing the original
            # ``self_memory``/manual source type lets the generic compiler emit
            # free nodes without ``source_unit_id`` and the fail-closed binding
            # gate then correctly rejects an otherwise valid Grow preview.
            source_type="source_investigation",
            source_trust=source_trust,
            learning_mode=learning_mode,
            question_limit=question_limit,
            ...
            source_investigation_id=source_investigation_id,
            source_purpose=compiler_source_purpose,
            operator_instruction=operator_instruction,
...
def _grow_v3_questions(investigation: Mapping[str, Any], *, limit: int) -> list[dict[str, Any]]:
    ...
    # Missing/legacy required_for_preview remains blocking. Explicitly optional
    # enrichment stays persisted on the investigation but must not suppress an
    # otherwise applicable preview.
    return [
        dict(item)
        for item in questions
        if isinstance(item, Mapping) and item.get("required_for_preview") is not False
    ]


def _is_interactive_grow_review(
    *,
    ...
    if clarification_answers:
        return True
    if str(learning_mode or "").strip().lower() == "guided_learning":
        return True
    source_request = source_context.get("source_request")
    source_options = source_request.get("options") if isinstance(source_request, Mapping) else {}
    effective_options = source_options.get("effective") if isinstance(source_options, Mapping) else {}
    if isinstance(source_options, Mapping) and (
        source_options.get("pause_on_questions") or source_options.get("semantic_preview")
    ):
        return True
    if isinstance(effective_options, Mapping) and (
        effective_options.get("pause_on_questions") or effective_options.get("semantic_preview")
    ):
        return True
    return bool(
        source_context.get("guided_grow")
        or source_context.get("clarification_questions")
        or source_context.get("open_questions")
        or source_context.get("questions_and_answers")
    )
...
def _grow_v3_source_bound_compiler_result(
    *,
    ...
    source_unit_formation: Mapping[str, Any] | None,
    source_investigation_id: str | None,
    source_purpose: str | Mapping[str, Any] | None,
    operator_instruction: str | None,
    source_context: Mapping[str, Any],
    compiler_timeout_seconds: float | None,
    api_key_override: str | None,
    ...
    source_purpose_text = (
        " ".join(
            str(dict(source_purpose).get(key) or "")
            for key in ("purpose", "source_purpose", "memory_purpose", "decision_source")
        )
        if isinstance(source_purpose, Mapping)
        else str(source_purpose or "")
    )
    source_context_purpose = source_context.get("source_purpose")
    if isinstance(source_context_purpose, Mapping):
        source_purpose_text = (
            source_purpose_text
            + " "
            + " ".join(
                str(dict(source_context_purpose).get(key) or "")
                for key in ("purpose", "source_purpose", "memory_purpose", "decision_source")
            )
        )
    else:
        source_purpose_text = f"{source_purpose_text} {source_context_purpose or ''}"
    short_self_memory_input = bool(
        len(compiler_input) >= 28
        and re.search(
        ...
        and re.search(
            ...
            f"{source_purpose_text} {operator_instruction or ''}",
            re.IGNORECASE,
        )
    )
    if len(compiler_input) < 40 and not short_self_memory_input:
        return None
    execution_metadata: dict[str, Any] = {}
...
def _grow_v3_compiler_source_purpose(
    source_purpose: str | Mapping[str, Any] | None,
    compiler_claims: list[dict[str, Any]],
) -> str | Mapping[str, Any] | None:
    """Carry investigator-approved personal authority into semantic compilation."""

    if compiler_claims and all(
        str(claim.get("epistemic_posture") or "").strip()
        in {"opinion", "instruction"}
        and str(claim.get("decision") or "").strip()
        in {"source_only", "enrich_existing", "evolve_existing"}
        for claim in compiler_claims
    ):
        return {
            "purpose": "self_memory",
            "confidence": 1.0,
            "decision_source": "grow_investigator_personal_claim_authority",
            "requires_confirmation": False,
            "reasons": ["investigator_approved_personal_claims"],
        }
    return source_purpose
...
def _bind_grow_v3_preview_to_claims(
    ...
    suppressed_duplicate_claim_bindings: list[dict[str, Any]] = []
    ...
    for node in raw_nodes:
        ...
        if claim_id in used_claim_ids:
            suppressed_duplicate_claim_bindings.append(
                {
                    "preview_id": str(node.get("id") or ""),
                    "claim_id": claim_id,
                    "decision_id": str(claim.get("decision_id") or ""),
                    "reason": "duplicate_compiler_preview_for_claim",
                }
            )
            continue
    ...
    bound["investigation_authority"] = {
        ...
        "suppressed_duplicate_claim_bindings": suppressed_duplicate_claim_bindings,
