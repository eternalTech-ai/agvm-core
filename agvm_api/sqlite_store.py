...
def _memory_learning_event_idempotency_digest(event: dict[str, Any]) -> str:
    ...
    if stable.get("event_kind") == "maintenance_feedback_recorded":
        # The same immutable Grow feedback packet is projected by preview,
        # status and apply endpoints.  The delivery endpoint is provenance,
        # not packet identity; payload/feedback digests below still reject any
        # conflicting replay for the same feedback_id.
        stable.pop("event_source", None)
    encoded = json.dumps(
        stable,
        ensure_ascii=False,
        sort_keys=True,
...
def rebuild_identity_nucleus_cache() -> dict[str, Any]:
    ...
    try:
        ...
        payload = {}
    with connect() as conn:
        _ensure_identity_nucleus_cache_table(conn)
        conn.execute(
            """
...
def _ensure_identity_nucleus_cache_table(conn: sqlite3.Connection) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS identity_nucleus_cache (
            cache_key TEXT PRIMARY KEY,
